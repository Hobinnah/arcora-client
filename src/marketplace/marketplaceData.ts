...
export const normalizeListing = (listing: Record<string, any>, index: number): MarketplaceListing | null => {
  ...
  const location = [address.city, address.state ?? address.province].filter(Boolean).join(', ') || String(property.city ?? listing.city ?? 'Available location');
  const details = [listing.bedrooms ? `${listing.bedrooms} bed${listing.bedrooms === 1 ? '' : 's'}` : '', listing.bathrooms ? `${listing.bathrooms} bath${listing.bathrooms === 1 ? '' : 's'}` : '', listing.furnished ? 'Furnished' : 'Monthly lease'].filter(Boolean).join(' · ');
  return { id: String(listing.listingID ?? listing.id ?? `live-${index}`), title, location, price, type: String(listing.listingType?.name ?? listing.listingType?.title ?? listing.type ?? 'Home'), image: listing.imageUrl ?? listing.coverImageUrl ?? listing.listingPhoto?.url ?? listing.listingPhoto?.imageUrl ?? fallbackListings[index % fallbackListings.length].image, details: details || 'Monthly lease', rating: Number(listing.rating ?? 0), reviews: Number(listing.reviewCount ?? listing.reviewsCount ?? 0), availableFrom: String(listing.availableFrom ?? new Date().toISOString().slice(0, 10)), availableTo: listing.availableTo ? String(listing.availableTo) : undefined, tag: listing.status === 'PUBLISHED' ? 'Available now' : undefined };
};
...
export const fallbackListings: MarketplaceListing[] = [
  {
    ...
    details: '1 bed · 1 bath · Furnished', rating: 4.9, reviews: 18, availableFrom: '2026-09-01', tag: 'Move-in ready',
  ...
  },
  {
    ...
    details: '2 beds · 1 bath · Pet friendly', rating: 4.8, reviews: 24, availableFrom: '2026-09-15',
  ...
  },
  {
    ...
    details: '2 beds · 2 baths · Workspace', rating: 5, reviews: 9, availableFrom: '2026-10-01', tag: 'New this week',
  ...
  },
  {
    ...
    details: 'Studio · 1 bath · Doorman', rating: 4.7, reviews: 31, availableFrom: '2026-09-01',
  ...
  },
  {
    ...
    details: '2 beds · 2 baths · Garden', rating: 4.9, reviews: 16, availableFrom: '2026-09-20', tag: 'Popular stay',
  ...
  },
  {
    ...
    details: '2 beds · 1.5 baths · Balcony', rating: 4.8, reviews: 22, availableFrom: '2026-10-01', tag: 'Great location',
  ...
  },
  {
    ...
    details: '1 bed · 1 bath · Workspace', rating: 4.9, reviews: 14, availableFrom: '2026-09-10',
  ...
  },
  {
    ...
    details: '3 beds · 2 baths · Parking', rating: 4.7, reviews: 28, availableFrom: '2026-11-01', tag: 'New this week',
  ...
  },
  {
    ...
    details: 'Studio · 1 bath · Transit nearby', rating: 4.6, reviews: 37, availableFrom: '2026-09-05',
    image: 'https://images.unsplash.com/photo-1505693416388-ac5ce068fe85?auto=format&fit=crop&w=900&q=85',
  },
  {
    id: 'demo-10', title: 'Coastal cottage with room to breathe', location: 'Santa Barbara, California', price: 2760, type: 'Cottage',
    details: '2 beds · 2 baths · Pet friendly', rating: 5, reviews: 11, availableFrom: '2026-10-15', tag: 'Pet friendly',
