...
export const useAccount = () => {
  ...
  const confirmEmail = useCallback(async (request: ConfirmEmailRequest): Promise<ApiResponse> => {
    try {
      ...
      console.log('[useAccount.confirmEmail] start', {
        userId: request.userId || null,
        emailAddress: request.emailAddress || null,
        tokenLength: normalizedToken.length,
      });

      // Support both the current backend contract (userId + token)
      // and the older email + token flow used by the UI.
      if (request.userId && request.userId.trim()) {
        ...
        const primaryUrl = possibleEndpoints[0];
        console.log('[useAccount.confirmEmail] sending userId flow request:', { url: primaryUrl, params });

        const response = await axios.post(primaryUrl, null, { params });
      ...
      console.log('Confirming email for:', emailAddress);
      ...
      const primaryUrl = possibleEndpoints[0];
      console.log('[useAccount.confirmEmail] sending legacy email flow request:', { url: primaryUrl, payload });
