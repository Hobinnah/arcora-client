...
import TenantEmergencyContactForm from '../../forms/TenantEmergencyContactForm.tsx';
import type {TenantEmergencyContact} from '../../types/TenantEmergencyContact';
import { getTenantEmergencyContact } from '../../apis/useTenantEmergencyContact';

export default function TenantEmergencyContactDetails() {
  ...
  const [tenantEmergencyContact, setTenantEmergencyContact] = React.useState<TenantEmergencyContact | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getTenantEmergencyContact(idParam!);
          setTenantEmergencyContact(data);
        } catch (error) {
        ...
        // New record
        setIsEditMode(false);
        setTenantEmergencyContact(null);
        setLoading(false);
      }
    };
    fetchItem();
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit TenantEmergencyContact" : "Create TenantEmergencyContact"}
        trail={[
          { label: 'Tables' },
          { label: 'TenantEmergencyContact List', href: '/tenantemergencycontacts' },
          { label: isEditMode ? 'Edit TenantEmergencyContact' : 'Create TenantEmergencyContact', active: true }
        ]}
        actions={[]}
      />
      ...
      {!loading && (
        <TenantEmergencyContactForm
          ...
          onAlert={handleAlert}
          initialTenantEmergencyContact={tenantEmergencyContact}
