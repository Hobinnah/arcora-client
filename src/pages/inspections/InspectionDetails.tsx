...
import { getInspection } from '../../apis/useInspection';
...
export default function InspectionDetails() {
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getInspection(idParam!);
          setInspection(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit Inspection" : "Create Inspection"}
        trail={[
          { label: 'Tables' },
          { label: 'Inspection List', href: '/inspections' },
          { label: isEditMode ? 'Edit Inspection' : 'Create Inspection', active: true }
        ]}
        actions={[]}
      />
