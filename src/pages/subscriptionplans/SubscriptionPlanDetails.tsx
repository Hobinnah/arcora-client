...
import SubscriptionPlanForm from '../../forms/SubscriptionPlanForm.tsx';
import type {SubscriptionPlan} from '../../types/SubscriptionPlan';
import { getSubscriptionPlan } from '../../apis/useSubscriptionPlan';
...
export default function SubscriptionPlanDetails() {
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getSubscriptionPlan(idParam!);
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit SubscriptionPlan" : "Create SubscriptionPlan"}
        trail={[
          { label: 'Tables' },
          { label: 'SubscriptionPlan List', href: '/subscriptionplans' },
          { label: isEditMode ? 'Edit SubscriptionPlan' : 'Create SubscriptionPlan', active: true }
        ]}
        actions={[]}
      />
