...
import UnitTypeGrid from '../../grids/UnitTypeGrid.tsx';
import BreadCrum  from '../../components/BreadCrum';
import MasterLayout from '../../components/MasterLayout';
import Alert from '../../components/Alert';
import { PlusIcon, DownloadIcon } from '../../components/Icons';
import { deleteUnitType, fetchUnitTypes } from '../../apis/useUnitType';
import { env, type AlertType } from '../../env';
import type { UnitType } from '../../types/UnitType';
import UnitTypeViewModal from './UnitTypeViewModal.tsx';
...
export default function UnitTypeList() {
  ...
  const [selected, setSelected] = React.useState<UnitType | null>(null);

  const handleAdd = () => {
    navigate('/unittypes/new');
  };
  ...
  const handleExportData = async () => {
    ...
    try {
      const exportData = await fetchUnitTypes({ pageSize: 1000, pageNumber: 0, sortBy: 'name', sortDirection: 'desc' });
      const csvContent = convertToCSV(exportData.data);
      downloadCSV(csvContent, `unittypes-export-${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const convertToCSV = (rows: UnitType[]): string => {
  ...
  const handleView = useCallback((row: UnitType) => {
    setSelected(row);
  ...
  const handleEdit = useCallback((row: UnitType) => {
    const id = (row as any)?.unitTypeID;
    if (id === undefined || id === null) return;
    navigate(`/unittypes/${id}`);
  }, [navigate]);

  const handleDelete = useCallback(async (row: UnitType, setItems: React.Dispatch<React.SetStateAction<UnitType[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => {
    const id = (row as any)?.unitTypeID;
    if (id === undefined || id === null) return;
    if (confirm('Are you sure you want to delete this record? This action cannot be undone.')) {
      try {
        await deleteUnitType(id);
        setItems(prev => prev.filter(x => (x as any)?.unitTypeID !== id));
  ...
  const pageContent = React.useMemo(() => [
    <div key="list-content">
      <BreadCrum
        title="UnitType List"
        trail={[{ label: 'Tables' }, { label: 'UnitType List', href: '/unittypes', active: true }]}
        actions={[
          { label: 'Add UnitType', variant: 'soft', onClick: handleAdd, icon: <PlusIcon /> },
