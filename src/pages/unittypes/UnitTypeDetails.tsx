...
import UnitTypeForm from '../../forms/UnitTypeForm.tsx';
import type {UnitType} from '../../types/UnitType';
import { getUnitType } from '../../apis/useUnitType';

export default function UnitTypeDetails() {
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getUnitType(idParam!);
          setUnitType(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit UnitType" : "Create UnitType"}
        trail={[
          { label: 'Tables' },
          { label: 'UnitType List', href: '/unittypes' },
          { label: isEditMode ? 'Edit UnitType' : 'Create UnitType', active: true }
        ]}
        actions={[]}
      />
