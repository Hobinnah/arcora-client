...
import PaymentIntentForm from '../../forms/PaymentIntentForm.tsx';
import type {PaymentIntent} from '../../types/PaymentIntent';
import { getPaymentIntent } from '../../apis/usePaymentIntent';

export default function PaymentIntentDetails() {
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getPaymentIntent(idParam!);
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit PaymentIntent" : "Create PaymentIntent"}
        trail={[
          { label: 'Tables' },
          { label: 'PaymentIntent List', href: '/paymentintents' },
          { label: isEditMode ? 'Edit PaymentIntent' : 'Create PaymentIntent', active: true }
        ]}
        actions={[]}
      />
