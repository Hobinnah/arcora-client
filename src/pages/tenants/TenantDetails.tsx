...
import TenantForm from '../../forms/TenantForm.tsx';
import type {Tenant} from '../../types/Tenant';
import { getTenant } from '../../apis/useTenant';
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getTenant(idParam!);
          setTenant(data);
        } catch (error) {
    ...
    <React.Fragment key="details-content">
      <BreadCrum
        ...
        trail={[
          ...
          { label: isEditMode ? 'Edit Tenant' : 'Create Tenant', active: true }
        ]}
        actions={[]}
      />
