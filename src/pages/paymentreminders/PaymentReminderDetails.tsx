...
import PaymentReminderForm from '../../forms/PaymentReminderForm.tsx';
import type {PaymentReminder} from '../../types/PaymentReminder';
import { getPaymentReminder } from '../../apis/usePaymentReminder';

export default function PaymentReminderDetails() {
  ...
  const [paymentReminder, setPaymentReminder] = React.useState<PaymentReminder | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getPaymentReminder(idParam!);
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit PaymentReminder" : "Create PaymentReminder"}
        trail={[
          { label: 'Tables' },
          { label: 'PaymentReminder List', href: '/paymentreminders' },
          { label: isEditMode ? 'Edit PaymentReminder' : 'Create PaymentReminder', active: true }
        ]}
        actions={[]}
      />
      ...
      {!loading && (
        <PaymentReminderForm
          ...
          onAlert={handleAlert}
          initialPaymentReminder={paymentReminder}
