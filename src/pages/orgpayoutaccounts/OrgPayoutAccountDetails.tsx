...
import OrgPayoutAccountForm from '../../forms/OrgPayoutAccountForm.tsx';
import type {OrgPayoutAccount} from '../../types/OrgPayoutAccount';
import { getOrgPayoutAccount } from '../../apis/useOrgPayoutAccount';

export default function OrgPayoutAccountDetails() {
  ...
  const [orgPayoutAccount, setOrgPayoutAccount] = React.useState<OrgPayoutAccount | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getOrgPayoutAccount(idParam!);
          setOrgPayoutAccount(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit OrgPayoutAccount" : "Create OrgPayoutAccount"}
        trail={[
          { label: 'Tables' },
          { label: 'OrgPayoutAccount List', href: '/orgpayoutaccounts' },
          { label: isEditMode ? 'Edit OrgPayoutAccount' : 'Create OrgPayoutAccount', active: true }
        ]}
        actions={[]}
      />
      ...
      {/* Form */}
      {!loading && (
        <OrgPayoutAccountForm
          key={isEditMode ? `edit-${idParam}` : 'create-new'}
          onAlert={handleAlert}
          initialOrgPayoutAccount={orgPayoutAccount}
