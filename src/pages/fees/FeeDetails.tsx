...
import FeeForm from '../../forms/FeeForm.tsx';
import type {Fee} from '../../types/Fee';
import { getFee } from '../../apis/useFee';
...
export default function FeeDetails() {
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getFee(idParam!);
          setFee(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit Fee" : "Create Fee"}
        trail={[
          { label: 'Tables' },
          { label: 'Fee List', href: '/fees' },
          { label: isEditMode ? 'Edit Fee' : 'Create Fee', active: true }
        ]}
        actions={[]}
      />
