...
import ReservationHoldForm from '../../forms/ReservationHoldForm.tsx';
import type {ReservationHold} from '../../types/ReservationHold';
import { getReservationHold } from '../../apis/useReservationHold';

export default function ReservationHoldDetails() {
  ...
  const [reservationHold, setReservationHold] = React.useState<ReservationHold | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getReservationHold(idParam!);
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit ReservationHold" : "Create ReservationHold"}
        trail={[
          { label: 'Tables' },
          { label: 'ReservationHold List', href: '/reservationholds' },
          { label: isEditMode ? 'Edit ReservationHold' : 'Create ReservationHold', active: true }
        ]}
        actions={[]}
      />
      ...
      {!loading && (
        <ReservationHoldForm
          ...
          onAlert={handleAlert}
          initialReservationHold={reservationHold}
