...
import InvoiceDetailForm from '../../forms/InvoiceDetailForm.tsx';
import type {InvoiceDetail} from '../../types/InvoiceDetail';
import { getInvoiceDetail } from '../../apis/useInvoiceDetail';

export default function InvoiceDetailDetails() {
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getInvoiceDetail(idParam!);
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit InvoiceDetail" : "Create InvoiceDetail"}
        trail={[
          { label: 'Tables' },
          { label: 'InvoiceDetail List', href: '/invoicedetails' },
          { label: isEditMode ? 'Edit InvoiceDetail' : 'Create InvoiceDetail', active: true }
        ]}
        actions={[]}
      />
      ...
      {!loading && (
        <InvoiceDetailForm
          ...
          onAlert={handleAlert}
          initialInvoiceDetail={invoiceDetail}
