...
import LeaseRecurringChargesForm from '../../forms/LeaseRecurringChargesForm.tsx';
import type {LeaseRecurringCharges} from '../../types/LeaseRecurringCharges';
import { getLeaseRecurringChargesById } from '../../apis/useLeaseRecurringCharges';

export default function LeaseRecurringChargesDetails() {
  ...
  const [leaseRecurringCharges, setLeaseRecurringCharges] = React.useState<LeaseRecurringCharges | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getLeaseRecurringChargesById(idParam!);
          setLeaseRecurringCharges(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit LeaseRecurringCharges" : "Create LeaseRecurringCharges"}
        trail={[
          { label: 'Tables' },
          { label: 'LeaseRecurringCharges List', href: '/leaserecurringcharges' },
          { label: isEditMode ? 'Edit LeaseRecurringCharges' : 'Create LeaseRecurringCharges', active: true }
        ]}
        actions={[]}
      />
      ...
      {/* Form */}
      {!loading && (
        <LeaseRecurringChargesForm
          key={isEditMode ? `edit-${idParam}` : 'create-new'}
          onAlert={handleAlert}
          initialLeaseRecurringCharges={leaseRecurringCharges}
