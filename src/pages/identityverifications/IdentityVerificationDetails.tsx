...
import IdentityVerificationForm from '../../forms/IdentityVerificationForm.tsx';
import type {IdentityVerification} from '../../types/IdentityVerification';
import { getIdentityVerification } from '../../apis/useIdentityVerification';
...
export default function IdentityVerificationDetails() {
  ...
  const [identityVerification, setIdentityVerification] = React.useState<IdentityVerification | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getIdentityVerification(idParam!);
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit IdentityVerification" : "Create IdentityVerification"}
        trail={[
          { label: 'Tables' },
          { label: 'IdentityVerification List', href: '/identityverifications' },
          { label: isEditMode ? 'Edit IdentityVerification' : 'Create IdentityVerification', active: true }
        ]}
        actions={[]}
      />
      ...
      {!loading && (
        <IdentityVerificationForm
          ...
          onAlert={handleAlert}
          initialIdentityVerification={identityVerification}
