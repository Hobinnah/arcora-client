...
import TenantGuarantorForm from '../../forms/TenantGuarantorForm.tsx';
import type {TenantGuarantor} from '../../types/TenantGuarantor';
import { getTenantGuarantor } from '../../apis/useTenantGuarantor';

export default function TenantGuarantorDetails() {
  ...
  const [tenantGuarantor, setTenantGuarantor] = React.useState<TenantGuarantor | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getTenantGuarantor(idParam!);
          setTenantGuarantor(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit TenantGuarantor" : "Create TenantGuarantor"}
        trail={[
          { label: 'Tables' },
          { label: 'TenantGuarantor List', href: '/tenantguarantors' },
          { label: isEditMode ? 'Edit TenantGuarantor' : 'Create TenantGuarantor', active: true }
        ]}
        actions={[]}
      />
      ...
      {!loading && (
        <TenantGuarantorForm
          ...
          onAlert={handleAlert}
          initialTenantGuarantor={tenantGuarantor}
