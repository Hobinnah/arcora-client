...
import InvoiceMasterForm from '../../forms/InvoiceMasterForm.tsx';
import type {InvoiceMaster} from '../../types/InvoiceMaster';
import { getInvoiceMaster } from '../../apis/useInvoiceMaster';

export default function InvoiceMasterDetails() {
  ...
  const [invoiceMaster, setInvoiceMaster] = React.useState<InvoiceMaster | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getInvoiceMaster(idParam!);
          setInvoiceMaster(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit InvoiceMaster" : "Create InvoiceMaster"}
        trail={[
          { label: 'Tables' },
          { label: 'InvoiceMaster List', href: '/invoicemasters' },
          { label: isEditMode ? 'Edit InvoiceMaster' : 'Create InvoiceMaster', active: true }
        ]}
        actions={[]}
      />
      ...
      {!loading && (
        <InvoiceMasterForm
          ...
          onAlert={handleAlert}
          initialInvoiceMaster={invoiceMaster}
