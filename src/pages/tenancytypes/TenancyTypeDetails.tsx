...
import TenancyTypeForm from '../../forms/TenancyTypeForm.tsx';
import type {TenancyType} from '../../types/TenancyType';
import { getTenancyType } from '../../apis/useTenancyType';

export default function TenancyTypeDetails() {
  ...
  const [tenancyType, setTenancyType] = React.useState<TenancyType | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getTenancyType(idParam!);
          setTenancyType(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit TenancyType" : "Create TenancyType"}
        trail={[
          { label: 'Tables' },
          { label: 'TenancyType List', href: '/tenancytypes' },
          { label: isEditMode ? 'Edit TenancyType' : 'Create TenancyType', active: true }
        ]}
        actions={[]}
      />
      ...
      {!loading && (
        <TenancyTypeForm
          ...
          onAlert={handleAlert}
          initialTenancyType={tenancyType}
