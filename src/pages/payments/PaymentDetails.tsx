...
import { getPayment } from '../../apis/usePayment';
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getPayment(idParam!);
          setPayment(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        ...
        trail={[
          { label: 'Tables' },
          { label: 'Payment List', href: '/payments' },
          { label: isEditMode ? 'Edit Payment' : 'Create Payment', active: true }
        ]}
        actions={[]}
      />
