...
import AttachmentForm from '../../forms/AttachmentForm.tsx';
import type {Attachment} from '../../types/Attachment';
import { getAttachment } from '../../apis/useAttachment';
...
export default function AttachmentDetails() {
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getAttachment(idParam!);
          setAttachment(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit Attachment" : "Create Attachment"}
        trail={[
          { label: 'Tables' },
          { label: 'Attachment List', href: '/attachments' },
          { label: isEditMode ? 'Edit Attachment' : 'Create Attachment', active: true }
        ]}
        actions={[]}
      />
