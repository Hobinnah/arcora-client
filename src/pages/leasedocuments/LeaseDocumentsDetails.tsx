...
import LeaseDocumentsForm from '../../forms/LeaseDocumentsForm.tsx';
import type {LeaseDocuments} from '../../types/LeaseDocuments';
import { getLeaseDocumentsById } from '../../apis/useLeaseDocuments';

export default function LeaseDocumentsDetails() {
  ...
  const [leaseDocuments, setLeaseDocuments] = React.useState<LeaseDocuments | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getLeaseDocumentsById(idParam!);
          setLeaseDocuments(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit LeaseDocuments" : "Create LeaseDocuments"}
        trail={[
          { label: 'Tables' },
          { label: 'LeaseDocuments List', href: '/leasedocuments' },
          { label: isEditMode ? 'Edit LeaseDocuments' : 'Create LeaseDocuments', active: true }
        ]}
        actions={[]}
      />
      ...
      {!loading && (
        <LeaseDocumentsForm
          ...
          onAlert={handleAlert}
          initialLeaseDocuments={leaseDocuments}
