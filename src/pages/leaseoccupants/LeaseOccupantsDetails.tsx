...
export default function LeaseOccupantsDetails() {
  ...
  const [leaseOccupants, setLeaseOccupants] = React.useState<LeaseOccupants | null>(null);
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit LeaseOccupants" : "Create LeaseOccupants"}
        trail={[
          { label: 'Tables' },
          { label: 'LeaseOccupants List', href: '/leaseoccupants' },
          { label: isEditMode ? 'Edit LeaseOccupants' : 'Create LeaseOccupants', active: true }
        ]}
        actions={[]}
      />
      ...
      {!loading && (
        <LeaseOccupantsForm
          ...
          onAlert={handleAlert}
          initialLeaseOccupants={leaseOccupants}
