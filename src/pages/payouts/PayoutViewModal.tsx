...
interface PayoutViewModalProps {
  payout: Payout | null;
  isOpen: boolean;
  onClose: () => void;
}

export default function PayoutViewModal({ payout, isOpen, onClose }: PayoutViewModalProps) {

  const [organizationIDMap, setOrganizationIDMap] = React.useState<Record<string, string>>({ });
  const [orgPayoutAccountIDMap, setOrgPayoutAccountIDMap] = React.useState<Record<string, string>>({ });
  ...
  React.useEffect(() => {
    const loadLookups = async () => {
      ...
      try {
        const res = await fetchData('api/orgpayoutaccount/get?pageSize=1000&pageNumber=1');
        const items = res?.data ?? res ?? [];
        const map: Record<string, string> = {};
        items.forEach((it: any) => { map[String(it.orgPayoutAccountID ?? it.id)] = it.orgPayoutAccountID ?? it.name ?? String(it.orgPayoutAccountID ?? ''); });
        setOrgPayoutAccountIDMap(map);
      } catch { /* non-critical */ }
    };
  ...
  const getStatusStyle = (status: string) => {
    switch (status) {
      ...
      case 'PAID': return { backgroundColor: '#e0ecff', color: '#1e3a8a' };
    ...
    >
      ...
      >
        <div className="card-header">
          <h3 className="card-title" style={{ fontSize: '1.25rem', textTransform: 'none', color: 'var(--fg)' }}>Payout Details</h3>
        ...
        <div className="form">
          <div style={{ display: 'grid', gap: '16px' }}>
            ...
            <div className="grid2">
              <div className="field">
                ...
                <div className="input" style={{ width: '100%', maxWidth: '100%', boxSizing: 'border-box', backgroundColor: 'var(--surface-2)', cursor: 'default' }}>
                  <span style={{ display: 'inline-block', padding: '2px 8px', borderRadius: '12px', fontSize: '0.75rem', fontWeight: 500, ...getStatusStyle(payout.status || '') }}>
                    {payout.status || 'N/A'}
                  </span>
                </div>
              </div>
              ...
              <div className="field">
                ...
                <div className="input" style={{ width: '100%', maxWidth: '100%', boxSizing: 'border-box', backgroundColor: 'var(--surface-2)', cursor: 'default' }}>
                  {organizationIDMap[String(payout.organizationID)] ?? payout.organizationID ?? 'N/A'}
                </div>
              </div>

              <div className="field">
                <label className="field-label">Org Payout Account</label>
                <div className="input" style={{ width: '100%', maxWidth: '100%', boxSizing: 'border-box', backgroundColor: 'var(--surface-2)', cursor: 'default' }}>
                  {orgPayoutAccountIDMap[String(payout.orgPayoutAccountID)] ?? payout.orgPayoutAccountID ?? 'N/A'}
                </div>
              </div>
              ...
              <div className="field">
                ...
                <div className="input" style={{ width: '100%', maxWidth: '100%', boxSizing: 'border-box', backgroundColor: 'var(--surface-2)', cursor: 'default' }}>
                  {payout.scheduledAt ? new Date(payout.scheduledAt as any).toLocaleDateString() : 'N/A'}
                </div>
              </div>
              ...
              <div className="field">
                ...
                <div className="input" style={{ width: '100%', maxWidth: '100%', boxSizing: 'border-box', backgroundColor: 'var(--surface-2)', cursor: 'default' }}>
                  {payout.requestedAt ? new Date(payout.requestedAt as any).toLocaleDateString() : 'N/A'}
                </div>
              </div>
              ...
              <div className="field">
                ...
                <div className="input" style={{ width: '100%', maxWidth: '100%', boxSizing: 'border-box', backgroundColor: 'var(--surface-2)', cursor: 'default' }}>
                  {payout.processedAt ? new Date(payout.processedAt as any).toLocaleDateString() : 'N/A'}
                </div>
              </div>
              ...
              <div className="field">
                ...
                <div className="input" style={{ width: '100%', maxWidth: '100%', boxSizing: 'border-box', backgroundColor: 'var(--surface-2)', cursor: 'default' }}>
                  {payout.paidAt ? new Date(payout.paidAt as any).toLocaleDateString() : 'N/A'}
                </div>
              </div>
              ...
              <div className="field">
                ...
                <div className="input" style={{ width: '100%', maxWidth: '100%', boxSizing: 'border-box', backgroundColor: 'var(--surface-2)', cursor: 'default' }}>
                  {payout.providerName ?? 'N/A'}
                </div>
              </div>

              <div className="field">
                <label className="field-label">Provider Payout ID</label>
                <div className="input" style={{ width: '100%', maxWidth: '100%', boxSizing: 'border-box', backgroundColor: 'var(--surface-2)', cursor: 'default' }}>
            ...
            <div className="field">
              <label className="field-label">Payout ID</label>
