...
import PayoutForm from '../../forms/PayoutForm.tsx';
import type {Payout} from '../../types/Payout';
import { getPayout } from '../../apis/usePayout';
...
export default function PayoutDetails() {
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getPayout(idParam!);
          setPayout(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit Payout" : "Create Payout"}
        trail={[
          { label: 'Tables' },
          { label: 'Payout List', href: '/payouts' },
          { label: isEditMode ? 'Edit Payout' : 'Create Payout', active: true }
        ]}
        actions={[]}
      />
