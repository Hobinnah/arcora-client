...
import { getAddress } from '../../apis/useAddress';
...
export default function AddressDetails() {
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getAddress(idParam!);
          setAddress(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit Address" : "Create Address"}
        trail={[
          { label: 'Tables' },
          { label: 'Address List', href: '/addresses' },
          { label: isEditMode ? 'Edit Address' : 'Create Address', active: true }
        ]}
        actions={[]}
      />
