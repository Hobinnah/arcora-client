...
import LeaseDocExtractedTermForm from '../../forms/LeaseDocExtractedTermForm.tsx';
import type {LeaseDocExtractedTerm} from '../../types/LeaseDocExtractedTerm';
import { getLeaseDocExtractedTerm } from '../../apis/useLeaseDocExtractedTerm';

export default function LeaseDocExtractedTermDetails() {
  ...
  const [leaseDocExtractedTerm, setLeaseDocExtractedTerm] = React.useState<LeaseDocExtractedTerm | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getLeaseDocExtractedTerm(idParam!);
          setLeaseDocExtractedTerm(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit LeaseDocExtractedTerm" : "Create LeaseDocExtractedTerm"}
        trail={[
          { label: 'Tables' },
          { label: 'LeaseDocExtractedTerm List', href: '/leasedocextractedterms' },
          { label: isEditMode ? 'Edit LeaseDocExtractedTerm' : 'Create LeaseDocExtractedTerm', active: true }
        ]}
        actions={[]}
      />
      ...
      {/* Form */}
      {!loading && (
        <LeaseDocExtractedTermForm
          key={isEditMode ? `edit-${idParam}` : 'create-new'}
          onAlert={handleAlert}
          initialLeaseDocExtractedTerm={leaseDocExtractedTerm}
