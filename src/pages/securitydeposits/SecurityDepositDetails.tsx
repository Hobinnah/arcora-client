...
import SecurityDepositForm from '../../forms/SecurityDepositForm.tsx';
import type {SecurityDeposit} from '../../types/SecurityDeposit';
import { getSecurityDeposit } from '../../apis/useSecurityDeposit';

export default function SecurityDepositDetails() {
  ...
  const [securityDeposit, setSecurityDeposit] = React.useState<SecurityDeposit | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getSecurityDeposit(idParam!);
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit SecurityDeposit" : "Create SecurityDeposit"}
        trail={[
          { label: 'Tables' },
          { label: 'SecurityDeposit List', href: '/securitydeposits' },
          { label: isEditMode ? 'Edit SecurityDeposit' : 'Create SecurityDeposit', active: true }
        ]}
        actions={[]}
      />
