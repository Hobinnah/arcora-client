...
import RentalApplicationForm from '../../forms/RentalApplicationForm.tsx';
import type {RentalApplication} from '../../types/RentalApplication';
import { getRentalApplication } from '../../apis/useRentalApplication';

export default function RentalApplicationDetails() {
  ...
  const [rentalApplication, setRentalApplication] = React.useState<RentalApplication | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getRentalApplication(idParam!);
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit RentalApplication" : "Create RentalApplication"}
        trail={[
          { label: 'Tables' },
          { label: 'RentalApplication List', href: '/rentalapplications' },
          { label: isEditMode ? 'Edit RentalApplication' : 'Create RentalApplication', active: true }
        ]}
        actions={[]}
      />
      ...
      {!loading && (
        <RentalApplicationForm
          ...
          onAlert={handleAlert}
          initialRentalApplication={rentalApplication}
