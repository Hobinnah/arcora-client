...
import OrganizationStatementForm from '../../forms/OrganizationStatementForm.tsx';
import type {OrganizationStatement} from '../../types/OrganizationStatement';
import { getOrganizationStatement } from '../../apis/useOrganizationStatement';

export default function OrganizationStatementDetails() {
  ...
  const [organizationStatement, setOrganizationStatement] = React.useState<OrganizationStatement | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getOrganizationStatement(idParam!);
          setOrganizationStatement(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit OrganizationStatement" : "Create OrganizationStatement"}
        trail={[
          { label: 'Tables' },
          { label: 'OrganizationStatement List', href: '/organizationstatements' },
          { label: isEditMode ? 'Edit OrganizationStatement' : 'Create OrganizationStatement', active: true }
        ]}
        actions={[]}
      />
      ...
      {/* Form */}
      {!loading && (
        <OrganizationStatementForm
          key={isEditMode ? `edit-${idParam}` : 'create-new'}
          onAlert={handleAlert}
          initialOrganizationStatement={organizationStatement}
