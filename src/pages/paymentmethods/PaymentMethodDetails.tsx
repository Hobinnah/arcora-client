...
import PaymentMethodForm from '../../forms/PaymentMethodForm.tsx';
import type {PaymentMethod} from '../../types/PaymentMethod';
import { getPaymentMethod } from '../../apis/usePaymentMethod';
...
export default function PaymentMethodDetails() {
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getPaymentMethod(idParam!);
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit PaymentMethod" : "Create PaymentMethod"}
        trail={[
          { label: 'Tables' },
          { label: 'PaymentMethod List', href: '/paymentmethods' },
          { label: isEditMode ? 'Edit PaymentMethod' : 'Create PaymentMethod', active: true }
        ]}
        actions={[]}
      />
