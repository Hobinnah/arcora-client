...
import AutopayMandateForm from '../../forms/AutopayMandateForm.tsx';
import type {AutopayMandate} from '../../types/AutopayMandate';
import { getAutopayMandate } from '../../apis/useAutopayMandate';

export default function AutopayMandateDetails() {
  ...
  const [autopayMandate, setAutopayMandate] = React.useState<AutopayMandate | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getAutopayMandate(idParam!);
          setAutopayMandate(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit AutopayMandate" : "Create AutopayMandate"}
        trail={[
          { label: 'Tables' },
          { label: 'AutopayMandate List', href: '/autopaymandates' },
          { label: isEditMode ? 'Edit AutopayMandate' : 'Create AutopayMandate', active: true }
        ]}
        actions={[]}
      />
      ...
      {!loading && (
        <AutopayMandateForm
          ...
          onAlert={handleAlert}
          initialAutopayMandate={autopayMandate}
