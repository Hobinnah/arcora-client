...
import CreditReportingEnrollmentForm from '../../forms/CreditReportingEnrollmentForm.tsx';
import type {CreditReportingEnrollment} from '../../types/CreditReportingEnrollment';
import { getCreditReportingEnrollment } from '../../apis/useCreditReportingEnrollment';

export default function CreditReportingEnrollmentDetails() {
  ...
  const [creditReportingEnrollment, setCreditReportingEnrollment] = React.useState<CreditReportingEnrollment | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getCreditReportingEnrollment(idParam!);
          setCreditReportingEnrollment(data);
        } catch (error) {
        ...
        // New record
        setIsEditMode(false);
        setCreditReportingEnrollment(null);
        setLoading(false);
      }
    };
    fetchItem();
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit CreditReportingEnrollment" : "Create CreditReportingEnrollment"}
        trail={[
          { label: 'Tables' },
          { label: 'CreditReportingEnrollment List', href: '/creditreportingenrollments' },
          { label: isEditMode ? 'Edit CreditReportingEnrollment' : 'Create CreditReportingEnrollment', active: true }
        ]}
        actions={[]}
      />
      ...
      {/* Form */}
      {!loading && (
        <CreditReportingEnrollmentForm
          key={isEditMode ? `edit-${idParam}` : 'create-new'}
          onAlert={handleAlert}
          initialCreditReportingEnrollment={creditReportingEnrollment}
