...
import ConversationMessageForm from '../../forms/ConversationMessageForm.tsx';
import type {ConversationMessage} from '../../types/ConversationMessage';
import { getConversationMessage } from '../../apis/useConversationMessage';

export default function ConversationMessageDetails() {
  ...
  const [conversationMessage, setConversationMessage] = React.useState<ConversationMessage | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getConversationMessage(idParam!);
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit ConversationMessage" : "Create ConversationMessage"}
        trail={[
          { label: 'Tables' },
          { label: 'ConversationMessage List', href: '/conversationmessages' },
          { label: isEditMode ? 'Edit ConversationMessage' : 'Create ConversationMessage', active: true }
        ]}
        actions={[]}
      />
      ...
      {!loading && (
        <ConversationMessageForm
          ...
          onAlert={handleAlert}
          initialConversationMessage={conversationMessage}
