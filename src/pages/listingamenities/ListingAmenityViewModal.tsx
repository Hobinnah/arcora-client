...
interface ListingAmenityViewModalProps {
  listingAmenity: ListingAmenity | null;
  isOpen: boolean;
  onClose: () => void;
}

export default function ListingAmenityViewModal({ listingAmenity, isOpen, onClose }: ListingAmenityViewModalProps) {

  const [listingIDMap, setListingIDMap] = React.useState<Record<string, string>>({ });
  const [amenityIDMap, setAmenityIDMap] = React.useState<Record<string, string>>({ });
  ...
  React.useEffect(() => {
    const loadLookups = async () => {
      ...
      try {
        const res = await fetchData('api/amenitycatalog/get?pageSize=1000&pageNumber=1');
        const items = res?.data ?? res ?? [];
        const map: Record<string, string> = {};
        items.forEach((it: any) => { map[String(it.amenityID ?? it.id)] = it.name ?? it.name ?? String(it.amenityID ?? ''); });
        setAmenityIDMap(map);
      } catch { /* non-critical */ }
    };
  ...
  if (!isOpen || !listingAmenity) return null;
    ...
    >
      ...
      >
        <div className="card-header">
          <h3 className="card-title" style={{ fontSize: '1.25rem', textTransform: 'none', color: 'var(--fg)' }}>ListingAmenity Details</h3>
        ...
        <div className="form">
          <div style={{ display: 'grid', gap: '16px' }}>
            ...
            <div className="grid2">
              <div className="field">
                ...
                <div className="input" style={{ width: '100%', maxWidth: '100%', boxSizing: 'border-box', backgroundColor: 'var(--surface-2)', cursor: 'default' }}>
                  {listingIDMap[String(listingAmenity.listingID)] ?? listingAmenity.listingID ?? 'N/A'}
                </div>
              </div>
              ...
              <div className="field">
                ...
                <div className="input" style={{ width: '100%', maxWidth: '100%', boxSizing: 'border-box', backgroundColor: 'var(--surface-2)', cursor: 'default' }}>
                  {amenityIDMap[String(listingAmenity.amenityID)] ?? listingAmenity.amenityID ?? 'N/A'}
                </div>
              </div>
            ...
            <div className="field">
              <label className="field-label">Listing Amenity ID</label>
              <div className="input" style={{ width: '100%', maxWidth: '100%', boxSizing: 'border-box', backgroundColor: 'var(--surface-2)', cursor: 'default', fontFamily: 'monospace', color: 'var(--muted)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {listingAmenity.listingAmenityID ?? 'N/A'}
              </div>
            </div>
