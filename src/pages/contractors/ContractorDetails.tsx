...
import ContractorForm from '../../forms/ContractorForm.tsx';
import type {Contractor} from '../../types/Contractor';
import { getContractor } from '../../apis/useContractor';
...
export default function ContractorDetails() {
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getContractor(idParam!);
          setContractor(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit Contractor" : "Create Contractor"}
        trail={[
          { label: 'Tables' },
          { label: 'Contractor List', href: '/contractors' },
          { label: isEditMode ? 'Edit Contractor' : 'Create Contractor', active: true }
        ]}
        actions={[]}
      />
