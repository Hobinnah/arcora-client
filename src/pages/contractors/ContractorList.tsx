...
import ContractorGrid from '../../grids/ContractorGrid.tsx';
import BreadCrum  from '../../components/BreadCrum';
import MasterLayout from '../../components/MasterLayout';
import Alert from '../../components/Alert';
import { PlusIcon, DownloadIcon } from '../../components/Icons';
import { deleteContractor, fetchContractors, fireContractorAction } from '../../apis/useContractor';
import { env, type AlertType } from '../../env';
import type { Contractor } from '../../types/Contractor';
import ContractorViewModal from './ContractorViewModal.tsx';
...
export default function ContractorList() {
  ...
  const handleExportData = async () => {
    ...
    try {
      const exportData = await fetchContractors({ pageSize: 1000, pageNumber: 0, sortBy: 'companyName', sortDirection: 'desc' });
      const csvContent = convertToCSV(exportData.data);
      downloadCSV(csvContent, `contractors-export-${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const handleView = useCallback((row: Contractor) => {
    setSelected(row);
  ...
  const handleEdit = useCallback((row: Contractor) => {
    const id = (row as any)?.contractorID;
    if (id === undefined || id === null) return;
    navigate(`/contractors/${id}`);
  }, [navigate]);

  const handleDelete = useCallback(async (row: Contractor, setItems: React.Dispatch<React.SetStateAction<Contractor[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => {
    const id = (row as any)?.contractorID;
    if (id === undefined || id === null) return;
    if (confirm('Are you sure you want to delete this record? This action cannot be undone.')) {
      try {
        await deleteContractor(id);
        setItems(prev => prev.filter(x => (x as any)?.contractorID !== id));
  ...
  const actionHandlers = React.useMemo(() => ({
    "set-inactive": async (row: any) => {
      const id = row?.contractorID;
      if (id === undefined || id === null) return;
      await fireContractorAction(id, "Set INACTIVE");
  ...
  const pageContent = React.useMemo(() => [
    <div key="list-content">
      <BreadCrum
        ...
        trail={[{ label: 'Tables' }, { label: 'Contractor List', href: '/contractors', active: true }]}
        actions={[
          { label: 'Add Contractor', variant: 'soft', onClick: handleAdd, icon: <PlusIcon /> },
