...
import RentalUnitForm from '../../forms/RentalUnitForm.tsx';
import type {RentalUnit} from '../../types/RentalUnit';
import { getRentalUnit } from '../../apis/useRentalUnit';
...
export default function RentalUnitDetails() {
  ...
  const [rentalUnit, setRentalUnit] = React.useState<RentalUnit | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getRentalUnit(idParam!);
          setRentalUnit(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit RentalUnit" : "Create RentalUnit"}
        trail={[
          { label: 'Tables' },
          { label: 'RentalUnit List', href: '/rentalunits' },
          { label: isEditMode ? 'Edit RentalUnit' : 'Create RentalUnit', active: true }
        ]}
        actions={[]}
      />
