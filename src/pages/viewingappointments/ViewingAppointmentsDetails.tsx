...
export default function ViewingAppointmentsDetails() {
  ...
  const [viewingAppointments, setViewingAppointments] = React.useState<ViewingAppointments | null>(null);
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit ViewingAppointments" : "Create ViewingAppointments"}
        trail={[
          { label: 'Tables' },
          { label: 'ViewingAppointments List', href: '/viewingappointments' },
          { label: isEditMode ? 'Edit ViewingAppointments' : 'Create ViewingAppointments', active: true }
        ]}
        actions={[]}
      />
      ...
      {/* Form */}
      {!loading && (
        <ViewingAppointmentsForm
          key={isEditMode ? `edit-${idParam}` : 'create-new'}
          onAlert={handleAlert}
          initialViewingAppointments={viewingAppointments}
