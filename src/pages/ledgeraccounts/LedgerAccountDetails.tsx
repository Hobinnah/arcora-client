...
import LedgerAccountForm from '../../forms/LedgerAccountForm.tsx';
import type {LedgerAccount} from '../../types/LedgerAccount';
import { getLedgerAccount } from '../../apis/useLedgerAccount';

export default function LedgerAccountDetails() {
  ...
  const [ledgerAccount, setLedgerAccount] = React.useState<LedgerAccount | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getLedgerAccount(idParam!);
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit LedgerAccount" : "Create LedgerAccount"}
        trail={[
          { label: 'Tables' },
          { label: 'LedgerAccount List', href: '/ledgeraccounts' },
          { label: isEditMode ? 'Edit LedgerAccount' : 'Create LedgerAccount', active: true }
        ]}
        actions={[]}
      />
      ...
      {!loading && (
        <LedgerAccountForm
          ...
          onAlert={handleAlert}
          initialLedgerAccount={ledgerAccount}
