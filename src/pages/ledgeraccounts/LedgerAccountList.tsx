...
import LedgerAccountGrid from '../../grids/LedgerAccountGrid.tsx';
import BreadCrum  from '../../components/BreadCrum';
import MasterLayout from '../../components/MasterLayout';
import Alert from '../../components/Alert';
import { PlusIcon, DownloadIcon } from '../../components/Icons';
import { deleteLedgerAccount, fetchLedgerAccounts } from '../../apis/useLedgerAccount';
import { env, type AlertType } from '../../env';
import type { LedgerAccount } from '../../types/LedgerAccount';
import LedgerAccountViewModal from './LedgerAccountViewModal.tsx';
...
export default function LedgerAccountList() {
  ...
  const [selected, setSelected] = React.useState<LedgerAccount | null>(null);

  const handleAdd = () => {
    navigate('/ledgeraccounts/new');
  };
  ...
  const handleExportData = async () => {
    ...
    try {
      const exportData = await fetchLedgerAccounts({ pageSize: 1000, pageNumber: 0, sortBy: 'accountCode', sortDirection: 'desc' });
      const csvContent = convertToCSV(exportData.data);
      downloadCSV(csvContent, `ledgeraccounts-export-${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const handleView = useCallback((row: LedgerAccount) => {
    setSelected(row);
  ...
  const handleEdit = useCallback((row: LedgerAccount) => {
    const id = (row as any)?.ledgerAccountID;
    if (id === undefined || id === null) return;
    navigate(`/ledgeraccounts/${id}`);
  }, [navigate]);

  const handleDelete = useCallback(async (row: LedgerAccount, setItems: React.Dispatch<React.SetStateAction<LedgerAccount[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => {
    const id = (row as any)?.ledgerAccountID;
    if (id === undefined || id === null) return;
    if (confirm('Are you sure you want to delete this record? This action cannot be undone.')) {
      try {
        await deleteLedgerAccount(id);
        setItems(prev => prev.filter(x => (x as any)?.ledgerAccountID !== id));
  ...
  const pageContent = React.useMemo(() => [
    <div key="list-content">
      <BreadCrum
        title="LedgerAccount List"
        trail={[{ label: 'Tables' }, { label: 'LedgerAccount List', href: '/ledgeraccounts', active: true }]}
        actions={[
          { label: 'Add LedgerAccount', variant: 'soft', onClick: handleAdd, icon: <PlusIcon /> },
