...
import AuditLogForm from '../../forms/AuditLogForm.tsx';
import type {AuditLog} from '../../types/AuditLog';
import { getAuditLog } from '../../apis/useAuditLog';
...
export default function AuditLogDetails() {
  ...
  const [auditLog, setAuditLog] = React.useState<AuditLog | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getAuditLog(idParam!);
          setAuditLog(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit AuditLog" : "Create AuditLog"}
        trail={[
          { label: 'Tables' },
          { label: 'AuditLog List', href: '/auditlogs' },
          { label: isEditMode ? 'Edit AuditLog' : 'Create AuditLog', active: true }
        ]}
        actions={[]}
      />
