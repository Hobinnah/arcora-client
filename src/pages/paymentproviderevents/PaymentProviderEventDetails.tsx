...
import PaymentProviderEventForm from '../../forms/PaymentProviderEventForm.tsx';
import type {PaymentProviderEvent} from '../../types/PaymentProviderEvent';
import { getPaymentProviderEvent } from '../../apis/usePaymentProviderEvent';

export default function PaymentProviderEventDetails() {
  ...
  const [paymentProviderEvent, setPaymentProviderEvent] = React.useState<PaymentProviderEvent | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getPaymentProviderEvent(idParam!);
          setPaymentProviderEvent(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit PaymentProviderEvent" : "Create PaymentProviderEvent"}
        trail={[
          { label: 'Tables' },
          { label: 'PaymentProviderEvent List', href: '/paymentproviderevents' },
          { label: isEditMode ? 'Edit PaymentProviderEvent' : 'Create PaymentProviderEvent', active: true }
        ]}
        actions={[]}
      />
      ...
      {/* Form */}
      {!loading && (
        <PaymentProviderEventForm
          key={isEditMode ? `edit-${idParam}` : 'create-new'}
          onAlert={handleAlert}
          initialPaymentProviderEvent={paymentProviderEvent}
