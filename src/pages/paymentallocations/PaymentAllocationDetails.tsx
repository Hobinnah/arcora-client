...
import PaymentAllocationForm from '../../forms/PaymentAllocationForm.tsx';
import type {PaymentAllocation} from '../../types/PaymentAllocation';
import { getPaymentAllocation } from '../../apis/usePaymentAllocation';

export default function PaymentAllocationDetails() {
  ...
  const [paymentAllocation, setPaymentAllocation] = React.useState<PaymentAllocation | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getPaymentAllocation(idParam!);
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit PaymentAllocation" : "Create PaymentAllocation"}
        trail={[
          { label: 'Tables' },
          { label: 'PaymentAllocation List', href: '/paymentallocations' },
          { label: isEditMode ? 'Edit PaymentAllocation' : 'Create PaymentAllocation', active: true }
        ]}
        actions={[]}
      />
      ...
      {!loading && (
        <PaymentAllocationForm
          ...
          onAlert={handleAlert}
          initialPaymentAllocation={paymentAllocation}
