...
import CreditReportingConsentAuditForm from '../../forms/CreditReportingConsentAuditForm.tsx';
import type {CreditReportingConsentAudit} from '../../types/CreditReportingConsentAudit';
import { getCreditReportingConsentAudit } from '../../apis/useCreditReportingConsentAudit';

export default function CreditReportingConsentAuditDetails() {
  ...
  const [creditReportingConsentAudit, setCreditReportingConsentAudit] = React.useState<CreditReportingConsentAudit | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getCreditReportingConsentAudit(idParam!);
          setCreditReportingConsentAudit(data);
        } catch (error) {
        ...
        // New record
        setIsEditMode(false);
        setCreditReportingConsentAudit(null);
        setLoading(false);
      }
    };
    fetchItem();
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit CreditReportingConsentAudit" : "Create CreditReportingConsentAudit"}
        trail={[
          { label: 'Tables' },
          { label: 'CreditReportingConsentAudit List', href: '/creditreportingconsentaudits' },
          { label: isEditMode ? 'Edit CreditReportingConsentAudit' : 'Create CreditReportingConsentAudit', active: true }
        ]}
        actions={[]}
      />
      ...
      {/* Form */}
      {!loading && (
        <CreditReportingConsentAuditForm
          key={isEditMode ? `edit-${idParam}` : 'create-new'}
          onAlert={handleAlert}
          initialCreditReportingConsentAudit={creditReportingConsentAudit}
