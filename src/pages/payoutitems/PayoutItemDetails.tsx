...
import PayoutItemForm from '../../forms/PayoutItemForm.tsx';
import type {PayoutItem} from '../../types/PayoutItem';
import { getPayoutItem } from '../../apis/usePayoutItem';

export default function PayoutItemDetails() {
  ...
  const [payoutItem, setPayoutItem] = React.useState<PayoutItem | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getPayoutItem(idParam!);
          setPayoutItem(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit PayoutItem" : "Create PayoutItem"}
        trail={[
          { label: 'Tables' },
          { label: 'PayoutItem List', href: '/payoutitems' },
          { label: isEditMode ? 'Edit PayoutItem' : 'Create PayoutItem', active: true }
        ]}
        actions={[]}
      />
