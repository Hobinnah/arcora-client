...
import ListingTermPriceForm from '../../forms/ListingTermPriceForm.tsx';
import type {ListingTermPrice} from '../../types/ListingTermPrice';
import { getListingTermPrice } from '../../apis/useListingTermPrice';

export default function ListingTermPriceDetails() {
  ...
  const [listingTermPrice, setListingTermPrice] = React.useState<ListingTermPrice | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getListingTermPrice(idParam!);
          setListingTermPrice(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit ListingTermPrice" : "Create ListingTermPrice"}
        trail={[
          { label: 'Tables' },
          { label: 'ListingTermPrice List', href: '/listingtermprices' },
          { label: isEditMode ? 'Edit ListingTermPrice' : 'Create ListingTermPrice', active: true }
        ]}
        actions={[]}
      />
      ...
      {/* Form */}
      {!loading && (
        <ListingTermPriceForm
          key={isEditMode ? `edit-${idParam}` : 'create-new'}
          onAlert={handleAlert}
          initialListingTermPrice={listingTermPrice}
