...
import { getRating } from '../../apis/useRating';
...
export default function RatingDetails() {
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getRating(idParam!);
          setRating(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit Rating" : "Create Rating"}
        trail={[
          { label: 'Tables' },
          { label: 'Rating List', href: '/ratings' },
          { label: isEditMode ? 'Edit Rating' : 'Create Rating', active: true }
        ]}
        actions={[]}
      />
