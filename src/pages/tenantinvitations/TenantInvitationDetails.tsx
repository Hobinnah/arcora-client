...
import TenantInvitationForm from '../../forms/TenantInvitationForm.tsx';
import type {TenantInvitation} from '../../types/TenantInvitation';
import { getTenantInvitation } from '../../apis/useTenantInvitation';

export default function TenantInvitationDetails() {
  ...
  const [tenantInvitation, setTenantInvitation] = React.useState<TenantInvitation | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getTenantInvitation(idParam!);
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit TenantInvitation" : "Create TenantInvitation"}
        trail={[
          { label: 'Tables' },
          { label: 'TenantInvitation List', href: '/tenantinvitations' },
          { label: isEditMode ? 'Edit TenantInvitation' : 'Create TenantInvitation', active: true }
        ]}
        actions={[]}
      />
      ...
      {!loading && (
        <TenantInvitationForm
          ...
          onAlert={handleAlert}
          initialTenantInvitation={tenantInvitation}
