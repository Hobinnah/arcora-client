...
import LedgerTransactionForm from '../../forms/LedgerTransactionForm.tsx';
import type {LedgerTransaction} from '../../types/LedgerTransaction';
import { getLedgerTransaction } from '../../apis/useLedgerTransaction';

export default function LedgerTransactionDetails() {
  ...
  const [ledgerTransaction, setLedgerTransaction] = React.useState<LedgerTransaction | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getLedgerTransaction(idParam!);
          setLedgerTransaction(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit LedgerTransaction" : "Create LedgerTransaction"}
        trail={[
          { label: 'Tables' },
          { label: 'LedgerTransaction List', href: '/ledgertransactions' },
          { label: isEditMode ? 'Edit LedgerTransaction' : 'Create LedgerTransaction', active: true }
        ]}
        actions={[]}
      />
      ...
      {!loading && (
        <LedgerTransactionForm
          ...
          onAlert={handleAlert}
          initialLedgerTransaction={ledgerTransaction}
