...
import ListingAccessInstructionForm from '../../forms/ListingAccessInstructionForm.tsx';
import type {ListingAccessInstruction} from '../../types/ListingAccessInstruction';
import { getListingAccessInstruction } from '../../apis/useListingAccessInstruction';

export default function ListingAccessInstructionDetails() {
  ...
  const [listingAccessInstruction, setListingAccessInstruction] = React.useState<ListingAccessInstruction | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getListingAccessInstruction(idParam!);
          setListingAccessInstruction(data);
        } catch (error) {
        ...
        // New record
        setIsEditMode(false);
        setListingAccessInstruction(null);
        setLoading(false);
      }
    };
    fetchItem();
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit ListingAccessInstruction" : "Create ListingAccessInstruction"}
        trail={[
          { label: 'Tables' },
          { label: 'ListingAccessInstruction List', href: '/listingaccessinstructions' },
          { label: isEditMode ? 'Edit ListingAccessInstruction' : 'Create ListingAccessInstruction', active: true }
        ]}
        actions={[]}
      />
      ...
      {/* Form */}
      {!loading && (
        <ListingAccessInstructionForm
          key={isEditMode ? `edit-${idParam}` : 'create-new'}
          onAlert={handleAlert}
          initialListingAccessInstruction={listingAccessInstruction}
