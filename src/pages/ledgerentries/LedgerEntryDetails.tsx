...
import LedgerEntryForm from '../../forms/LedgerEntryForm.tsx';
import type {LedgerEntry} from '../../types/LedgerEntry';
import { getLedgerEntry } from '../../apis/useLedgerEntry';
...
export default function LedgerEntryDetails() {
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getLedgerEntry(idParam!);
          setLedgerEntry(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit LedgerEntry" : "Create LedgerEntry"}
        trail={[
          { label: 'Tables' },
          { label: 'LedgerEntry List', href: '/ledgerentries' },
          { label: isEditMode ? 'Edit LedgerEntry' : 'Create LedgerEntry', active: true }
        ]}
        actions={[]}
      />
      ...
      {!loading && (
        <LedgerEntryForm
          ...
          onAlert={handleAlert}
          initialLedgerEntry={ledgerEntry}
