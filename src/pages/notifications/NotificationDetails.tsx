...
import NotificationForm from '../../forms/NotificationForm.tsx';
import type {Notification} from '../../types/Notification';
import { getNotification } from '../../apis/useNotification';
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getNotification(idParam!);
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        ...
        trail={[
          { label: 'Tables' },
          { label: 'Notification List', href: '/notifications' },
          { label: isEditMode ? 'Edit Notification' : 'Create Notification', active: true }
        ]}
        actions={[]}
      />
