...
import InspectionItemForm from '../../forms/InspectionItemForm.tsx';
import type {InspectionItem} from '../../types/InspectionItem';
import { getInspectionItem } from '../../apis/useInspectionItem';
...
export default function InspectionItemDetails() {
  ...
  const [inspectionItem, setInspectionItem] = React.useState<InspectionItem | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getInspectionItem(idParam!);
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit InspectionItem" : "Create InspectionItem"}
        trail={[
          { label: 'Tables' },
          { label: 'InspectionItem List', href: '/inspectionitems' },
          { label: isEditMode ? 'Edit InspectionItem' : 'Create InspectionItem', active: true }
        ]}
        actions={[]}
      />
      ...
      {!loading && (
        <InspectionItemForm
          ...
          onAlert={handleAlert}
          initialInspectionItem={inspectionItem}
