...
import ListingRuleForm from '../../forms/ListingRuleForm.tsx';
import type {ListingRule} from '../../types/ListingRule';
import { getListingRule } from '../../apis/useListingRule';

export default function ListingRuleDetails() {
  ...
  const [listingRule, setListingRule] = React.useState<ListingRule | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getListingRule(idParam!);
          setListingRule(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit ListingRule" : "Create ListingRule"}
        trail={[
          { label: 'Tables' },
          { label: 'ListingRule List', href: '/listingrules' },
          { label: isEditMode ? 'Edit ListingRule' : 'Create ListingRule', active: true }
        ]}
        actions={[]}
      />
      ...
      {!loading && (
        <ListingRuleForm
          ...
          onAlert={handleAlert}
          initialListingRule={listingRule}
