...
import AutopayConsentAuditForm from '../../forms/AutopayConsentAuditForm.tsx';
import type {AutopayConsentAudit} from '../../types/AutopayConsentAudit';
import { getAutopayConsentAudit } from '../../apis/useAutopayConsentAudit';

export default function AutopayConsentAuditDetails() {
  ...
  const [autopayConsentAudit, setAutopayConsentAudit] = React.useState<AutopayConsentAudit | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getAutopayConsentAudit(idParam!);
          setAutopayConsentAudit(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit AutopayConsentAudit" : "Create AutopayConsentAudit"}
        trail={[
          { label: 'Tables' },
          { label: 'AutopayConsentAudit List', href: '/autopayconsentaudits' },
          { label: isEditMode ? 'Edit AutopayConsentAudit' : 'Create AutopayConsentAudit', active: true }
        ]}
        actions={[]}
      />
      ...
      {/* Form */}
      {!loading && (
        <AutopayConsentAuditForm
          key={isEditMode ? `edit-${idParam}` : 'create-new'}
          onAlert={handleAlert}
          initialAutopayConsentAudit={autopayConsentAudit}
