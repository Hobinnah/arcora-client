...
import TaxRateForm from '../../forms/TaxRateForm.tsx';
import type {TaxRate} from '../../types/TaxRate';
import { getTaxRate } from '../../apis/useTaxRate';

export default function TaxRateDetails() {
  ...
  const [taxRate, setTaxRate] = React.useState<TaxRate | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getTaxRate(idParam!);
          setTaxRate(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit TaxRate" : "Create TaxRate"}
        trail={[
          { label: 'Tables' },
          { label: 'TaxRate List', href: '/taxrates' },
          { label: isEditMode ? 'Edit TaxRate' : 'Create TaxRate', active: true }
        ]}
        actions={[]}
      />
