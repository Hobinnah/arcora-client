...
import PaymentAttemptForm from '../../forms/PaymentAttemptForm.tsx';
import type {PaymentAttempt} from '../../types/PaymentAttempt';
import { getPaymentAttempt } from '../../apis/usePaymentAttempt';
...
export default function PaymentAttemptDetails() {
  ...
  const [paymentAttempt, setPaymentAttempt] = React.useState<PaymentAttempt | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getPaymentAttempt(idParam!);
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit PaymentAttempt" : "Create PaymentAttempt"}
        trail={[
          { label: 'Tables' },
          { label: 'PaymentAttempt List', href: '/paymentattempts' },
          { label: isEditMode ? 'Edit PaymentAttempt' : 'Create PaymentAttempt', active: true }
        ]}
        actions={[]}
      />
      ...
      {!loading && (
        <PaymentAttemptForm
          ...
          onAlert={handleAlert}
          initialPaymentAttempt={paymentAttempt}
