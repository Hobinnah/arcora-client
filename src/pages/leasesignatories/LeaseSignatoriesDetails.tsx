...
import LeaseSignatoriesForm from '../../forms/LeaseSignatoriesForm.tsx';
import type {LeaseSignatories} from '../../types/LeaseSignatories';
import { getLeaseSignatoriesById } from '../../apis/useLeaseSignatories';

export default function LeaseSignatoriesDetails() {
  ...
  const [leaseSignatories, setLeaseSignatories] = React.useState<LeaseSignatories | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getLeaseSignatoriesById(idParam!);
          setLeaseSignatories(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit LeaseSignatories" : "Create LeaseSignatories"}
        trail={[
          { label: 'Tables' },
          { label: 'LeaseSignatories List', href: '/leasesignatories' },
          { label: isEditMode ? 'Edit LeaseSignatories' : 'Create LeaseSignatories', active: true }
        ]}
        actions={[]}
      />
      ...
      {/* Form */}
      {!loading && (
        <LeaseSignatoriesForm
          key={isEditMode ? `edit-${idParam}` : 'create-new'}
          onAlert={handleAlert}
          initialLeaseSignatories={leaseSignatories}
