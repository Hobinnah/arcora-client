...
import TenantScreeningCheckForm from '../../forms/TenantScreeningCheckForm.tsx';
import type {TenantScreeningCheck} from '../../types/TenantScreeningCheck';
import { getTenantScreeningCheck } from '../../apis/useTenantScreeningCheck';

export default function TenantScreeningCheckDetails() {
  ...
  const [tenantScreeningCheck, setTenantScreeningCheck] = React.useState<TenantScreeningCheck | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getTenantScreeningCheck(idParam!);
          setTenantScreeningCheck(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit TenantScreeningCheck" : "Create TenantScreeningCheck"}
        trail={[
          { label: 'Tables' },
          { label: 'TenantScreeningCheck List', href: '/tenantscreeningchecks' },
          { label: isEditMode ? 'Edit TenantScreeningCheck' : 'Create TenantScreeningCheck', active: true }
        ]}
        actions={[]}
      />
      ...
      {/* Form */}
      {!loading && (
        <TenantScreeningCheckForm
          key={isEditMode ? `edit-${idParam}` : 'create-new'}
          onAlert={handleAlert}
          initialTenantScreeningCheck={tenantScreeningCheck}
