...
import FraudCaseForm from '../../forms/FraudCaseForm.tsx';
import type {FraudCase} from '../../types/FraudCase';
import { getFraudCase } from '../../apis/useFraudCase';
...
export default function FraudCaseDetails() {
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getFraudCase(idParam!);
          setFraudCase(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit FraudCase" : "Create FraudCase"}
        trail={[
          { label: 'Tables' },
          { label: 'FraudCase List', href: '/fraudcases' },
          { label: isEditMode ? 'Edit FraudCase' : 'Create FraudCase', active: true }
        ]}
        actions={[]}
      />
