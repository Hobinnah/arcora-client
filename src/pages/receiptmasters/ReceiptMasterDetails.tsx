...
import ReceiptMasterForm from '../../forms/ReceiptMasterForm.tsx';
import type {ReceiptMaster} from '../../types/ReceiptMaster';
import { getReceiptMaster } from '../../apis/useReceiptMaster';

export default function ReceiptMasterDetails() {
  ...
  const [receiptMaster, setReceiptMaster] = React.useState<ReceiptMaster | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getReceiptMaster(idParam!);
          setReceiptMaster(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit ReceiptMaster" : "Create ReceiptMaster"}
        trail={[
          { label: 'Tables' },
          { label: 'ReceiptMaster List', href: '/receiptmasters' },
          { label: isEditMode ? 'Edit ReceiptMaster' : 'Create ReceiptMaster', active: true }
        ]}
        actions={[]}
      />
      ...
      {!loading && (
        <ReceiptMasterForm
          ...
          onAlert={handleAlert}
          initialReceiptMaster={receiptMaster}
