...
import ListingPhotoForm from '../../forms/ListingPhotoForm.tsx';
import type {ListingPhoto} from '../../types/ListingPhoto';
import { getListingPhoto } from '../../apis/useListingPhoto';

export default function ListingPhotoDetails() {
  ...
  const [listingPhoto, setListingPhoto] = React.useState<ListingPhoto | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getListingPhoto(idParam!);
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit ListingPhoto" : "Create ListingPhoto"}
        trail={[
          { label: 'Tables' },
          { label: 'ListingPhoto List', href: '/listingphotos' },
          { label: isEditMode ? 'Edit ListingPhoto' : 'Create ListingPhoto', active: true }
        ]}
        actions={[]}
      />
      ...
      {!loading && (
        <ListingPhotoForm
          ...
          onAlert={handleAlert}
          initialListingPhoto={listingPhoto}
