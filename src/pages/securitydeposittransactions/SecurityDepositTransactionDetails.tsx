...
import SecurityDepositTransactionForm from '../../forms/SecurityDepositTransactionForm.tsx';
import type {SecurityDepositTransaction} from '../../types/SecurityDepositTransaction';
import { getSecurityDepositTransaction } from '../../apis/useSecurityDepositTransaction';

export default function SecurityDepositTransactionDetails() {
  ...
  const [securityDepositTransaction, setSecurityDepositTransaction] = React.useState<SecurityDepositTransaction | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getSecurityDepositTransaction(idParam!);
          setSecurityDepositTransaction(data);
        } catch (error) {
        ...
        // New record
        setIsEditMode(false);
        setSecurityDepositTransaction(null);
        setLoading(false);
      }
    };
    fetchItem();
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit SecurityDepositTransaction" : "Create SecurityDepositTransaction"}
        trail={[
          { label: 'Tables' },
          { label: 'SecurityDepositTransaction List', href: '/securitydeposittransactions' },
          { label: isEditMode ? 'Edit SecurityDepositTransaction' : 'Create SecurityDepositTransaction', active: true }
        ]}
        actions={[]}
      />
      ...
      {!loading && (
        <SecurityDepositTransactionForm
          ...
          onAlert={handleAlert}
          initialSecurityDepositTransaction={securityDepositTransaction}
