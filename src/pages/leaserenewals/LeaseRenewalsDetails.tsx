...
export default function LeaseRenewalsDetails() {
  ...
  const [leaseRenewals, setLeaseRenewals] = React.useState<LeaseRenewals | null>(null);
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit LeaseRenewals" : "Create LeaseRenewals"}
        trail={[
          { label: 'Tables' },
          { label: 'LeaseRenewals List', href: '/leaserenewals' },
          { label: isEditMode ? 'Edit LeaseRenewals' : 'Create LeaseRenewals', active: true }
        ]}
        actions={[]}
      />
      ...
      {!loading && (
        <LeaseRenewalsForm
          ...
          onAlert={handleAlert}
          initialLeaseRenewals={leaseRenewals}
