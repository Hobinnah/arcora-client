...
import FeeTypeForm from '../../forms/FeeTypeForm.tsx';
import type {FeeType} from '../../types/FeeType';
import { getFeeType } from '../../apis/useFeeType';

export default function FeeTypeDetails() {
  ...
  const [feeType, setFeeType] = React.useState<FeeType | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getFeeType(idParam!);
          setFeeType(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit FeeType" : "Create FeeType"}
        trail={[
          { label: 'Tables' },
          { label: 'FeeType List', href: '/feetypes' },
          { label: isEditMode ? 'Edit FeeType' : 'Create FeeType', active: true }
        ]}
        actions={[]}
      />
