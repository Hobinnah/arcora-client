...
import ListingPolicyForm from '../../forms/ListingPolicyForm.tsx';
import type {ListingPolicy} from '../../types/ListingPolicy';
import { getListingPolicy } from '../../apis/useListingPolicy';

export default function ListingPolicyDetails() {
  ...
  const [listingPolicy, setListingPolicy] = React.useState<ListingPolicy | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getListingPolicy(idParam!);
          setListingPolicy(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit ListingPolicy" : "Create ListingPolicy"}
        trail={[
          { label: 'Tables' },
          { label: 'ListingPolicy List', href: '/listingpolicies' },
          { label: isEditMode ? 'Edit ListingPolicy' : 'Create ListingPolicy', active: true }
        ]}
        actions={[]}
      />
      ...
      {!loading && (
        <ListingPolicyForm
          ...
          onAlert={handleAlert}
          initialListingPolicy={listingPolicy}
