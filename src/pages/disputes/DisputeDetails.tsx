...
import DisputeForm from '../../forms/DisputeForm.tsx';
import type {Dispute} from '../../types/Dispute';
import { getDispute } from '../../apis/useDispute';
...
export default function DisputeDetails() {
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getDispute(idParam!);
          setDispute(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit Dispute" : "Create Dispute"}
        trail={[
          { label: 'Tables' },
          { label: 'Dispute List', href: '/disputes' },
          { label: isEditMode ? 'Edit Dispute' : 'Create Dispute', active: true }
        ]}
        actions={[]}
      />
