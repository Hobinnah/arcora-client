...
import DisputeGrid from '../../grids/DisputeGrid.tsx';
import BreadCrum  from '../../components/BreadCrum';
import MasterLayout from '../../components/MasterLayout';
import Alert from '../../components/Alert';
import { PlusIcon, DownloadIcon } from '../../components/Icons';
import { deleteDispute, fetchDisputes, fireDisputeAction } from '../../apis/useDispute';
import { env, type AlertType } from '../../env';
import type { Dispute } from '../../types/Dispute';
import DisputeViewModal from './DisputeViewModal.tsx';
...
export default function DisputeList() {
  ...
  const handleExportData = async () => {
    ...
    try {
      const exportData = await fetchDisputes({ pageSize: 1000, pageNumber: 0, sortBy: 'status', sortDirection: 'desc' });
      const csvContent = convertToCSV(exportData.data);
      downloadCSV(csvContent, `disputes-export-${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const convertToCSV = (rows: Dispute[]): string => {
    ...
    const fmt = (k: string, v: any) => {
      try {
        ...
        switch (k) {
          ...
          case 'resolvedAt': return new Date(v as any).toLocaleDateString();
          default: return v;
        }
      } catch { return v; }
    };
    const headers = ["Status", "Title", "Description", "Resolution Notes", "Opened At", "Resolved At"];
    const keys = ["status", "title", "description", "resolutionNotes", "openedAt", "resolvedAt"];
  ...
  const handleView = useCallback((row: Dispute) => {
    setSelected(row);
  ...
  const handleEdit = useCallback((row: Dispute) => {
    const id = (row as any)?.disputeID;
    if (id === undefined || id === null) return;
    navigate(`/disputes/${id}`);
  }, [navigate]);

  const handleDelete = useCallback(async (row: Dispute, setItems: React.Dispatch<React.SetStateAction<Dispute[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => {
    const id = (row as any)?.disputeID;
    if (id === undefined || id === null) return;
    if (confirm('Are you sure you want to delete this record? This action cannot be undone.')) {
      try {
        await deleteDispute(id);
        setItems(prev => prev.filter(x => (x as any)?.disputeID !== id));
  ...
  const actionHandlers = React.useMemo(() => ({
    "set-active": async (row: any) => {
      const id = row?.disputeID;
      if (id === undefined || id === null) return;
      await fireDisputeAction(id, "Set ACTIVE");
  ...
  const pageContent = React.useMemo(() => [
    <div key="list-content">
      <BreadCrum
        title="Dispute List"
        trail={[{ label: 'Tables' }, { label: 'Dispute List', href: '/disputes', active: true }]}
        actions={[
          { label: 'Add Dispute', variant: 'soft', onClick: handleAdd, icon: <PlusIcon /> },
