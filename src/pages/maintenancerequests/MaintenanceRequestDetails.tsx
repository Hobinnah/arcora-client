...
import MaintenanceRequestForm from '../../forms/MaintenanceRequestForm.tsx';
import type {MaintenanceRequest} from '../../types/MaintenanceRequest';
import { getMaintenanceRequest } from '../../apis/useMaintenanceRequest';
...
export default function MaintenanceRequestDetails() {
  ...
  const [maintenanceRequest, setMaintenanceRequest] = React.useState<MaintenanceRequest | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getMaintenanceRequest(idParam!);
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit MaintenanceRequest" : "Create MaintenanceRequest"}
        trail={[
          { label: 'Tables' },
          { label: 'MaintenanceRequest List', href: '/maintenancerequests' },
          { label: isEditMode ? 'Edit MaintenanceRequest' : 'Create MaintenanceRequest', active: true }
        ]}
        actions={[]}
      />
      ...
      {!loading && (
        <MaintenanceRequestForm
          ...
          onAlert={handleAlert}
          initialMaintenanceRequest={maintenanceRequest}
