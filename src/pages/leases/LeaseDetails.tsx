...
import LeaseForm from '../../forms/LeaseForm.tsx';
import type {Lease} from '../../types/Lease';
import { getLease } from '../../apis/useLease';
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getLease(idParam!);
          setLease(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        ...
        trail={[
          { label: 'Tables' },
          { label: 'Lease List', href: '/leases' },
          { label: isEditMode ? 'Edit Lease' : 'Create Lease', active: true }
        ]}
        actions={[]}
      />
