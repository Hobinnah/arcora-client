...
import CreditReportingForm from '../../forms/CreditReportingForm.tsx';
import type {CreditReporting} from '../../types/CreditReporting';
import { getCreditReporting } from '../../apis/useCreditReporting';

export default function CreditReportingDetails() {
  ...
  const [creditReporting, setCreditReporting] = React.useState<CreditReporting | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getCreditReporting(idParam!);
          setCreditReporting(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit CreditReporting" : "Create CreditReporting"}
        trail={[
          { label: 'Tables' },
          { label: 'CreditReporting List', href: '/creditreportings' },
          { label: isEditMode ? 'Edit CreditReporting' : 'Create CreditReporting', active: true }
        ]}
        actions={[]}
      />
      ...
      {!loading && (
        <CreditReportingForm
          ...
          onAlert={handleAlert}
          initialCreditReporting={creditReporting}
