...
import ChargebackForm from '../../forms/ChargebackForm.tsx';
import type {Chargeback} from '../../types/Chargeback';
import { getChargeback } from '../../apis/useChargeback';
...
export default function ChargebackDetails() {
  ...
  const [chargeback, setChargeback] = React.useState<Chargeback | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getChargeback(idParam!);
          setChargeback(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit Chargeback" : "Create Chargeback"}
        trail={[
          { label: 'Tables' },
          { label: 'Chargeback List', href: '/chargebacks' },
          { label: isEditMode ? 'Edit Chargeback' : 'Create Chargeback', active: true }
        ]}
        actions={[]}
      />
