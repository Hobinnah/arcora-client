...
import ChargebackGrid from '../../grids/ChargebackGrid.tsx';
import BreadCrum  from '../../components/BreadCrum';
import MasterLayout from '../../components/MasterLayout';
import Alert from '../../components/Alert';
import { PlusIcon, DownloadIcon } from '../../components/Icons';
import { deleteChargeback, fetchChargebacks, fireChargebackAction } from '../../apis/useChargeback';
import { env, type AlertType } from '../../env';
import type { Chargeback } from '../../types/Chargeback';
import ChargebackViewModal from './ChargebackViewModal.tsx';
...
export default function ChargebackList() {
  ...
  const [selected, setSelected] = React.useState<Chargeback | null>(null);

  const handleAdd = () => {
    navigate('/chargebacks/new');
  };
  ...
  const handleExportData = async () => {
    ...
    try {
      const exportData = await fetchChargebacks({ pageSize: 1000, pageNumber: 0, sortBy: 'tenantID', sortDirection: 'desc' });
      const csvContent = convertToCSV(exportData.data);
      downloadCSV(csvContent, `chargebacks-export-${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const convertToCSV = (rows: Chargeback[]): string => {
    ...
    const headers = ["Tenant", "Provider Dispute ID", "Amount", "Currency", "Status", "Opened At", "Captured Date", "Captured By"];
    const keys = ["tenantID", "providerDisputeID", "amount", "currency", "status", "openedAt", "capturedDate", "capturedBy"];
  ...
  const handleView = useCallback((row: Chargeback) => {
    setSelected(row);
  ...
  const handleEdit = useCallback((row: Chargeback) => {
    const id = (row as any)?.chargebackID;
    if (id === undefined || id === null) return;
    navigate(`/chargebacks/${id}`);
  }, [navigate]);

  const handleDelete = useCallback(async (row: Chargeback, setItems: React.Dispatch<React.SetStateAction<Chargeback[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => {
    const id = (row as any)?.chargebackID;
    if (id === undefined || id === null) return;
    if (confirm('Are you sure you want to delete this record? This action cannot be undone.')) {
      try {
        await deleteChargeback(id);
        setItems(prev => prev.filter(x => (x as any)?.chargebackID !== id));
  ...
  const actionHandlers = React.useMemo(() => ({
    "set-closed": async (row: any) => {
      const id = row?.chargebackID;
      if (id === undefined || id === null) return;
      await fireChargebackAction(id, "Set CLOSED");
  ...
  const pageContent = React.useMemo(() => [
    <div key="list-content">
      <BreadCrum
        ...
        trail={[{ label: 'Tables' }, { label: 'Chargeback List', href: '/chargebacks', active: true }]}
        actions={[
          { label: 'Add Chargeback', variant: 'soft', onClick: handleAdd, icon: <PlusIcon /> },
