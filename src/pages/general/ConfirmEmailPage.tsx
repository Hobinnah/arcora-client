...
const ConfirmEmailPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    console.log('[ConfirmEmailPage] URL parsed:', {
    ...
    if (!tokenParam) {
      console.warn('[ConfirmEmailPage] Missing token param');
      setConfirmationError('Invalid confirmation link. Missing token parameter.');
      setTokenValid(false);
      setProcessing(false);
      return;
    }

    const hasUserId = !!userIdParam;
    const hasEmail = !!emailParam;

    if (!hasUserId && !hasEmail) {
      console.warn('[ConfirmEmailPage] Missing both userId and email');
      setConfirmationError('Invalid confirmation link. Missing userId or email parameter.');
      setTokenValid(false);
      setProcessing(false);
      return;
    }
    ...
    if (hasEmail) {
      ...
      if (!emailRegex.test(emailParam)) {
        console.warn('[ConfirmEmailPage] Invalid email format:', emailParam);
    ...
    if (tokenParam.length < 10) {
      console.warn('[ConfirmEmailPage] Token too short:', tokenParam.length);
    ...
    handleConfirmation(userIdParam || emailParam, tokenParam);
  }, [searchParams]);
   ...
   */
  const handleConfirmation = async (_identifier: string, confirmationToken: string) => {
    try {
      setProcessing(true);
      setConfirmationError(null);
      setConfirmationSuccess(false);

      const userIdValue = new URLSearchParams(window.location.search).get('userId');
      const emailValue = new URLSearchParams(window.location.search).get('email');

      console.log('[ConfirmEmailPage] Submitting confirmation request:', {
      ...
      await confirmEmail({
        ...
        emailAddress: emailValue || undefined,
