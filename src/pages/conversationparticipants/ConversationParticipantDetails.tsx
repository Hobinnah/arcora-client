...
import ConversationParticipantForm from '../../forms/ConversationParticipantForm.tsx';
import type {ConversationParticipant} from '../../types/ConversationParticipant';
import { getConversationParticipant } from '../../apis/useConversationParticipant';

export default function ConversationParticipantDetails() {
  ...
  const [conversationParticipant, setConversationParticipant] = React.useState<ConversationParticipant | null>(null);
  ...
  React.useEffect(() => {
    const fetchItem = async () => {
      ...
      if (isValidId) {
        ...
        setIsEditMode(true);
        try {
          const data = await getConversationParticipant(idParam!);
          setConversationParticipant(data);
        } catch (error) {
  ...
  const pageContent = [
    <React.Fragment key="details-content">
      <BreadCrum
        title={isEditMode ? "Edit ConversationParticipant" : "Create ConversationParticipant"}
        trail={[
          { label: 'Tables' },
          { label: 'ConversationParticipant List', href: '/conversationparticipants' },
          { label: isEditMode ? 'Edit ConversationParticipant' : 'Create ConversationParticipant', active: true }
        ]}
        actions={[]}
      />
      ...
      {!loading && (
        <ConversationParticipantForm
          ...
          onAlert={handleAlert}
          initialConversationParticipant={conversationParticipant}
