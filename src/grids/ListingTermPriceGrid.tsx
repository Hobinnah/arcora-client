...
import { fetchListingTermPrices } from '../apis/useListingTermPrice';
...
export interface ListingTermPriceGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: ListingTermPrice)=>void;
  onEdit?: (r: ListingTermPrice)=>void;
  onDelete?: (r: ListingTermPrice, setItems: React.Dispatch<React.SetStateAction<ListingTermPrice[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const ListingTermPriceGrid: React.FC<ListingTermPriceGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    ...
    { key: "securityDepositAmount", label: "Security Deposit Amount" },
  ...
  const [items, setItems] = useState<ListingTermPrice[]>([]);
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchListingTermPrices(params);
  ...
  const handleDelete = async (row: ListingTermPrice) => {
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.listingTermPriceID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.listingTermPriceID ?? Math.random()}>
                    <td>{String(r?.leaseTermMonths ?? '-')}</td>
                    <td>{String(r?.monthlyRentAmount ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.listingTermPriceID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.listingTermPriceID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.listingTermPriceID ?? '')] || {}),
                                right: 0,
...
export default ListingTermPriceGrid;
