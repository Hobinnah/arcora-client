...
import { fetchReceiptMasters } from '../apis/useReceiptMaster';
...
export interface ReceiptMasterGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: ReceiptMaster)=>void;
  onEdit?: (r: ReceiptMaster)=>void;
  onDelete?: (r: ReceiptMaster, setItems: React.Dispatch<React.SetStateAction<ReceiptMaster[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const ReceiptMasterGrid: React.FC<ReceiptMasterGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const [items, setItems] = useState<ReceiptMaster[]>([]);
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchReceiptMasters(params);
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: ReceiptMaster) => {
      const s = (String((r as any)?.receiptNumber ?? '') + ' ' + String((r as any)?.amount ?? '') + ' ' + String((r as any)?.currency ?? '') + ' ' + String((r as any)?.issuedAt ?? '') + ' ' + String((r as any)?.voidedAt ?? '')).toLowerCase();
  ...
  const handleDelete = async (row: ReceiptMaster) => {
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.receiptMasterID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.receiptMasterID ?? Math.random()}>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.receiptMasterID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.receiptMasterID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.receiptMasterID ?? '')] || {}),
                                right: 0,
...
export default ReceiptMasterGrid;
