...
export interface ListingTypeGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: ListingType)=>void;
  onEdit?: (r: ListingType)=>void;
  onDelete?: (r: ListingType, setItems: React.Dispatch<React.SetStateAction<ListingType[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const ListingTypeGrid: React.FC<ListingTypeGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchListingTypes(params);
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: ListingType) => {
  ...
  const handleDelete = async (row: ListingType) => {
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.listingTypeID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.listingTypeID ?? Math.random()}>
                    <td>{String(r?.name ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.listingTypeID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.listingTypeID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.listingTypeID ?? '')] || {}),
                                right: 0,
...
export default ListingTypeGrid;
