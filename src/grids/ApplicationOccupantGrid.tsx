...
export interface ApplicationOccupantGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: ApplicationOccupant)=>void;
  onEdit?: (r: ApplicationOccupant)=>void;
  onDelete?: (r: ApplicationOccupant, setItems: React.Dispatch<React.SetStateAction<ApplicationOccupant[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const ApplicationOccupantGrid: React.FC<ApplicationOccupantGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    ...
    { key: "isPrimaryApplicant", label: "Is Primary Applicant" },
  ...
  const [items, setItems] = useState<ApplicationOccupant[]>([]);
  ...
  const [occupant_type, set_occupant_type] = useState<string>('');
  ...
  const load = useCallback(async () => {
    try {
      ...
      const params: any = env.SEARCH_STRATEGY === 'server' ? {
        ...
        occupantType: occupant_type || undefined,
      } : {
      ...
      const result = await fetchApplicationOccupants(params);
    ...
    ? [pageSize, page, query, sort, status, occupant_type, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && status === '' && occupant_type === '') return items;

    let res = items.filter((r: ApplicationOccupant) => {
      const s = (String((r as any)?.firstName ?? '') + ' ' + String((r as any)?.lastName ?? '') + ' ' + String((r as any)?.dateOfBirth ?? '') + ' ' + String((r as any)?.email ?? '') + ' ' + String((r as any)?.phoneNumber ?? '') + ' ' + String((r as any)?.occupantType ?? '') + ' ' + String((r as any)?.status ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.capturedBy ?? '') + ' ' + String((r as any)?.updatedDate ?? '') + ' ' + String((r as any)?.updatedBy ?? '')).toLowerCase();
      const matchesSearch = !query || s.includes(query.toLowerCase());
      const match_status = !status || String((r as any)?.status ?? '') === status;
      const match_occupant_type = !occupant_type || String((r as any)?.occupantType ?? '') === occupant_type;
      return matchesSearch && match_status && match_occupant_type;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, status, occupant_type]);
  ...
  useEffect(() => { if (query || sort || status || occupant_type) setPage(1); }, [query, sort, status, occupant_type]);
  ...
  const handleDelete = async (row: ApplicationOccupant) => {
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/applicationoccupant/updateapplicationoccupantstatus/{ApplicationOccupantID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "ApplicationOccupantID", type: "string", required: true, source: "record", options: [] }, { name: "Status", type: "string", required: true, source: "constant", default: "APPROVED", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.applicationOccupantID) === recordId);
      ...
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            ...
            <select className="select" value={ occupant_type } onChange={(e) => set_occupant_type(e.target.value)}>
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.applicationOccupantID ?? Math.random()}>
                    <td>{String(r?.firstName ?? '-')}</td>
                    <td>{String(r?.lastName ?? '-')}</td>
                    <td>{r?.dateOfBirth ? new Date(r?.dateOfBirth as any).toLocaleDateString() : '-'}</td>
                    <td>{String(r?.email ?? '-')}</td>
                    <td>{String(r?.phoneNumber ?? '-')}</td>
                    <td>{String(r?.occupantType ?? '-')}</td>
                    <td>{String(r?.isPrimaryApplicant ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.applicationOccupantID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.applicationOccupantID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.applicationOccupantID ?? '')] || {}),
                                right: 0,
...
export default ApplicationOccupantGrid;
