...
import { fetchInspectionItems } from '../apis/useInspectionItem';
...
export interface InspectionItemGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: InspectionItem)=>void;
  onEdit?: (r: InspectionItem)=>void;
  onDelete?: (r: InspectionItem, setItems: React.Dispatch<React.SetStateAction<InspectionItem[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const InspectionItemGrid: React.FC<InspectionItemGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    ...
    { key: "requiresRepair", label: "Requires Repair" },
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchInspectionItems(params);
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: InspectionItem) => {
      const s = (String((r as any)?.area ?? '') + ' ' + String((r as any)?.itemName ?? '') + ' ' + String((r as any)?.condition ?? '') + ' ' + String((r as any)?.estimatedRepairCost ?? '')).toLowerCase();
  ...
  const handleDelete = async (row: InspectionItem) => {
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.inspectionItemID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.inspectionItemID ?? Math.random()}>
                    <td>{String(r?.area ?? '-')}</td>
                    <td>{String(r?.itemName ?? '-')}</td>
                    <td>{String(r?.condition ?? '-')}</td>
                    <td>{String(r?.requiresRepair ?? '-')}</td>
                    <td>{String(r?.estimatedRepairCost ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.inspectionItemID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.inspectionItemID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.inspectionItemID ?? '')] || {}),
                                right: 0,
...
export default InspectionItemGrid;
