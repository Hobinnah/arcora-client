...
import { fetchFeeTypes } from '../apis/useFeeType';
...
export interface FeeTypeGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: FeeType)=>void;
  onEdit?: (r: FeeType)=>void;
  onDelete?: (r: FeeType, setItems: React.Dispatch<React.SetStateAction<FeeType[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const FeeTypeGrid: React.FC<FeeTypeGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    { key: "name", label: "Name" },
    { key: "isPlatformFee", label: "Is Platform Fee" },
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchFeeTypes(params);
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: FeeType) => {
      const s = (String((r as any)?.name ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.capturedBy ?? '')).toLowerCase();
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.feeTypeID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.feeTypeID ?? Math.random()}>
                    <td>{String(r?.name ?? '-')}</td>
                    <td>{String(r?.isPlatformFee ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.feeTypeID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.feeTypeID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.feeTypeID ?? '')] || {}),
                                right: 0,
