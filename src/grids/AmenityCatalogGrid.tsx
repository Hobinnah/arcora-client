...
export interface AmenityCatalogGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: AmenityCatalog)=>void;
  onEdit?: (r: AmenityCatalog)=>void;
  onDelete?: (r: AmenityCatalog, setItems: React.Dispatch<React.SetStateAction<AmenityCatalog[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const AmenityCatalogGrid: React.FC<AmenityCatalogGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const [items, setItems] = useState<AmenityCatalog[]>([]);
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchAmenityCatalogs(params);
    ...
    ? [pageSize, page, query, sort, category, active_status, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && category === '' && active_status === '') return items;

    let res = items.filter((r: AmenityCatalog) => {
      const s = (String((r as any)?.name ?? '') + ' ' + String((r as any)?.category ?? '') + ' ' + String((r as any)?.capturedDate ?? '')).toLowerCase();
      const matchesSearch = !query || s.includes(query.toLowerCase());
      const match_category = !category || String((r as any)?.category ?? '') === category;
      const match_active_status = !active_status || String((r as any)?.isActive ?? '') === active_status;
      return matchesSearch && match_category && match_active_status;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, category, active_status]);
  ...
  useEffect(() => { if (query || sort || category || active_status) setPage(1); }, [query, sort, category, active_status]);
  ...
  const handleDelete = async (row: AmenityCatalog) => {
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.amenityID) === recordId);
      ...
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            <span>Category</span>
            <input className="input" placeholder="All" value={ category } onChange={(e) => set_category(e.target.value)} />
          </label>
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.amenityID ?? Math.random()}>
                    <td>{String(r?.name ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.amenityID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.amenityID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.amenityID ?? '')] || {}),
                                right: 0,
...
export default AmenityCatalogGrid;
