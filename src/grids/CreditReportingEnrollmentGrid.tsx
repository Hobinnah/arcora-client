...
import { fetchCreditReportingEnrollments } from '../apis/useCreditReportingEnrollment';
...
export interface CreditReportingEnrollmentGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: CreditReportingEnrollment)=>void;
  onEdit?: (r: CreditReportingEnrollment)=>void;
  onDelete?: (r: CreditReportingEnrollment, setItems: React.Dispatch<React.SetStateAction<CreditReportingEnrollment[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const CreditReportingEnrollmentGrid: React.FC<CreditReportingEnrollmentGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const [items, setItems] = useState<CreditReportingEnrollment[]>([]);
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchCreditReportingEnrollments(params);
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: CreditReportingEnrollment) => {
      const s = (String((r as any)?.status ?? '') + ' ' + String((r as any)?.consentedAt ?? '') + ' ' + String((r as any)?.cancelledAt ?? '')).toLowerCase();
  ...
  const handleDelete = async (row: CreditReportingEnrollment) => {
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/creditreportingenrollment/updateCreditReportingEnrollment/{CreditReportingEnrollmentID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "CreditReportingEnrollmentID", type: "string", required: true, source: "record", options: [] }, { name: "status", type: "string", required: true, source: "constant", default: "ACTIVE", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.creditReportingEnrollmentID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.creditReportingEnrollmentID ?? Math.random()}>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.creditReportingEnrollmentID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.creditReportingEnrollmentID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.creditReportingEnrollmentID ?? '')] || {}),
                                right: 0,
...
export default CreditReportingEnrollmentGrid;
