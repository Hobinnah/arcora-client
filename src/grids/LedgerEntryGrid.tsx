...
import { fetchLedgerEntries } from '../apis/useLedgerEntry';
...
export interface LedgerEntryGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: LedgerEntry)=>void;
  onEdit?: (r: LedgerEntry)=>void;
  onDelete?: (r: LedgerEntry, setItems: React.Dispatch<React.SetStateAction<LedgerEntry[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const LedgerEntryGrid: React.FC<LedgerEntryGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchLedgerEntries(params);
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: LedgerEntry) => {
      const s = (String((r as any)?.debitAmount ?? '') + ' ' + String((r as any)?.creditAmount ?? '') + ' ' + String((r as any)?.currency ?? '')).toLowerCase();
  ...
  const handleDelete = async (row: LedgerEntry) => {
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.ledgerEntryID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.ledgerEntryID ?? Math.random()}>
                    <td>{String(r?.debitAmount ?? '-')}</td>
                    <td>{String(r?.creditAmount ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.ledgerEntryID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.ledgerEntryID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.ledgerEntryID ?? '')] || {}),
                                right: 0,
...
export default LedgerEntryGrid;
