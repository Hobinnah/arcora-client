...
export interface FeeGridProps {
  ...
  onView?: (r: Fee)=>void;
  onEdit?: (r: Fee)=>void;
  onDelete?: (r: Fee, setItems: React.Dispatch<React.SetStateAction<Fee[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const FeeGrid: React.FC<FeeGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    ...
    { key: "percentageRate", label: "Percentage Rate" },
    { key: "minimumFeeAmount", label: "Minimum Fee Amount" },
    { key: "maximumFeeAmount", label: "Maximum Fee Amount" },
    { key: "currency", label: "Currency" },
    { key: "isTaxable", label: "Is Taxable" },
  ...
  const [effective_from, set_effective_from] = useState<string>('');

  const [lookup_feeTypeID, setLookup_feeTypeID] = useState<Record<string, string>>({});
  useEffect(() => {
    (async () => {
      try {
        const res = await fetchData<any>('api/feetype/get?pageSize=1000&pageNumber=1', 'lookup_feeTypeID');
        const payload = (res as any)?.data;
        const arr = Array.isArray(payload?.data) ? payload.data : (Array.isArray(payload) ? payload : []);
        const map: Record<string, string> = {};
        arr.forEach((it: any) => { map[String(it?.feeTypeID ?? it?.id ?? '')] = String(it?.name ?? it?.name ?? ''); });
        setLookup_feeTypeID(map);
      } catch { /* soft-fail */ }
    })();
  }, []);
  ...
  const load = useCallback(async () => {
    try {
      ...
      const params: any = env.SEARCH_STRATEGY === 'server' ? {
        ...
        effectiveFrom: effective_from || undefined,
      } : {
      ...
      const result = await fetchFees(params);
    ...
    ? [pageSize, page, query, sort, active_status, effective_from, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && active_status === '' && effective_from === '') return items;

    let res = items.filter((r: Fee) => {
      const s = (String((r as any)?.feeTypeID ?? '') + ' ' + String((r as any)?.code ?? '') + ' ' + String((r as any)?.name ?? '') + ' ' + String((r as any)?.calculationType ?? '') + ' ' + String((r as any)?.fixedAmount ?? '') + ' ' + String((r as any)?.percentageRate ?? '') + ' ' + String((r as any)?.minimumFeeAmount ?? '') + ' ' + String((r as any)?.maximumFeeAmount ?? '') + ' ' + String((r as any)?.currency ?? '') + ' ' + String((r as any)?.effectiveFrom ?? '') + ' ' + String((r as any)?.effectiveTo ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.capturedBy ?? '') + ' ' + String((r as any)?.updatedDate ?? '') + ' ' + String((r as any)?.updatedBy ?? '')).toLowerCase();
      ...
      const match_effective_from = !effective_from || String((r as any)?.effectiveFrom ?? '') === effective_from;
      return matchesSearch && match_active_status && match_effective_from;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, active_status, effective_from]);
  ...
  useEffect(() => { if (query || sort || active_status || effective_from) setPage(1); }, [query, sort, active_status, effective_from]);
  ...
  const handleDelete = async (row: Fee) => {
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.feeID) === recordId);
      ...
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            ...
            <input className="input" placeholder="All" value={ effective_from } onChange={(e) => set_effective_from(e.target.value)} />
          </label>
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.feeID ?? Math.random()}>
                    <td>{lookup_feeTypeID[String(r?.feeTypeID ?? '')] || String(r?.feeTypeID ?? '-')}</td>
                    <td>{String(r?.code ?? '-')}</td>
                    <td>{String(r?.name ?? '-')}</td>
                    <td>{String(r?.calculationType ?? '-')}</td>
                    <td>{String(r?.fixedAmount ?? '-')}</td>
                    <td>{String(r?.percentageRate ?? '-')}</td>
                    <td>{String(r?.minimumFeeAmount ?? '-')}</td>
                    <td>{String(r?.maximumFeeAmount ?? '-')}</td>
                    <td>{String(r?.currency ?? '-')}</td>
                    <td>{String(r?.isTaxable ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.feeID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.feeID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.feeID ?? '')] || {}),
                                right: 0,
