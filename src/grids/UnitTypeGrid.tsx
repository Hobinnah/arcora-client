...
export interface UnitTypeGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: UnitType)=>void;
  onEdit?: (r: UnitType)=>void;
  onDelete?: (r: UnitType, setItems: React.Dispatch<React.SetStateAction<UnitType[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const UnitTypeGrid: React.FC<UnitTypeGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchUnitTypes(params);
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: UnitType) => {
  ...
  const handleDelete = async (row: UnitType) => {
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.unitTypeID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.unitTypeID ?? Math.random()}>
                    <td>{String(r?.name ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.unitTypeID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.unitTypeID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.unitTypeID ?? '')] || {}),
                                right: 0,
