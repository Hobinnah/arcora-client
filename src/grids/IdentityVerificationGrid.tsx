...
import { fetchIdentityVerifications } from '../apis/useIdentityVerification';
...
export interface IdentityVerificationGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: IdentityVerification)=>void;
  onEdit?: (r: IdentityVerification)=>void;
  onDelete?: (r: IdentityVerification, setItems: React.Dispatch<React.SetStateAction<IdentityVerification[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const IdentityVerificationGrid: React.FC<IdentityVerificationGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchIdentityVerifications(params);
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: IdentityVerification) => {
      const s = (String((r as any)?.verificationType ?? '') + ' ' + String((r as any)?.status ?? '') + ' ' + String((r as any)?.requestedAt ?? '') + ' ' + String((r as any)?.verifiedAt ?? '') + ' ' + String((r as any)?.expiresAt ?? '') + ' ' + String((r as any)?.failureReason ?? '')).toLowerCase();
  ...
  const handleDelete = async (row: IdentityVerification) => {
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/identityverification/updateIdentityVerification/{IdentityVerificationID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "IdentityVerificationID", type: "string", required: true, source: "record", options: [] }, { name: "status", type: "string", required: true, source: "constant", default: "ACTIVE", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.identityVerificationID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.identityVerificationID ?? Math.random()}>
                    <td>{String(r?.verificationType ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.identityVerificationID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.identityVerificationID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.identityVerificationID ?? '')] || {}),
                                right: 0,
...
export default IdentityVerificationGrid;
