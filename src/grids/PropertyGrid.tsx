...
import { fetchProperties } from '../apis/useProperty';
...
export interface PropertyGridProps {
  ...
  onView?: (r: Property)=>void;
  onEdit?: (r: Property)=>void;
  onDelete?: (r: Property, setItems: React.Dispatch<React.SetStateAction<Property[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const PropertyGrid: React.FC<PropertyGridProps> = ({
  ...
  const [property_type, set_property_type] = useState<string>('');
  ...
  const load = useCallback(async () => {
    ...
    ? [pageSize, page, query, sort, property_type, status, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && property_type === '' && status === '') return items;

    let res = items.filter((r: Property) => {
      const s = (String((r as any)?.organizationID ?? '') + ' ' + String((r as any)?.addressID ?? '') + ' ' + String((r as any)?.name ?? '') + ' ' + String((r as any)?.propertyType ?? '') + ' ' + String((r as any)?.yearBuilt ?? '') + ' ' + String((r as any)?.timeZone ?? '') + ' ' + String((r as any)?.description ?? '') + ' ' + String((r as any)?.status ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.capturedBy ?? '') + ' ' + String((r as any)?.updatedDate ?? '') + ' ' + String((r as any)?.updatedBy ?? '')).toLowerCase();
      const matchesSearch = !query || s.includes(query.toLowerCase());
      const match_property_type = !property_type || String((r as any)?.propertyType ?? '') === property_type;
      const match_status = !status || String((r as any)?.status ?? '') === status;
      return matchesSearch && match_property_type && match_status;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, property_type, status]);
  ...
  useEffect(() => { if (query || sort || property_type || status) setPage(1); }, [query, sort, property_type, status]);
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/property/updateProperty/{propertyID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "propertyID", type: "string", required: true, source: "record", options: [] }, { name: "status", type: "string", required: true, source: "constant", default: "INACTIVE", options: [] } ]
      }
    },
    {
      key: "mark-active",
      ...
      api: {
        ...
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "propertyID", type: "string", required: true, source: "record", options: [] }, { name: "status", type: "string", required: true, source: "constant", default: "ACTIVE", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.propertyID) === recordId);
      ...
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            ...
            <select className="select" value={ property_type } onChange={(e) => set_property_type(e.target.value)}>
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.propertyID ?? Math.random()}>
                    <td>{String(r?.organizationID ?? '-')}</td>
                    <td>{String(r?.addressID ?? '-')}</td>
                    <td>{String(r?.name ?? '-')}</td>
                    <td>{String(r?.propertyType ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.propertyID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.propertyID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.propertyID ?? '')] || {}),
                                right: 0,
