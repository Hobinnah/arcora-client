...
import { fetchFraudCases } from '../apis/useFraudCase';
...
export interface FraudCaseGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: FraudCase)=>void;
  onEdit?: (r: FraudCase)=>void;
  onDelete?: (r: FraudCase, setItems: React.Dispatch<React.SetStateAction<FraudCase[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const FraudCaseGrid: React.FC<FraudCaseGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    ...
    { key: "isBlocking", label: "Is Blocking" },
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchFraudCases(params);
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: FraudCase) => {
      const s = (String((r as any)?.status ?? '') + ' ' + String((r as any)?.riskScore ?? '') + ' ' + String((r as any)?.reason ?? '') + ' ' + String((r as any)?.openedAt ?? '') + ' ' + String((r as any)?.reviewedAt ?? '') + ' ' + String((r as any)?.closedAt ?? '') + ' ' + String((r as any)?.resolution ?? '')).toLowerCase();
  ...
  const handleDelete = async (row: FraudCase) => {
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/fraudcase/updateFraudCase/{FraudCaseID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "FraudCaseID", type: "string", required: true, source: "record", options: [] }, { name: "status", type: "string", required: true, source: "constant", default: "ACTIVE", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.fraudCaseID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.fraudCaseID ?? Math.random()}>
                    <td>{String(r?.status ?? '-')}</td>
                    <td>{String(r?.riskScore ?? '-')}</td>
                    <td>{String(r?.reason ?? '-')}</td>
                    <td>{String(r?.isBlocking ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.fraudCaseID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.fraudCaseID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.fraudCaseID ?? '')] || {}),
                                right: 0,
