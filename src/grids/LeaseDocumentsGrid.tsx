...
export interface LeaseDocumentsGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: LeaseDocuments)=>void;
  onEdit?: (r: LeaseDocuments)=>void;
  onDelete?: (r: LeaseDocuments, setItems: React.Dispatch<React.SetStateAction<LeaseDocuments[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const LeaseDocumentsGrid: React.FC<LeaseDocumentsGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const [items, setItems] = useState<LeaseDocuments[]>([]);
  ...
  const [document_status, set_document_status] = useState<string>('');
  ...
  const load = useCallback(async () => {
    try {
      ...
      const params: any = env.SEARCH_STRATEGY === 'server' ? {
        ...
        documentStatus: document_status || undefined,
      ...
      const result = await fetchLeaseDocuments(params);
    ...
    ? [pageSize, page, query, sort, document_status, document_type, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && document_status === '' && document_type === '') return items;

    let res = items.filter((r: LeaseDocuments) => {
      const s = (String((r as any)?.documentType ?? '') + ' ' + String((r as any)?.documentStatus ?? '') + ' ' + String((r as any)?.originalFilename ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.capturedBy ?? '') + ' ' + String((r as any)?.updatedDate ?? '') + ' ' + String((r as any)?.updatedBy ?? '')).toLowerCase();
      const matchesSearch = !query || s.includes(query.toLowerCase());
      const match_document_status = !document_status || String((r as any)?.documentStatus ?? '') === document_status;
      const match_document_type = !document_type || String((r as any)?.documentType ?? '') === document_type;
      return matchesSearch && match_document_status && match_document_type;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, document_status, document_type]);
  ...
  useEffect(() => { if (query || sort || document_status || document_type) setPage(1); }, [query, sort, document_status, document_type]);
  ...
  const handleDelete = async (row: LeaseDocuments) => {
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/leasedocuments/updateLeaseDocumentStatus/{LeaseDocumentID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "LeaseDocumentID", type: "string", required: true, source: "record", options: [] }, { name: "DocumentStatus", type: "string", required: true, source: "constant", default: "SIGNED", options: [] } ]
      }
    },
    {
      key: "mark-as-draft",
      ...
      api: {
        ...
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "LeaseDocumentID", type: "string", required: true, source: "record", options: [] }, { name: "DocumentStatus", type: "string", required: true, source: "constant", default: "DRAFT", options: [] } ]
      }
    },
  ], []);
  ...
  function isActionVisible(action: any, row: any) {
    ...
    const v = String(row?.documentStatus ?? '');
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.leaseDocumentID) === recordId);
      ...
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            ...
            <select className="select" value={ document_status } onChange={(e) => set_document_status(e.target.value)}>
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            ...
            <input className="input" placeholder="All" value={ document_type } onChange={(e) => set_document_type(e.target.value)} />
          </label>
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.leaseDocumentID ?? Math.random()}>
                    <td>{String(r?.documentType ?? '-')}</td>
                    <td>{String(r?.documentStatus ?? '-')}</td>
                    <td>{String(r?.originalFilename ?? '-')}</td>
                    <td>{String(r?.isPrimary ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.leaseDocumentID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.leaseDocumentID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.leaseDocumentID ?? '')] || {}),
                                right: 0,
...
export default LeaseDocumentsGrid;
