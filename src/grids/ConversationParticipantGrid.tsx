...
export interface ConversationParticipantGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: ConversationParticipant)=>void;
  onEdit?: (r: ConversationParticipant)=>void;
  onDelete?: (r: ConversationParticipant, setItems: React.Dispatch<React.SetStateAction<ConversationParticipant[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const ConversationParticipantGrid: React.FC<ConversationParticipantGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    { key: "participantRole", label: "Participant Role" },
    { key: "joinedAt", label: "Joined At" },
    { key: "leftAt", label: "Left At" },
    { key: "lastReadAt", label: "Last Read At" },
    { key: "isMuted", label: "Is Muted" },
  ] as const;
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchConversationParticipants(params);
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: ConversationParticipant) => {
      const s = (String((r as any)?.participantRole ?? '') + ' ' + String((r as any)?.joinedAt ?? '') + ' ' + String((r as any)?.leftAt ?? '') + ' ' + String((r as any)?.lastReadAt ?? '')).toLowerCase();
  ...
  const handleDelete = async (row: ConversationParticipant) => {
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.conversationParticipantID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.conversationParticipantID ?? Math.random()}>
                    <td>{String(r?.participantRole ?? '-')}</td>
                    <td>{r?.joinedAt ? new Date(r?.joinedAt as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.leftAt ? new Date(r?.leftAt as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.lastReadAt ? new Date(r?.lastReadAt as any).toLocaleDateString() : '-'}</td>
                    <td>{String(r?.isMuted ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.conversationParticipantID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.conversationParticipantID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.conversationParticipantID ?? '')] || {}),
                                right: 0,
...
export default ConversationParticipantGrid;
