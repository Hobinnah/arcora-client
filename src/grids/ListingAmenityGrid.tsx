...
import { fetchListingAmenities } from '../apis/useListingAmenity';
...
export interface ListingAmenityGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: ListingAmenity)=>void;
  onEdit?: (r: ListingAmenity)=>void;
  onDelete?: (r: ListingAmenity, setItems: React.Dispatch<React.SetStateAction<ListingAmenity[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const ListingAmenityGrid: React.FC<ListingAmenityGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const [items, setItems] = useState<ListingAmenity[]>([]);
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchListingAmenities(params);
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: ListingAmenity) => {
      const s = (String((r as any)?.notes ?? '') + ' ' + String((r as any)?.capturedDate ?? '')).toLowerCase();
  ...
  const handleDelete = async (row: ListingAmenity) => {
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.listingAmenityID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.listingAmenityID ?? Math.random()}>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.listingAmenityID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.listingAmenityID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.listingAmenityID ?? '')] || {}),
                                right: 0,
...
export default ListingAmenityGrid;
