...
import { fetchRentalUnits } from '../apis/useRentalUnit';
...
export interface RentalUnitGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: RentalUnit)=>void;
  onEdit?: (r: RentalUnit)=>void;
  onDelete?: (r: RentalUnit, setItems: React.Dispatch<React.SetStateAction<RentalUnit[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const RentalUnitGrid: React.FC<RentalUnitGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const [items, setItems] = useState<RentalUnit[]>([]);
  ...
  const [unit_type, set_unit_type] = useState<string>('');
  const [filterOpts_unit_type, setFilterOpts_unit_type] = useState<{label:string;value:string}[]>([]);
  useEffect(() => {
    (async () => {
      try {
        const res = await fetchData<any>('api/unittype/get?pageSize=1000&pageNumber=1', 'unit_type');
        const payload = (res as any)?.data;
        const arr = Array.isArray(payload?.data) ? payload.data : (Array.isArray(payload) ? payload : []);
        setFilterOpts_unit_type(arr.map((it:any) => ({ label: String(it?.name ?? it?.name ?? ''), value: String(it?.unitTypeID ?? it?.id ?? '') })));
      } catch { /* soft-fail */ }
    })();
  }, []);

  const [lookup_unitTypeID, setLookup_unitTypeID] = useState<Record<string, string>>({});
  useEffect(() => {
    (async () => {
      try {
        const res = await fetchData<any>('api/unittype/get?pageSize=1000&pageNumber=1', 'lookup_unitTypeID');
        const payload = (res as any)?.data;
        const arr = Array.isArray(payload?.data) ? payload.data : (Array.isArray(payload) ? payload : []);
        const map: Record<string, string> = {};
        arr.forEach((it: any) => { map[String(it?.unitTypeID ?? it?.id ?? '')] = String(it?.name ?? it?.name ?? ''); });
        setLookup_unitTypeID(map);
      } catch { /* soft-fail */ }
    })();
  }, []);
  ...
  const load = useCallback(async () => {
    try {
      ...
      const params: any = env.SEARCH_STRATEGY === 'server' ? {
        ...
        unitTypeID: unit_type || undefined,
      } : {
      ...
      const result = await fetchRentalUnits(params);
    ...
    ? [pageSize, page, query, sort, status, unit_type, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && status === '' && unit_type === '') return items;

    let res = items.filter((r: RentalUnit) => {
      const s = (String((r as any)?.unitTypeID ?? '') + ' ' + String((r as any)?.unitNumber ?? '') + ' ' + String((r as any)?.floorNumber ?? '') + ' ' + String((r as any)?.bedrooms ?? '') + ' ' + String((r as any)?.bathrooms ?? '') + ' ' + String((r as any)?.squareFeet ?? '') + ' ' + String((r as any)?.maximumOccupants ?? '') + ' ' + String((r as any)?.status ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.capturedBy ?? '') + ' ' + String((r as any)?.updatedDate ?? '') + ' ' + String((r as any)?.updatedBy ?? '')).toLowerCase();
      const matchesSearch = !query || s.includes(query.toLowerCase());
      const match_status = !status || String((r as any)?.status ?? '') === status;
      const match_unit_type = !unit_type || String((r as any)?.unitTypeID ?? '') === unit_type;
      return matchesSearch && match_status && match_unit_type;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, status, unit_type]);
  ...
  useEffect(() => { if (query || sort || status || unit_type) setPage(1); }, [query, sort, status, unit_type]);
  ...
  const handleDelete = async (row: RentalUnit) => {
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/rentalunit/updateRentalUnitStatus/{RentalUnitID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "RentalUnitID", type: "string", required: true, source: "record", options: [] }, { name: "Status", type: "string", required: true, source: "constant", default: "INACTIVE", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.rentalUnitID) === recordId);
    ...
    <section className="card container">
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            <span>Unit Type</span>
            <select className="select" value={ unit_type } onChange={(e) => set_unit_type(e.target.value)}>
              <option value="">All</option>
              {filterOpts_unit_type.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
            </select>
          </label>
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.rentalUnitID ?? Math.random()}>
                    <td>{lookup_unitTypeID[String(r?.unitTypeID ?? '')] || String(r?.unitTypeID ?? '-')}</td>
                    <td>{String(r?.unitNumber ?? '-')}</td>
                    <td>{String(r?.floorNumber ?? '-')}</td>
                    <td>{String(r?.bedrooms ?? '-')}</td>
                    <td>{String(r?.bathrooms ?? '-')}</td>
                    <td>{String(r?.squareFeet ?? '-')}</td>
                    <td>{String(r?.maximumOccupants ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.rentalUnitID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.rentalUnitID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.rentalUnitID ?? '')] || {}),
                                right: 0,
