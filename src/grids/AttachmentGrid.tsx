...
import { fetchAttachments } from '../apis/useAttachment';
...
export interface AttachmentGridProps {
  ...
  onView?: (r: Attachment)=>void;
  onEdit?: (r: Attachment)=>void;
  onDelete?: (r: Attachment, setItems: React.Dispatch<React.SetStateAction<Attachment[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const AttachmentGrid: React.FC<AttachmentGridProps> = ({
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchAttachments(params);
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: Attachment) => {
      const s = (String((r as any)?.entityType ?? '') + ' ' + String((r as any)?.entityID ?? '') + ' ' + String((r as any)?.fileName ?? '') + ' ' + String((r as any)?.mimeType ?? '') + ' ' + String((r as any)?.fileSizeBytes ?? '') + ' ' + String((r as any)?.storageProvider ?? '') + ' ' + String((r as any)?.attachmentType ?? '')).toLowerCase();
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.attachmentID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.attachmentID ?? Math.random()}>
                    <td>{String(r?.entityType ?? '-')}</td>
                    <td>{String(r?.entityID ?? '-')}</td>
                    <td>{String(r?.fileName ?? '-')}</td>
                    <td>{String(r?.mimeType ?? '-')}</td>
                    <td>{String(r?.fileSizeBytes ?? '-')}</td>
                    <td>{String(r?.storageProvider ?? '-')}</td>
                    <td>{String(r?.attachmentType ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.attachmentID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.attachmentID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.attachmentID ?? '')] || {}),
                                right: 0,
