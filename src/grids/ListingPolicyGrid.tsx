...
import { fetchListingPolicies } from '../apis/useListingPolicy';
...
export interface ListingPolicyGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: ListingPolicy)=>void;
  onEdit?: (r: ListingPolicy)=>void;
  onDelete?: (r: ListingPolicy, setItems: React.Dispatch<React.SetStateAction<ListingPolicy[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const ListingPolicyGrid: React.FC<ListingPolicyGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    ...
    { key: "allowsSmoking", label: "Allows Smoking" },
    { key: "allowsChildren", label: "Allows Children" },
    ...
    { key: "minimumCreditScore", label: "Minimum Credit Score" },
    { key: "requiresBackgroundCheck", label: "Requires Background Check" },
    { key: "applicationInstructions", label: "Application Instructions" },
    { key: "capturedDate", label: "Captured Date" },
  ] as const;

  const [items, setItems] = useState<ListingPolicy[]>([]);
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchListingPolicies(params);
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: ListingPolicy) => {
      const s = (String((r as any)?.maximumOccupants ?? '') + ' ' + String((r as any)?.minimumCreditScore ?? '') + ' ' + String((r as any)?.applicationInstructions ?? '') + ' ' + String((r as any)?.capturedDate ?? '')).toLowerCase();
  ...
  const handleDelete = async (row: ListingPolicy) => {
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.listingPolicyID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.listingPolicyID ?? Math.random()}>
                    <td>{String(r?.allowsPets ?? '-')}</td>
                    <td>{String(r?.allowsSmoking ?? '-')}</td>
                    <td>{String(r?.allowsChildren ?? '-')}</td>
                    <td>{String(r?.maximumOccupants ?? '-')}</td>
                    <td>{String(r?.furnished ?? '-')}</td>
                    <td>{String(r?.parkingIncluded ?? '-')}</td>
                    <td>{String(r?.utilitiesIncluded ?? '-')}</td>
                    <td>{String(r?.minimumCreditScore ?? '-')}</td>
                    <td>{String(r?.requiresBackgroundCheck ?? '-')}</td>
                    <td>{String(r?.applicationInstructions ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.listingPolicyID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.listingPolicyID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.listingPolicyID ?? '')] || {}),
                                right: 0,
...
export default ListingPolicyGrid;
