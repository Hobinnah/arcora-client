...
import { fetchAddresses } from '../apis/useAddress';
...
export interface AddressGridProps {
  ...
  onView?: (r: Address)=>void;
  onEdit?: (r: Address)=>void;
  onDelete?: (r: Address, setItems: React.Dispatch<React.SetStateAction<Address[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const AddressGrid: React.FC<AddressGridProps> = ({
  ...
  const columns = [
    ...
    { key: "placeProvider", label: "Place Provider" },
    { key: "placeProviderReferenceID", label: "Place Provider Reference ID" },
  ...
  const load = useCallback(async () => {
    ...
    ? [pageSize, page, query, sort, city, country, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && city === '' && country === '') return items;

    let res = items.filter((r: Address) => {
      const s = (String((r as any)?.addressType ?? '') + ' ' + String((r as any)?.line1 ?? '') + ' ' + String((r as any)?.line2 ?? '') + ' ' + String((r as any)?.city ?? '') + ' ' + String((r as any)?.provinceCode ?? '') + ' ' + String((r as any)?.postalCode ?? '') + ' ' + String((r as any)?.countryCode ?? '') + ' ' + String((r as any)?.latitude ?? '') + ' ' + String((r as any)?.longitude ?? '') + ' ' + String((r as any)?.placeProvider ?? '') + ' ' + String((r as any)?.placeProviderReferenceID ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.capturedBy ?? '') + ' ' + String((r as any)?.updatedDate ?? '') + ' ' + String((r as any)?.updatedBy ?? '')).toLowerCase();
      const matchesSearch = !query || s.includes(query.toLowerCase());
      const match_city = !city || String((r as any)?.city ?? '') === city;
      const match_country = !country || String((r as any)?.countryCode ?? '') === country;
      return matchesSearch && match_city && match_country;
    });
    if (sort) {
      const { key, dir } = sort;
  ...
  useEffect(() => { if (query || sort || city || country) setPage(1); }, [query, sort, city, country]);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.addressID) === recordId);
      ...
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            <span>City</span>
            <input className="input" placeholder="All" value={ city } onChange={(e) => set_city(e.target.value)} />
          </label>
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            <span>Country</span>
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.addressID ?? Math.random()}>
                    <td>{String(r?.addressType ?? '-')}</td>
                    <td>{String(r?.line1 ?? '-')}</td>
                    <td>{String(r?.line2 ?? '-')}</td>
                    <td>{String(r?.city ?? '-')}</td>
                    <td>{String(r?.provinceCode ?? '-')}</td>
                    <td>{String(r?.postalCode ?? '-')}</td>
                    <td>{String(r?.countryCode ?? '-')}</td>
                    <td>{String(r?.latitude ?? '-')}</td>
                    <td>{String(r?.longitude ?? '-')}</td>
                    <td>{String(r?.placeProvider ?? '-')}</td>
                    <td>{String(r?.placeProviderReferenceID ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.addressID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.addressID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.addressID ?? '')] || {}),
                                right: 0,
