...
import { fetchLeaseRecurringCharges } from '../apis/useLeaseRecurringCharges';
...
export interface LeaseRecurringChargesGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: LeaseRecurringCharges)=>void;
  onEdit?: (r: LeaseRecurringCharges)=>void;
  onDelete?: (r: LeaseRecurringCharges, setItems: React.Dispatch<React.SetStateAction<LeaseRecurringCharges[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const LeaseRecurringChargesGrid: React.FC<LeaseRecurringChargesGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    ...
    { key: "firstDueDate", label: "First Due Date" },
    { key: "lastDueDate", label: "Last Due Date" },
    { key: "autoGenerateInvoice", label: "Auto Generate Invoice" },
  ...
  const [items, setItems] = useState<LeaseRecurringCharges[]>([]);
  ...
  const [frequency, set_frequency] = useState<string>('');
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchLeaseRecurringCharges(params);
    ...
    ? [pageSize, page, query, sort, active_status, frequency, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && active_status === '' && frequency === '') return items;

    let res = items.filter((r: LeaseRecurringCharges) => {
      const s = (String((r as any)?.chargeCode ?? '') + ' ' + String((r as any)?.description ?? '') + ' ' + String((r as any)?.amount ?? '') + ' ' + String((r as any)?.currency ?? '') + ' ' + String((r as any)?.frequency ?? '') + ' ' + String((r as any)?.firstDueDate ?? '') + ' ' + String((r as any)?.lastDueDate ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.capturedBy ?? '') + ' ' + String((r as any)?.updatedDate ?? '') + ' ' + String((r as any)?.updatedBy ?? '')).toLowerCase();
      ...
      const match_frequency = !frequency || String((r as any)?.frequency ?? '') === frequency;
      return matchesSearch && match_active_status && match_frequency;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, active_status, frequency]);
  ...
  useEffect(() => { if (query || sort || active_status || frequency) setPage(1); }, [query, sort, active_status, frequency]);
  ...
  const handleDelete = async (row: LeaseRecurringCharges) => {
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.leaseRecurringChargeID) === recordId);
    ...
    <section className="card container">
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            <span>Frequency</span>
            <select className="select" value={ frequency } onChange={(e) => set_frequency(e.target.value)}>
              <option value="">All</option>
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.leaseRecurringChargeID ?? Math.random()}>
                    <td>{String(r?.chargeCode ?? '-')}</td>
                    <td>{String(r?.description ?? '-')}</td>
                    <td>{String(r?.amount ?? '-')}</td>
                    <td>{String(r?.currency ?? '-')}</td>
                    <td>{String(r?.frequency ?? '-')}</td>
                    <td>{r?.firstDueDate ? new Date(r?.firstDueDate as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.lastDueDate ? new Date(r?.lastDueDate as any).toLocaleDateString() : '-'}</td>
                    <td>{String(r?.autoGenerateInvoice ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.leaseRecurringChargeID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.leaseRecurringChargeID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.leaseRecurringChargeID ?? '')] || {}),
                                right: 0,
...
export default LeaseRecurringChargesGrid;
