...
import { fetchPaymentProviderEvents } from '../apis/usePaymentProviderEvent';
...
export interface PaymentProviderEventGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: PaymentProviderEvent)=>void;
  onEdit?: (r: PaymentProviderEvent)=>void;
  onDelete?: (r: PaymentProviderEvent, setItems: React.Dispatch<React.SetStateAction<PaymentProviderEvent[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const PaymentProviderEventGrid: React.FC<PaymentProviderEventGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const [items, setItems] = useState<PaymentProviderEvent[]>([]);
  ...
  const [processing_status, set_processing_status] = useState<string>('');
  ...
  const load = useCallback(async () => {
    try {
      ...
      const params: any = env.SEARCH_STRATEGY === 'server' ? {
        ...
        processingStatus: processing_status || undefined,
      } : {
      ...
      const result = await fetchPaymentProviderEvents(params);
    ...
    ? [pageSize, page, query, sort, processing_status, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && processing_status === '') return items;

    let res = items.filter((r: PaymentProviderEvent) => {
      const s = (String((r as any)?.providerName ?? '') + ' ' + String((r as any)?.providerEventID ?? '') + ' ' + String((r as any)?.eventType ?? '') + ' ' + String((r as any)?.processingStatus ?? '') + ' ' + String((r as any)?.receivedAt ?? '') + ' ' + String((r as any)?.capturedDate ?? '')).toLowerCase();
      const matchesSearch = !query || s.includes(query.toLowerCase());
      const match_processing_status = !processing_status || String((r as any)?.processingStatus ?? '') === processing_status;
      return matchesSearch && match_processing_status;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, processing_status]);
  ...
  useEffect(() => { if (query || sort || processing_status) setPage(1); }, [query, sort, processing_status]);
  ...
  const handleDelete = async (row: PaymentProviderEvent) => {
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/paymentproviderevent/updatePaymentProviderEvent/{PaymentProviderEventID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "PaymentProviderEventID", type: "string", required: true, source: "record", options: [] }, { name: "processingStatus", type: "string", required: true, source: "constant", default: "PROCESSED", options: [] } ]
      }
    },
  ], []);
  ...
  function isActionVisible(action: any, row: any) {
    ...
    const v = String(row?.processingStatus ?? '');
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.paymentProviderEventID) === recordId);
      ...
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            ...
            <select className="select" value={ processing_status } onChange={(e) => set_processing_status(e.target.value)}>
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.paymentProviderEventID ?? Math.random()}>
                    <td>{String(r?.providerName ?? '-')}</td>
                    <td>{String(r?.providerEventID ?? '-')}</td>
                    <td>{String(r?.eventType ?? '-')}</td>
                    <td>{String(r?.processingStatus ?? '-')}</td>
                    <td>{r?.receivedAt ? new Date(r?.receivedAt as any).toLocaleDateString() : '-'}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.paymentProviderEventID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.paymentProviderEventID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.paymentProviderEventID ?? '')] || {}),
                                right: 0,
...
export default PaymentProviderEventGrid;
