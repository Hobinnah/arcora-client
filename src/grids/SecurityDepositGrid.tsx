...
import { fetchSecurityDeposits } from '../apis/useSecurityDeposit';
...
export interface SecurityDepositGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: SecurityDeposit)=>void;
  onEdit?: (r: SecurityDeposit)=>void;
  onDelete?: (r: SecurityDeposit, setItems: React.Dispatch<React.SetStateAction<SecurityDeposit[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const SecurityDepositGrid: React.FC<SecurityDepositGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    ...
    { key: "appliedAmount", label: "Applied Amount" },
    { key: "returnedAmount", label: "Returned Amount" },
    { key: "currency", label: "Currency" },
    { key: "status", label: "Status" },
    { key: "dueDate", label: "Due Date" },
    { key: "fullyFundedAt", label: "Fully Funded At" },
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchSecurityDeposits(params);
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: SecurityDeposit) => {
      const s = (String((r as any)?.requiredAmount ?? '') + ' ' + String((r as any)?.receivedAmount ?? '') + ' ' + String((r as any)?.appliedAmount ?? '') + ' ' + String((r as any)?.returnedAmount ?? '') + ' ' + String((r as any)?.currency ?? '') + ' ' + String((r as any)?.status ?? '') + ' ' + String((r as any)?.dueDate ?? '') + ' ' + String((r as any)?.fullyFundedAt ?? '') + ' ' + String((r as any)?.heldAt ?? '') + ' ' + String((r as any)?.closedAt ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.capturedBy ?? '') + ' ' + String((r as any)?.updatedDate ?? '') + ' ' + String((r as any)?.updatedBy ?? '')).toLowerCase();
  ...
  const handleDelete = async (row: SecurityDeposit) => {
  ...
  const actions = useMemo(() => [
    {
      key: "set-partially-funded",
      label: "Set PARTIALLY_FUNDED",
      variant: "default",
      values: ["EXPECTED"],
      api: {
        method: "POST",
        pathTemplate: "api/securitydeposit/updateSecurityDeposit/{SecurityDepositID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "SecurityDepositID", type: "string", required: true, source: "record", options: [] }, { name: "status", type: "string", required: true, source: "constant", default: "PARTIALLY_FUNDED", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.securityDepositID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.securityDepositID ?? Math.random()}>
                    <td>{String(r?.requiredAmount ?? '-')}</td>
                    <td>{String(r?.receivedAmount ?? '-')}</td>
                    <td>{String(r?.appliedAmount ?? '-')}</td>
                    <td>{String(r?.returnedAmount ?? '-')}</td>
                    <td>{String(r?.currency ?? '-')}</td>
                    <td>{String(r?.status ?? '-')}</td>
                    <td>{r?.dueDate ? new Date(r?.dueDate as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.fullyFundedAt ? new Date(r?.fullyFundedAt as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.heldAt ? new Date(r?.heldAt as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.closedAt ? new Date(r?.closedAt as any).toLocaleDateString() : '-'}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.securityDepositID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.securityDepositID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.securityDepositID ?? '')] || {}),
                                right: 0,
...
export default SecurityDepositGrid;
