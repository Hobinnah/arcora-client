...
import { fetchPayouts } from '../apis/usePayout';
...
export interface PayoutGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: Payout)=>void;
  onEdit?: (r: Payout)=>void;
  onDelete?: (r: Payout, setItems: React.Dispatch<React.SetStateAction<Payout[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const PayoutGrid: React.FC<PayoutGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    ...
    { key: "providerPayoutID", label: "Provider Payout ID" },
  ] as const;
  ...
  const [requested_at, set_requested_at] = useState<string>('');
  ...
  const load = useCallback(async () => {
    try {
      ...
      const params: any = env.SEARCH_STRATEGY === 'server' ? {
        ...
        requestedAt: requested_at || undefined,
      } : {
      ...
      const result = await fetchPayouts(params);
    ...
    ? [pageSize, page, query, sort, status, requested_at, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && status === '' && requested_at === '') return items;

    let res = items.filter((r: Payout) => {
      const s = (String((r as any)?.amount ?? '') + ' ' + String((r as any)?.currency ?? '') + ' ' + String((r as any)?.status ?? '') + ' ' + String((r as any)?.scheduledAt ?? '') + ' ' + String((r as any)?.requestedAt ?? '') + ' ' + String((r as any)?.processedAt ?? '') + ' ' + String((r as any)?.paidAt ?? '') + ' ' + String((r as any)?.providerName ?? '') + ' ' + String((r as any)?.providerPayoutID ?? '')).toLowerCase();
      const matchesSearch = !query || s.includes(query.toLowerCase());
      const match_status = !status || String((r as any)?.status ?? '') === status;
      const match_requested_at = !requested_at || String((r as any)?.requestedAt ?? '') === requested_at;
      return matchesSearch && match_status && match_requested_at;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, status, requested_at]);
  ...
  useEffect(() => { if (query || sort || status || requested_at) setPage(1); }, [query, sort, status, requested_at]);
  ...
  const handleDelete = async (row: Payout) => {
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "GET",
        pathTemplate: "api/payout/UpdatePayoutStatus/{PayoutID}/{Status}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "PayoutID", type: "number", required: true, source: "record", default: "", options: [] }, { name: "Status", type: "string", required: true, source: "constant", default: "PROCESSED", options: [] } ]
      }
    },
    {
      key: "mark-paid",
      ...
      api: {
        ...
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "PayoutID", type: "number", required: true, source: "record", default: "", options: [] }, { name: "Status", type: "string", required: true, source: "constant", default: "PAID", options: [] } ]
      }
    },
    {
      key: "mark-failed",
      ...
      api: {
        ...
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "PayoutID", type: "number", required: true, source: "record", default: "", options: [] }, { name: "Status", type: "string", required: true, source: "constant", default: "FAILED", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.payoutID) === recordId);
      ...
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            ...
            <input className="input" placeholder="All" value={ requested_at } onChange={(e) => set_requested_at(e.target.value)} />
          </label>
              ...
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.payoutID ?? Math.random()}>
                    ...
                    <td>{r?.processedAt ? new Date(r?.processedAt as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.paidAt ? new Date(r?.paidAt as any).toLocaleDateString() : '-'}</td>
                    <td>{String(r?.providerName ?? '-')}</td>
                    <td>{String(r?.providerPayoutID ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.payoutID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.payoutID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.payoutID ?? '')] || {}),
                                right: 0,
