...
export interface TenantEmploymentGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: TenantEmployment)=>void;
  onEdit?: (r: TenantEmployment)=>void;
  onDelete?: (r: TenantEmployment, setItems: React.Dispatch<React.SetStateAction<TenantEmployment[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const TenantEmploymentGrid: React.FC<TenantEmploymentGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    ...
    { key: "employerPhoneNumber", label: "Employer Phone Number" },
  ...
  const [items, setItems] = useState<TenantEmployment[]>([]);
  ...
  const [verification_status, set_verification_status] = useState<string>('');
  const [current_employment, set_current_employment] = useState<string>('');
  ...
  const load = useCallback(async () => {
    try {
      ...
      const params: any = env.SEARCH_STRATEGY === 'server' ? {
        ...
        isCurrent: current_employment || undefined,
      } : {
      ...
      const result = await fetchTenantEmployments(params);
    ...
    ? [pageSize, page, query, sort, verification_status, current_employment, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && verification_status === '' && current_employment === '') return items;

    let res = items.filter((r: TenantEmployment) => {
      const s = (String((r as any)?.employerName ?? '') + ' ' + String((r as any)?.jobTitle ?? '') + ' ' + String((r as any)?.employmentType ?? '') + ' ' + String((r as any)?.employerEmail ?? '') + ' ' + String((r as any)?.employerPhoneNumber ?? '') + ' ' + String((r as any)?.annualIncome ?? '') + ' ' + String((r as any)?.currency ?? '') + ' ' + String((r as any)?.startedAt ?? '') + ' ' + String((r as any)?.endedAt ?? '') + ' ' + String((r as any)?.verificationStatus ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.capturedBy ?? '') + ' ' + String((r as any)?.updatedDate ?? '') + ' ' + String((r as any)?.updatedBy ?? '')).toLowerCase();
      const matchesSearch = !query || s.includes(query.toLowerCase());
      const match_verification_status = !verification_status || String((r as any)?.verificationStatus ?? '') === verification_status;
      const match_current_employment = !current_employment || String((r as any)?.isCurrent ?? '') === current_employment;
      return matchesSearch && match_verification_status && match_current_employment;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, verification_status, current_employment]);
  ...
  useEffect(() => { if (query || sort || verification_status || current_employment) setPage(1); }, [query, sort, verification_status, current_employment]);
  ...
  const handleDelete = async (row: TenantEmployment) => {
  ...
  const actions = useMemo(() => [
    {
      key: "verify-employment",
      label: "Verify Employment",
      variant: "default",
      values: ["NOT_VERIFIED"],
      api: {
        method: "POST",
        pathTemplate: "api/tenantemployment/updateVerificationStatus/{TenantEmploymentID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "TenantEmploymentID", type: "string", required: true, source: "record", options: [] }, { name: "VerificationStatus", type: "string", required: true, source: "constant", default: "VERIFIED", options: [] } ]
      }
    },
    {
      key: "reject-employment",
      ...
      api: {
        ...
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "TenantEmploymentID", type: "string", required: true, source: "record", options: [] }, { name: "VerificationStatus", type: "string", required: true, source: "constant", default: "REJECTED", options: [] } ]
      }
    },
  ], []);
  ...
  function isActionVisible(action: any, row: any) {
    ...
    const v = String(row?.verificationStatus ?? '');
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.tenantEmploymentID) === recordId);
      ...
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            ...
            <select className="select" value={ verification_status } onChange={(e) => set_verification_status(e.target.value)}>
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            ...
            <select className="select" value={ current_employment } onChange={(e) => set_current_employment(e.target.value)}>
              <option value="">All</option>
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.tenantEmploymentID ?? Math.random()}>
                    <td>{String(r?.employerName ?? '-')}</td>
                    <td>{String(r?.jobTitle ?? '-')}</td>
                    <td>{String(r?.employmentType ?? '-')}</td>
                    <td>{String(r?.employerEmail ?? '-')}</td>
                    <td>{String(r?.employerPhoneNumber ?? '-')}</td>
                    <td>{String(r?.annualIncome ?? '-')}</td>
                    <td>{String(r?.currency ?? '-')}</td>
                    <td>{r?.startedAt ? new Date(r?.startedAt as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.endedAt ? new Date(r?.endedAt as any).toLocaleDateString() : '-'}</td>
                    <td>{String(r?.isCurrent ?? '-')}</td>
                    <td>{String(r?.verificationStatus ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.tenantEmploymentID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.tenantEmploymentID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.tenantEmploymentID ?? '')] || {}),
                                right: 0,
...
export default TenantEmploymentGrid;
