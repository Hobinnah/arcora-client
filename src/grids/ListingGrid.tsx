...
export interface ListingGridProps {
  ...
  onView?: (r: Listing)=>void;
  onEdit?: (r: Listing)=>void;
  onDelete?: (r: Listing, setItems: React.Dispatch<React.SetStateAction<Listing[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const ListingGrid: React.FC<ListingGridProps> = ({
  ...
  const columns = [
    ...
    { key: "baseMonthlyRentAmount", label: "Base Monthly Rent" },
    { key: "securityDepositAmount", label: "Security Deposit" },
    { key: "yearBuilt", label: "Year Built" },
    { key: "status", label: "Status" },
    { key: "publishedAt", label: "Published At" },
    { key: "unpublishedAt", label: "Unpublished At" },
    { key: "currency", label: "Currency" },
    { key: "availableFrom", label: "Available From" },
    { key: "availableTo", label: "Available To" },
    { key: "minimumLeaseMonths", label: "Minimum Lease Months" },
    { key: "maximumLeaseMonths", label: "Maximum Lease Months" },
    { key: "applicationDeadline", label: "Application Deadline" },
    { key: "acceptingApplications", label: "Accepting Applications" },
  ...
  const [available_from, set_available_from] = useState<string>('');
  ...
  const load = useCallback(async () => {
    try {
      ...
      const params: any = env.SEARCH_STRATEGY === 'server' ? {
        ...
        availableFrom: available_from || undefined,
      } : {
    ...
    ? [pageSize, page, query, sort, status, available_from, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && status === '' && available_from === '') return items;

    let res = items.filter((r: Listing) => {
      const s = (String((r as any)?.title ?? '') + ' ' + String((r as any)?.bedrooms ?? '') + ' ' + String((r as any)?.bathrooms ?? '') + ' ' + String((r as any)?.squareFeet ?? '') + ' ' + String((r as any)?.baseMonthlyRentAmount ?? '') + ' ' + String((r as any)?.securityDepositAmount ?? '') + ' ' + String((r as any)?.yearBuilt ?? '') + ' ' + String((r as any)?.status ?? '') + ' ' + String((r as any)?.publishedAt ?? '') + ' ' + String((r as any)?.unpublishedAt ?? '') + ' ' + String((r as any)?.currency ?? '') + ' ' + String((r as any)?.availableFrom ?? '') + ' ' + String((r as any)?.availableTo ?? '') + ' ' + String((r as any)?.minimumLeaseMonths ?? '') + ' ' + String((r as any)?.maximumLeaseMonths ?? '') + ' ' + String((r as any)?.applicationDeadline ?? '') + ' ' + String((r as any)?.capturedBy ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.updatedDate ?? '') + ' ' + String((r as any)?.updatedBy ?? '')).toLowerCase();
      const matchesSearch = !query || s.includes(query.toLowerCase());
      const match_status = !status || String((r as any)?.status ?? '') === status;
      const match_available_from = !available_from || String((r as any)?.availableFrom ?? '') === available_from;
      return matchesSearch && match_status && match_available_from;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, status, available_from]);
  ...
  useEffect(() => { if (query || sort || status || available_from) setPage(1); }, [query, sort, status, available_from]);
  ...
  const handleDelete = async (row: Listing) => {
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/listing/updateListingStatus/{ListingID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "ListingID", type: "string", required: true, source: "record", options: [] }, { name: "Status", type: "string", required: true, source: "constant", default: "PUBLISHED", options: [] } ]
      }
    },
    {
      key: "unpublish",
      ...
      api: {
        ...
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "ListingID", type: "string", required: true, source: "record", options: [] }, { name: "Status", type: "string", required: true, source: "constant", default: "UNPUBLISHED", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.listingID) === recordId);
      ...
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            ...
            <input className="input" placeholder="All" value={ available_from } onChange={(e) => set_available_from(e.target.value)} />
          </label>
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.listingID ?? Math.random()}>
                    <td>{String(r?.title ?? '-')}</td>
                    <td>{String(r?.bedrooms ?? '-')}</td>
                    <td>{String(r?.bathrooms ?? '-')}</td>
                    <td>{String(r?.squareFeet ?? '-')}</td>
                    <td>{String(r?.baseMonthlyRentAmount ?? '-')}</td>
                    <td>{String(r?.securityDepositAmount ?? '-')}</td>
                    <td>{String(r?.yearBuilt ?? '-')}</td>
                    <td>{String(r?.status ?? '-')}</td>
                    <td>{r?.publishedAt ? new Date(r?.publishedAt as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.unpublishedAt ? new Date(r?.unpublishedAt as any).toLocaleDateString() : '-'}</td>
                    <td>{String(r?.currency ?? '-')}</td>
                    <td>{r?.availableFrom ? new Date(r?.availableFrom as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.availableTo ? new Date(r?.availableTo as any).toLocaleDateString() : '-'}</td>
                    <td>{String(r?.minimumLeaseMonths ?? '-')}</td>
                    <td>{String(r?.maximumLeaseMonths ?? '-')}</td>
                    <td>{r?.applicationDeadline ? new Date(r?.applicationDeadline as any).toLocaleDateString() : '-'}</td>
                    <td>{String(r?.acceptingApplications ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.listingID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.listingID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.listingID ?? '')] || {}),
                                right: 0,
