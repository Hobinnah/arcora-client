...
import { fetchTenancyTypes } from '../apis/useTenancyType';
...
export interface TenancyTypeGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: TenancyType)=>void;
  onEdit?: (r: TenancyType)=>void;
  onDelete?: (r: TenancyType, setItems: React.Dispatch<React.SetStateAction<TenancyType[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const TenancyTypeGrid: React.FC<TenancyTypeGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    { key: "code", label: "Code" },
    { key: "name", label: "Name" },
    { key: "minimumMonths", label: "Minimum Months" },
    { key: "maximumMonths", label: "Maximum Months" },
    { key: "isPeriodic", label: "Is Periodic" },
    { key: "isActive", label: "Active Status" },
    { key: "capturedDate", label: "Captured Date" },
    { key: "capturedBy", label: "Captured By" },
  ] as const;

  const [items, setItems] = useState<TenancyType[]>([]);
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchTenancyTypes(params);
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: TenancyType) => {
      const s = (String((r as any)?.code ?? '') + ' ' + String((r as any)?.name ?? '') + ' ' + String((r as any)?.minimumMonths ?? '') + ' ' + String((r as any)?.maximumMonths ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.capturedBy ?? '')).toLowerCase();
  ...
  const handleDelete = async (row: TenancyType) => {
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.tenancyTypeID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.tenancyTypeID ?? Math.random()}>
                    <td>{String(r?.code ?? '-')}</td>
                    <td>{String(r?.name ?? '-')}</td>
                    <td>{String(r?.minimumMonths ?? '-')}</td>
                    <td>{String(r?.maximumMonths ?? '-')}</td>
                    <td>{String(r?.isPeriodic ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.tenancyTypeID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.tenancyTypeID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.tenancyTypeID ?? '')] || {}),
                                right: 0,
...
export default TenancyTypeGrid;
