...
import { fetchMaintenanceRequests } from '../apis/useMaintenanceRequest';
...
export interface MaintenanceRequestGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: MaintenanceRequest)=>void;
  onEdit?: (r: MaintenanceRequest)=>void;
  onDelete?: (r: MaintenanceRequest, setItems: React.Dispatch<React.SetStateAction<MaintenanceRequest[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const MaintenanceRequestGrid: React.FC<MaintenanceRequestGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    ...
    { key: "permissionToEnter", label: "Permission To Enter" },
  ...
  const [priority, set_priority] = useState<string>('');
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchMaintenanceRequests(params);
    ...
    ? [pageSize, page, query, sort, status, priority, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && status === '' && priority === '') return items;

    let res = items.filter((r: MaintenanceRequest) => {
      const s = (String((r as any)?.priority ?? '') + ' ' + String((r as any)?.status ?? '') + ' ' + String((r as any)?.title ?? '') + ' ' + String((r as any)?.submittedAt ?? '') + ' ' + String((r as any)?.acknowledgedAt ?? '') + ' ' + String((r as any)?.scheduledAt ?? '') + ' ' + String((r as any)?.completedAt ?? '') + ' ' + String((r as any)?.cancelledAt ?? '')).toLowerCase();
      const matchesSearch = !query || s.includes(query.toLowerCase());
      const match_status = !status || String((r as any)?.status ?? '') === status;
      const match_priority = !priority || String((r as any)?.priority ?? '') === priority;
      return matchesSearch && match_status && match_priority;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, status, priority]);
  ...
  useEffect(() => { if (query || sort || status || priority) setPage(1); }, [query, sort, status, priority]);
  ...
  const handleDelete = async (row: MaintenanceRequest) => {
  ...
  const actions = useMemo(() => [
    {
      key: "set-acknowledged",
      label: "Set ACKNOWLEDGED",
      variant: "default",
      values: ["OPEN"],
      api: {
        method: "POST",
        pathTemplate: "api/maintenancerequest/updateMaintenanceRequest/{MaintenanceRequestID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "MaintenanceRequestID", type: "string", required: true, source: "record", options: [] }, { name: "status", type: "string", required: true, source: "constant", default: "ACKNOWLEDGED", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.maintenanceRequestID) === recordId);
    ...
    <section className="card container">
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            <span>Priority</span>
            <select className="select" value={ priority } onChange={(e) => set_priority(e.target.value)}>
              <option value="">All</option>
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.maintenanceRequestID ?? Math.random()}>
                    <td>{String(r?.priority ?? '-')}</td>
                    <td>{String(r?.status ?? '-')}</td>
                    <td>{String(r?.title ?? '-')}</td>
                    <td>{String(r?.permissionToEnter ?? '-')}</td>
                    <td>{r?.submittedAt ? new Date(r?.submittedAt as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.acknowledgedAt ? new Date(r?.acknowledgedAt as any).toLocaleDateString() : '-'}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.maintenanceRequestID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.maintenanceRequestID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.maintenanceRequestID ?? '')] || {}),
                                right: 0,
