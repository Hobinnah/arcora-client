...
import { fetchAuditLogs } from '../apis/useAuditLog';
...
export interface AuditLogGridProps {
  ...
  onView?: (r: AuditLog)=>void;
  onEdit?: (r: AuditLog)=>void;
  onDelete?: (r: AuditLog, setItems: React.Dispatch<React.SetStateAction<AuditLog[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const AuditLogGrid: React.FC<AuditLogGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: AuditLog) => {
      const s = (String((r as any)?.actorType ?? '') + ' ' + String((r as any)?.action ?? '') + ' ' + String((r as any)?.entityType ?? '') + ' ' + String((r as any)?.entityID ?? '') + ' ' + String((r as any)?.ipAddress ?? '') + ' ' + String((r as any)?.userAgent ?? '') + ' ' + String((r as any)?.correlationID ?? '') + ' ' + String((r as any)?.capturedDate ?? '')).toLowerCase();
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.auditLogID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.auditLogID ?? Math.random()}>
                    <td>{String(r?.actorType ?? '-')}</td>
                    <td>{String(r?.action ?? '-')}</td>
                    <td>{String(r?.entityType ?? '-')}</td>
                    <td>{String(r?.entityID ?? '-')}</td>
                    <td>{String(r?.ipAddress ?? '-')}</td>
                    <td>{String(r?.userAgent ?? '-')}</td>
                    <td>{String(r?.correlationID ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.auditLogID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.auditLogID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.auditLogID ?? '')] || {}),
                                right: 0,
