...
export interface LeaseGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: Lease)=>void;
  onEdit?: (r: Lease)=>void;
  onDelete?: (r: Lease, setItems: React.Dispatch<React.SetStateAction<Lease[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const LeaseGrid: React.FC<LeaseGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    ...
    { key: "rentalUnitID", label: "Rental Unit" },
    { key: "tenancyTypeID", label: "Tenancy Type" },
    { key: "tenantID", label: "Tenant" },
    { key: "rentalApplicationID", label: "Rental Application" },
    { key: "leaseCode", label: "Lease Code" },
    { key: "leaseNumber", label: "Lease Number" },
    { key: "status", label: "Status" },
    { key: "startDate", label: "Start Date" },
    { key: "endDate", label: "End Date" },
    { key: "leaseTermMonths", label: "Lease Term Months" },
    { key: "baseRentAmount", label: "Base Rent Amount" },
    { key: "currency", label: "Currency" },
    { key: "gracePeriodDays", label: "Grace Period Days" },
    { key: "lateFeeFixedAmount", label: "Late Fee Fixed Amount" },
    { key: "lateFeePercentage", label: "Late Fee Percentage" },
    { key: "autoRenew", label: "Auto Renew" },
    { key: "renewalNoticeDays", label: "Renewal Notice Days" },
    { key: "signedAt", label: "Signed At" },
    { key: "activatedAt", label: "Activated At" },
    { key: "actualMoveInAt", label: "Actual Move In At" },
    { key: "actualMoveOutAt", label: "Actual Move Out At" },
  ...
  const [lookup_organizationID, setLookup_organizationID] = useState<Record<string, string>>({});
  useEffect(() => {
    (async () => {
      try {
        const res = await fetchData<any>('api/organization/get?pageSize=1000&pageNumber=1', 'lookup_organizationID');
        const payload = (res as any)?.data;
        const arr = Array.isArray(payload?.data) ? payload.data : (Array.isArray(payload) ? payload : []);
        const map: Record<string, string> = {};
        arr.forEach((it: any) => { map[String(it?.organizationID ?? it?.id ?? '')] = String(it?.displayName ?? it?.name ?? ''); });
        setLookup_organizationID(map);
      } catch { /* soft-fail */ }
    })();
  }, []);
  const [lookup_listingID, setLookup_listingID] = useState<Record<string, string>>({});
  useEffect(() => {
    (async () => {
      try {
        const res = await fetchData<any>('api/listing/get?pageSize=1000&pageNumber=1', 'lookup_listingID');
        const payload = (res as any)?.data;
        const arr = Array.isArray(payload?.data) ? payload.data : (Array.isArray(payload) ? payload : []);
        const map: Record<string, string> = {};
        arr.forEach((it: any) => { map[String(it?.listingID ?? it?.id ?? '')] = String(it?.title ?? it?.name ?? ''); });
        setLookup_listingID(map);
      } catch { /* soft-fail */ }
    })();
  }, []);
  const [lookup_rentalUnitID, setLookup_rentalUnitID] = useState<Record<string, string>>({});
  useEffect(() => {
    (async () => {
      try {
        const res = await fetchData<any>('api/rentalunit/get?pageSize=1000&pageNumber=1', 'lookup_rentalUnitID');
        const payload = (res as any)?.data;
        const arr = Array.isArray(payload?.data) ? payload.data : (Array.isArray(payload) ? payload : []);
        const map: Record<string, string> = {};
        arr.forEach((it: any) => { map[String(it?.rentalUnitID ?? it?.id ?? '')] = String(it?.unitNumber ?? it?.name ?? ''); });
        setLookup_rentalUnitID(map);
      } catch { /* soft-fail */ }
    })();
  }, []);
  const [lookup_tenantID, setLookup_tenantID] = useState<Record<string, string>>({});
  useEffect(() => {
    (async () => {
      try {
        const res = await fetchData<any>('api/tenant/get?pageSize=1000&pageNumber=1', 'lookup_tenantID');
        const payload = (res as any)?.data;
        const arr = Array.isArray(payload?.data) ? payload.data : (Array.isArray(payload) ? payload : []);
        const map: Record<string, string> = {};
        arr.forEach((it: any) => { map[String(it?.tenantID ?? it?.id ?? '')] = String(it?.lastName ?? it?.name ?? ''); });
        setLookup_tenantID(map);
      } catch { /* soft-fail */ }
    })();
  }, []);
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchLeases(params);
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: Lease) => {
      const s = (String((r as any)?.organizationID ?? '') + ' ' + String((r as any)?.listingID ?? '') + ' ' + String((r as any)?.rentalUnitID ?? '') + ' ' + String((r as any)?.tenancyTypeID ?? '') + ' ' + String((r as any)?.tenantID ?? '') + ' ' + String((r as any)?.rentalApplicationID ?? '') + ' ' + String((r as any)?.leaseCode ?? '') + ' ' + String((r as any)?.leaseNumber ?? '') + ' ' + String((r as any)?.status ?? '') + ' ' + String((r as any)?.startDate ?? '') + ' ' + String((r as any)?.endDate ?? '') + ' ' + String((r as any)?.leaseTermMonths ?? '') + ' ' + String((r as any)?.baseRentAmount ?? '') + ' ' + String((r as any)?.currency ?? '') + ' ' + String((r as any)?.gracePeriodDays ?? '') + ' ' + String((r as any)?.lateFeeFixedAmount ?? '') + ' ' + String((r as any)?.lateFeePercentage ?? '') + ' ' + String((r as any)?.renewalNoticeDays ?? '') + ' ' + String((r as any)?.signedAt ?? '') + ' ' + String((r as any)?.activatedAt ?? '') + ' ' + String((r as any)?.actualMoveInAt ?? '') + ' ' + String((r as any)?.actualMoveOutAt ?? '') + ' ' + String((r as any)?.terminatedAt ?? '') + ' ' + String((r as any)?.terminationReason ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.capturedBy ?? '') + ' ' + String((r as any)?.updatedDate ?? '') + ' ' + String((r as any)?.updatedBy ?? '')).toLowerCase();
  ...
  const handleDelete = async (row: Lease) => {
  ...
  const actions = useMemo(() => [
    {
      key: "set-pending-signature",
      label: "Set PENDING_SIGNATURE",
      variant: "default",
      values: ["DRAFT"],
      api: {
        method: "POST",
        pathTemplate: "api/lease/updateLease/{LeaseID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "LeaseID", type: "string", required: true, source: "record", options: [] }, { name: "status", type: "string", required: true, source: "constant", default: "PENDING_SIGNATURE", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.leaseID) === recordId);
    ...
    <section className="card container">
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            <span>Currency</span>
            <input className="input" placeholder="All" value={ currency } onChange={(e) => set_currency(e.target.value)} />
          </label>
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.leaseID ?? Math.random()}>
                    <td>{lookup_organizationID[String(r?.organizationID ?? '')] || String(r?.organizationID ?? '-')}</td>
                    <td>{lookup_listingID[String(r?.listingID ?? '')] || String(r?.listingID ?? '-')}</td>
                    <td>{lookup_rentalUnitID[String(r?.rentalUnitID ?? '')] || String(r?.rentalUnitID ?? '-')}</td>
                    <td>{String(r?.tenancyTypeID ?? '-')}</td>
                    <td>{lookup_tenantID[String(r?.tenantID ?? '')] || String(r?.tenantID ?? '-')}</td>
                    <td>{String(r?.rentalApplicationID ?? '-')}</td>
                    <td>{String(r?.leaseCode ?? '-')}</td>
                    <td>{String(r?.leaseNumber ?? '-')}</td>
                    <td>{String(r?.status ?? '-')}</td>
                    <td>{r?.startDate ? new Date(r?.startDate as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.endDate ? new Date(r?.endDate as any).toLocaleDateString() : '-'}</td>
                    <td>{String(r?.leaseTermMonths ?? '-')}</td>
                    <td>{String(r?.baseRentAmount ?? '-')}</td>
                    <td>{String(r?.currency ?? '-')}</td>
                    <td>{String(r?.gracePeriodDays ?? '-')}</td>
                    <td>{String(r?.lateFeeFixedAmount ?? '-')}</td>
                    <td>{String(r?.lateFeePercentage ?? '-')}</td>
                    <td>{String(r?.autoRenew ?? '-')}</td>
                    <td>{String(r?.renewalNoticeDays ?? '-')}</td>
                    <td>{r?.signedAt ? new Date(r?.signedAt as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.activatedAt ? new Date(r?.activatedAt as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.actualMoveInAt ? new Date(r?.actualMoveInAt as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.actualMoveOutAt ? new Date(r?.actualMoveOutAt as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.terminatedAt ? new Date(r?.terminatedAt as any).toLocaleDateString() : '-'}</td>
                    <td>{String(r?.terminationReason ?? '-')}</td>
                    <td>{r?.capturedDate ? new Date(r?.capturedDate as any).toLocaleDateString() : '-'}</td>
                    <td>{String(r?.capturedBy ?? '-')}</td>
                    <td>{r?.updatedDate ? new Date(r?.updatedDate as any).toLocaleDateString() : '-'}</td>
                    <td>{String(r?.updatedBy ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.leaseID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.leaseID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.leaseID ?? '')] || {}),
                                right: 0,
