...
import { fetchPaymentReminders } from '../apis/usePaymentReminder';
...
export interface PaymentReminderGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: PaymentReminder)=>void;
  onEdit?: (r: PaymentReminder)=>void;
  onDelete?: (r: PaymentReminder, setItems: React.Dispatch<React.SetStateAction<PaymentReminder[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const PaymentReminderGrid: React.FC<PaymentReminderGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const [items, setItems] = useState<PaymentReminder[]>([]);
  ...
  const [notification_channel, set_notification_channel] = useState<string>('');
  ...
  const load = useCallback(async () => {
    try {
      ...
      const params: any = env.SEARCH_STRATEGY === 'server' ? {
        ...
        channel: notification_channel || undefined,
      } : {
      ...
      const result = await fetchPaymentReminders(params);
    ...
    ? [pageSize, page, query, sort, status, notification_channel, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && status === '' && notification_channel === '') return items;

    let res = items.filter((r: PaymentReminder) => {
      const s = (String((r as any)?.channel ?? '') + ' ' + String((r as any)?.scheduledAt ?? '') + ' ' + String((r as any)?.sentAt ?? '') + ' ' + String((r as any)?.status ?? '') + ' ' + String((r as any)?.messageSubject ?? '')).toLowerCase();
      const matchesSearch = !query || s.includes(query.toLowerCase());
      const match_status = !status || String((r as any)?.status ?? '') === status;
      const match_notification_channel = !notification_channel || String((r as any)?.channel ?? '') === notification_channel;
      return matchesSearch && match_status && match_notification_channel;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, status, notification_channel]);
  ...
  useEffect(() => { if (query || sort || status || notification_channel) setPage(1); }, [query, sort, status, notification_channel]);
  ...
  const handleDelete = async (row: PaymentReminder) => {
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/paymentreminder/updatePaymentReminder/{PaymentReminderID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "PaymentReminderID", type: "string", required: true, source: "record", options: [] }, { name: "status", type: "string", required: true, source: "constant", default: "SENT", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.paymentReminderID) === recordId);
      ...
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            ...
            <input className="input" placeholder="All" value={ notification_channel } onChange={(e) => set_notification_channel(e.target.value)} />
          </label>
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.paymentReminderID ?? Math.random()}>
                    <td>{String(r?.channel ?? '-')}</td>
                    <td>{r?.scheduledAt ? new Date(r?.scheduledAt as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.sentAt ? new Date(r?.sentAt as any).toLocaleDateString() : '-'}</td>
                    <td>{String(r?.status ?? '-')}</td>
                    <td>{String(r?.messageSubject ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.paymentReminderID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.paymentReminderID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.paymentReminderID ?? '')] || {}),
                                right: 0,
...
export default PaymentReminderGrid;
