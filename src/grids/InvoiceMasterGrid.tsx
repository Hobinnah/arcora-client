...
import { fetchInvoiceMasters } from '../apis/useInvoiceMaster';
...
export interface InvoiceMasterGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: InvoiceMaster)=>void;
  onEdit?: (r: InvoiceMaster)=>void;
  onDelete?: (r: InvoiceMaster, setItems: React.Dispatch<React.SetStateAction<InvoiceMaster[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const InvoiceMasterGrid: React.FC<InvoiceMasterGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    ...
    { key: "subtotalAmount", label: "Subtotal Amount" },
  ...
  const [items, setItems] = useState<InvoiceMaster[]>([]);
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchInvoiceMasters(params);
    ...
    ? [pageSize, page, query, sort, status, due_date, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && status === '' && due_date === '') return items;

    let res = items.filter((r: InvoiceMaster) => {
      const s = (String((r as any)?.invoiceNumber ?? '') + ' ' + String((r as any)?.billingPeriodStart ?? '') + ' ' + String((r as any)?.billingPeriodEnd ?? '') + ' ' + String((r as any)?.dueDate ?? '') + ' ' + String((r as any)?.subtotalAmount ?? '') + ' ' + String((r as any)?.taxAmount ?? '') + ' ' + String((r as any)?.discountAmount ?? '') + ' ' + String((r as any)?.lateFeeAmount ?? '') + ' ' + String((r as any)?.adjustmentAmount ?? '') + ' ' + String((r as any)?.totalAmount ?? '') + ' ' + String((r as any)?.amountPaid ?? '') + ' ' + String((r as any)?.balanceDue ?? '') + ' ' + String((r as any)?.currency ?? '') + ' ' + String((r as any)?.status ?? '') + ' ' + String((r as any)?.issuedAt ?? '') + ' ' + String((r as any)?.paidAt ?? '') + ' ' + String((r as any)?.voidedAt ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.capturedBy ?? '') + ' ' + String((r as any)?.updatedDate ?? '') + ' ' + String((r as any)?.updatedBy ?? '')).toLowerCase();
      const matchesSearch = !query || s.includes(query.toLowerCase());
      const match_status = !status || String((r as any)?.status ?? '') === status;
      const match_due_date = !due_date || String((r as any)?.dueDate ?? '') === due_date;
      return matchesSearch && match_status && match_due_date;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, status, due_date]);
  ...
  useEffect(() => { if (query || sort || status || due_date) setPage(1); }, [query, sort, status, due_date]);
  ...
  const handleDelete = async (row: InvoiceMaster) => {
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/invoicemaster/updateInvoiceMasterStatus/{InvoiceMasterID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "InvoiceMasterID", type: "string", required: true, source: "record", options: [] }, { name: "Status", type: "string", required: true, source: "constant", default: "PAID", options: [] } ]
      }
    },
    {
      key: "void-invoice",
      ...
      api: {
        method: "POST",
        pathTemplate: "api/invoicemaster/voidInvoice/{InvoiceMasterID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "InvoiceMasterID", type: "string", required: true, source: "record", options: [] }, { name: "Status", type: "string", required: true, source: "constant", default: "VOID", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.invoiceMasterID) === recordId);
    ...
    <section className="card container">
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            <span>Due Date</span>
            <input className="input" placeholder="All" value={ due_date } onChange={(e) => set_due_date(e.target.value)} />
          </label>
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.invoiceMasterID ?? Math.random()}>
                    <td>{String(r?.invoiceNumber ?? '-')}</td>
                    <td>{r?.billingPeriodStart ? new Date(r?.billingPeriodStart as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.billingPeriodEnd ? new Date(r?.billingPeriodEnd as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.dueDate ? new Date(r?.dueDate as any).toLocaleDateString() : '-'}</td>
                    <td>{String(r?.subtotalAmount ?? '-')}</td>
                    <td>{String(r?.taxAmount ?? '-')}</td>
                    <td>{String(r?.discountAmount ?? '-')}</td>
                    <td>{String(r?.lateFeeAmount ?? '-')}</td>
                    <td>{String(r?.adjustmentAmount ?? '-')}</td>
                    <td>{String(r?.totalAmount ?? '-')}</td>
                    <td>{String(r?.amountPaid ?? '-')}</td>
                    <td>{String(r?.balanceDue ?? '-')}</td>
                    <td>{String(r?.currency ?? '-')}</td>
                    <td>{String(r?.status ?? '-')}</td>
                    <td>{r?.issuedAt ? new Date(r?.issuedAt as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.paidAt ? new Date(r?.paidAt as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.voidedAt ? new Date(r?.voidedAt as any).toLocaleDateString() : '-'}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.invoiceMasterID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.invoiceMasterID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.invoiceMasterID ?? '')] || {}),
                                right: 0,
...
export default InvoiceMasterGrid;
