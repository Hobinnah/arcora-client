...
import { fetchNotifications } from '../apis/useNotification';
...
export interface NotificationGridProps {
  ...
  onView?: (r: Notification)=>void;
  onEdit?: (r: Notification)=>void;
  onDelete?: (r: Notification, setItems: React.Dispatch<React.SetStateAction<Notification[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const NotificationGrid: React.FC<NotificationGridProps> = ({
  ...
  const [channel, set_channel] = useState<string>('');
  ...
  const load = useCallback(async () => {
    ...
    ? [pageSize, page, query, sort, status, channel, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && status === '' && channel === '') return items;

    let res = items.filter((r: Notification) => {
      const s = (String((r as any)?.recipientEmail ?? '') + ' ' + String((r as any)?.channel ?? '') + ' ' + String((r as any)?.subject ?? '') + ' ' + String((r as any)?.status ?? '') + ' ' + String((r as any)?.scheduledAt ?? '') + ' ' + String((r as any)?.sentAt ?? '') + ' ' + String((r as any)?.deliveredAt ?? '') + ' ' + String((r as any)?.readAt ?? '')).toLowerCase();
      const matchesSearch = !query || s.includes(query.toLowerCase());
      const match_status = !status || String((r as any)?.status ?? '') === status;
      const match_channel = !channel || String((r as any)?.channel ?? '') === channel;
      return matchesSearch && match_status && match_channel;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, status, channel]);
  ...
  useEffect(() => { if (query || sort || status || channel) setPage(1); }, [query, sort, status, channel]);
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/notification/updateNotification/{NotificationID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "NotificationID", type: "string", required: true, source: "record", options: [] }, { name: "status", type: "string", required: true, source: "constant", default: "ACTIVE", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.notificationID) === recordId);
    ...
    <section className="card container">
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            <span>Channel</span>
            <input className="input" placeholder="All" value={ channel } onChange={(e) => set_channel(e.target.value)} />
          </label>
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.notificationID ?? Math.random()}>
                    <td>{String(r?.recipientEmail ?? '-')}</td>
                    ...
                    <td>{r?.deliveredAt ? new Date(r?.deliveredAt as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.readAt ? new Date(r?.readAt as any).toLocaleDateString() : '-'}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.notificationID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.notificationID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.notificationID ?? '')] || {}),
                                right: 0,
