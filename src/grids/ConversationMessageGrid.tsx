...
import { fetchConversationMessages } from '../apis/useConversationMessage';
...
export interface ConversationMessageGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: ConversationMessage)=>void;
  onEdit?: (r: ConversationMessage)=>void;
  onDelete?: (r: ConversationMessage, setItems: React.Dispatch<React.SetStateAction<ConversationMessage[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const ConversationMessageGrid: React.FC<ConversationMessageGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchConversationMessages(params);
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: ConversationMessage) => {
      const s = (String((r as any)?.message ?? '') + ' ' + String((r as any)?.sentAt ?? '') + ' ' + String((r as any)?.editedAt ?? '')).toLowerCase();
  ...
  const handleDelete = async (row: ConversationMessage) => {
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.conversationMessageID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.conversationMessageID ?? Math.random()}>
                    <td>{String(r?.message ?? '-')}</td>
                    <td>{r?.sentAt ? new Date(r?.sentAt as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.editedAt ? new Date(r?.editedAt as any).toLocaleDateString() : '-'}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.conversationMessageID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.conversationMessageID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.conversationMessageID ?? '')] || {}),
                                right: 0,
...
export default ConversationMessageGrid;
