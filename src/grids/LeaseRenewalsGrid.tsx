...
import { fetchLeaseRenewals } from '../apis/useLeaseRenewals';
...
export interface LeaseRenewalsGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: LeaseRenewals)=>void;
  onEdit?: (r: LeaseRenewals)=>void;
  onDelete?: (r: LeaseRenewals, setItems: React.Dispatch<React.SetStateAction<LeaseRenewals[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const LeaseRenewalsGrid: React.FC<LeaseRenewalsGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    ...
    { key: "securityDepositAdjustmentAmount", label: "Security Deposit Adjustment Amount" },
  ...
  const [items, setItems] = useState<LeaseRenewals[]>([]);
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchLeaseRenewals(params);
    ...
    ? [pageSize, page, query, sort, status, start_date, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && status === '' && start_date === '') return items;

    let res = items.filter((r: LeaseRenewals) => {
      const s = (String((r as any)?.status ?? '') + ' ' + String((r as any)?.startDate ?? '') + ' ' + String((r as any)?.endDate ?? '') + ' ' + String((r as any)?.leaseTermMonths ?? '') + ' ' + String((r as any)?.rentAmount ?? '') + ' ' + String((r as any)?.securityDepositAdjustmentAmount ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.capturedBy ?? '') + ' ' + String((r as any)?.updatedDate ?? '') + ' ' + String((r as any)?.updatedBy ?? '')).toLowerCase();
      const matchesSearch = !query || s.includes(query.toLowerCase());
      const match_status = !status || String((r as any)?.status ?? '') === status;
      const match_start_date = !start_date || String((r as any)?.startDate ?? '') === start_date;
      return matchesSearch && match_status && match_start_date;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, status, start_date]);
  ...
  useEffect(() => { if (query || sort || status || start_date) setPage(1); }, [query, sort, status, start_date]);
  ...
  const handleDelete = async (row: LeaseRenewals) => {
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/leaserenewals/updateLeaseRenewalStatus/{LeaseRenewalID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "LeaseRenewalID", type: "string", required: true, source: "record", options: [] }, { name: "Status", type: "string", required: true, source: "constant", default: "ACCEPTED", options: [] } ]
      }
    },
    {
      key: "mark-as-declined",
      ...
      api: {
        ...
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "LeaseRenewalID", type: "string", required: true, source: "record", options: [] }, { name: "Status", type: "string", required: true, source: "constant", default: "DECLINED", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.leaseRenewalID) === recordId);
      ...
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            ...
            <input className="input" placeholder="All" value={ start_date } onChange={(e) => set_start_date(e.target.value)} />
          </label>
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.leaseRenewalID ?? Math.random()}>
                    ...
                    <td>{String(r?.rentAmount ?? '-')}</td>
                    <td>{String(r?.securityDepositAdjustmentAmount ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.leaseRenewalID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.leaseRenewalID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.leaseRenewalID ?? '')] || {}),
                                right: 0,
...
export default LeaseRenewalsGrid;
