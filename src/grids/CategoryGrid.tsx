...
import { fetchCategories } from '../apis/useCategory';
...
export interface CategoryGridProps {
  ...
  onDelete?: (r: Category, setItems: React.Dispatch<React.SetStateAction<Category[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const CategoryGrid: React.FC<CategoryGridProps> = ({
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: Category) => {
      const s = (String((r as any)?.name ?? '')).toLowerCase();
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.categoryID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.categoryID ?? Math.random()}>
                    <td>{String(r?.name ?? '-')}</td>
                    <td>{String(r?.maintenance ?? '-')}</td>
                    <td>{String(r?.contractor ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.categoryID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.categoryID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.categoryID ?? '')] || {}),
                                right: 0,
