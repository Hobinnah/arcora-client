...
export interface TenantGuarantorGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: TenantGuarantor)=>void;
  onEdit?: (r: TenantGuarantor)=>void;
  onDelete?: (r: TenantGuarantor, setItems: React.Dispatch<React.SetStateAction<TenantGuarantor[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const TenantGuarantorGrid: React.FC<TenantGuarantorGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const [items, setItems] = useState<TenantGuarantor[]>([]);
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchTenantGuarantors(params);
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: TenantGuarantor) => {
      const s = (String((r as any)?.firstName ?? '') + ' ' + String((r as any)?.lastName ?? '') + ' ' + String((r as any)?.email ?? '') + ' ' + String((r as any)?.phoneNumber ?? '') + ' ' + String((r as any)?.relationship ?? '') + ' ' + String((r as any)?.annualIncome ?? '') + ' ' + String((r as any)?.status ?? '') + ' ' + String((r as any)?.invitedAt ?? '') + ' ' + String((r as any)?.acceptedAt ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.capturedBy ?? '') + ' ' + String((r as any)?.updatedDate ?? '') + ' ' + String((r as any)?.updatedBy ?? '')).toLowerCase();
  ...
  const handleDelete = async (row: TenantGuarantor) => {
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/tenantguarantor/updatetenantguarantorstatus/{TenantGuarantorID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "TenantGuarantorID", type: "string", required: true, source: "record", options: [] }, { name: "Status", type: "string", required: true, source: "constant", default: "APPROVED", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.tenantGuarantorID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.tenantGuarantorID ?? Math.random()}>
                    ...
                    <td>{r?.invitedAt ? new Date(r?.invitedAt as any).toLocaleDateString() : '-'}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.tenantGuarantorID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.tenantGuarantorID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.tenantGuarantorID ?? '')] || {}),
                                right: 0,
...
export default TenantGuarantorGrid;
