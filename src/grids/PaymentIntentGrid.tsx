...
import { fetchPaymentIntents } from '../apis/usePaymentIntent';
...
export interface PaymentIntentGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: PaymentIntent)=>void;
  onEdit?: (r: PaymentIntent)=>void;
  onDelete?: (r: PaymentIntent, setItems: React.Dispatch<React.SetStateAction<PaymentIntent[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const PaymentIntentGrid: React.FC<PaymentIntentGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    ...
    { key: "providerPaymentIntentID", label: "Provider Payment Intent ID" },
    { key: "scheduledChargeAt", label: "Scheduled Charge At" },
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        arr.forEach((it: any) => { map[String(it?.tenantID ?? it?.id ?? '')] = String(it?.fullName ?? it?.name ?? ''); });
        setLookup_tenantID(map);
      } catch { /* soft-fail */ }
    })();
  }, []);
  const [lookup_paymentMethodID, setLookup_paymentMethodID] = useState<Record<string, string>>({});
  useEffect(() => {
    (async () => {
      try {
        const res = await fetchData<any>('api/paymentmethod/get?pageSize=1000&pageNumber=1', 'lookup_paymentMethodID');
        const payload = (res as any)?.data;
        const arr = Array.isArray(payload?.data) ? payload.data : (Array.isArray(payload) ? payload : []);
        const map: Record<string, string> = {};
        arr.forEach((it: any) => { map[String(it?.paymentMethodID ?? it?.id ?? '')] = String(it?.methodName ?? it?.name ?? ''); });
        setLookup_paymentMethodID(map);
      } catch { /* soft-fail */ }
    })();
  }, []);
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchPaymentIntents(params);
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: PaymentIntent) => {
      const s = (String((r as any)?.tenantID ?? '') + ' ' + String((r as any)?.paymentMethodID ?? '') + ' ' + String((r as any)?.amount ?? '') + ' ' + String((r as any)?.currency ?? '') + ' ' + String((r as any)?.status ?? '') + ' ' + String((r as any)?.collectionMethod ?? '') + ' ' + String((r as any)?.providerName ?? '') + ' ' + String((r as any)?.providerPaymentIntentID ?? '') + ' ' + String((r as any)?.scheduledChargeAt ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.capturedBy ?? '')).toLowerCase();
  ...
  const handleDelete = async (row: PaymentIntent) => {
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/paymentintent/updatePaymentIntent/{PaymentIntentID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "PaymentIntentID", type: "string", required: true, source: "record", options: [] }, { name: "status", type: "string", required: true, source: "constant", default: "SCHEDULED", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.paymentIntentID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.paymentIntentID ?? Math.random()}>
                    <td>{lookup_tenantID[String(r?.tenantID ?? '')] || String(r?.tenantID ?? '-')}</td>
                    <td>{lookup_paymentMethodID[String(r?.paymentMethodID ?? '')] || String(r?.paymentMethodID ?? '-')}</td>
                    <td>{String(r?.amount ?? '-')}</td>
                    <td>{String(r?.currency ?? '-')}</td>
                    <td>{String(r?.status ?? '-')}</td>
                    <td>{String(r?.collectionMethod ?? '-')}</td>
                    <td>{String(r?.providerName ?? '-')}</td>
                    <td>{String(r?.providerPaymentIntentID ?? '-')}</td>
                    <td>{r?.scheduledChargeAt ? new Date(r?.scheduledChargeAt as any).toLocaleDateString() : '-'}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.paymentIntentID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.paymentIntentID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.paymentIntentID ?? '')] || {}),
                                right: 0,
...
export default PaymentIntentGrid;
