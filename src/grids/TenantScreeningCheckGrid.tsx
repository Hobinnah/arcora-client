...
import { fetchTenantScreeningChecks } from '../apis/useTenantScreeningCheck';
...
export interface TenantScreeningCheckGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: TenantScreeningCheck)=>void;
  onEdit?: (r: TenantScreeningCheck)=>void;
  onDelete?: (r: TenantScreeningCheck, setItems: React.Dispatch<React.SetStateAction<TenantScreeningCheck[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const TenantScreeningCheckGrid: React.FC<TenantScreeningCheckGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    ...
    { key: "providerReferenceID", label: "Provider Reference ID" },
    { key: "status", label: "Status" },
    { key: "consentCapturedAt", label: "Consent Captured At" },
  ...
  const [items, setItems] = useState<TenantScreeningCheck[]>([]);
  ...
  const [check_type, set_check_type] = useState<string>('');
  ...
  const load = useCallback(async () => {
    try {
      ...
      const params: any = env.SEARCH_STRATEGY === 'server' ? {
        ...
        checkType: check_type || undefined,
      } : {
      ...
      const result = await fetchTenantScreeningChecks(params);
    ...
    ? [pageSize, page, query, sort, status, check_type, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && status === '' && check_type === '') return items;

    let res = items.filter((r: TenantScreeningCheck) => {
      const s = (String((r as any)?.checkType ?? '') + ' ' + String((r as any)?.providerName ?? '') + ' ' + String((r as any)?.providerReferenceID ?? '') + ' ' + String((r as any)?.status ?? '') + ' ' + String((r as any)?.consentCapturedAt ?? '') + ' ' + String((r as any)?.score ?? '') + ' ' + String((r as any)?.requestedAt ?? '') + ' ' + String((r as any)?.completedAt ?? '') + ' ' + String((r as any)?.expiresAt ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.capturedBy ?? '')).toLowerCase();
      const matchesSearch = !query || s.includes(query.toLowerCase());
      const match_status = !status || String((r as any)?.status ?? '') === status;
      const match_check_type = !check_type || String((r as any)?.checkType ?? '') === check_type;
      return matchesSearch && match_status && match_check_type;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, status, check_type]);
  ...
  useEffect(() => { if (query || sort || status || check_type) setPage(1); }, [query, sort, status, check_type]);
  ...
  const handleDelete = async (row: TenantScreeningCheck) => {
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/tenantscreeningcheck/updatetenantscreeningcheckstatus/{TenantScreeningCheckID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "TenantScreeningCheckID", type: "string", required: true, source: "record", options: [] }, { name: "Status", type: "string", required: true, source: "constant", default: "COMPLETED", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.tenantScreeningCheckID) === recordId);
      ...
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            ...
            <input className="input" placeholder="All" value={ check_type } onChange={(e) => set_check_type(e.target.value)} />
          </label>
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.tenantScreeningCheckID ?? Math.random()}>
                    <td>{String(r?.checkType ?? '-')}</td>
                    <td>{String(r?.providerName ?? '-')}</td>
                    <td>{String(r?.providerReferenceID ?? '-')}</td>
                    <td>{String(r?.status ?? '-')}</td>
                    <td>{r?.consentCapturedAt ? new Date(r?.consentCapturedAt as any).toLocaleDateString() : '-'}</td>
                    <td>{String(r?.score ?? '-')}</td>
                    <td>{r?.requestedAt ? new Date(r?.requestedAt as any).toLocaleDateString() : '-'}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.tenantScreeningCheckID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.tenantScreeningCheckID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.tenantScreeningCheckID ?? '')] || {}),
                                right: 0,
...
export default TenantScreeningCheckGrid;
