...
import { fetchTenantEmergencyContacts } from '../apis/useTenantEmergencyContact';
...
export interface TenantEmergencyContactGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: TenantEmergencyContact)=>void;
  onEdit?: (r: TenantEmergencyContact)=>void;
  onDelete?: (r: TenantEmergencyContact, setItems: React.Dispatch<React.SetStateAction<TenantEmergencyContact[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const TenantEmergencyContactGrid: React.FC<TenantEmergencyContactGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const [items, setItems] = useState<TenantEmergencyContact[]>([]);
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchTenantEmergencyContacts(params);
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: TenantEmergencyContact) => {
      const s = (String((r as any)?.name ?? '') + ' ' + String((r as any)?.relationship ?? '') + ' ' + String((r as any)?.phoneNumber ?? '') + ' ' + String((r as any)?.email ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.capturedBy ?? '') + ' ' + String((r as any)?.updatedDate ?? '') + ' ' + String((r as any)?.updatedBy ?? '')).toLowerCase();
  ...
  const handleDelete = async (row: TenantEmergencyContact) => {
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.tenantEmergencyContactID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.tenantEmergencyContactID ?? Math.random()}>
                    <td>{String(r?.name ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.tenantEmergencyContactID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.tenantEmergencyContactID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.tenantEmergencyContactID ?? '')] || {}),
                                right: 0,
...
export default TenantEmergencyContactGrid;
