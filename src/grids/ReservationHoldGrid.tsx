...
import { fetchReservationHolds } from '../apis/useReservationHold';
...
export interface ReservationHoldGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: ReservationHold)=>void;
  onEdit?: (r: ReservationHold)=>void;
  onDelete?: (r: ReservationHold, setItems: React.Dispatch<React.SetStateAction<ReservationHold[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const ReservationHoldGrid: React.FC<ReservationHoldGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    ...
    { key: "convertedToLeaseAt", label: "Converted To Lease At" },
    { key: "capturedDate", label: "Captured Date" },
    { key: "capturedBy", label: "Captured By" },
  ] as const;

  const [items, setItems] = useState<ReservationHold[]>([]);
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchReservationHolds(params);
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: ReservationHold) => {
      const s = (String((r as any)?.startDate ?? '') + ' ' + String((r as any)?.endDate ?? '') + ' ' + String((r as any)?.status ?? '') + ' ' + String((r as any)?.expiresAt ?? '') + ' ' + String((r as any)?.releasedAt ?? '') + ' ' + String((r as any)?.convertedToLeaseAt ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.capturedBy ?? '')).toLowerCase();
  ...
  const handleDelete = async (row: ReservationHold) => {
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/reservationhold/updateReservationHold/{ReservationHoldID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "ReservationHoldID", type: "string", required: true, source: "record", options: [] }, { name: "status", type: "string", required: true, source: "constant", default: "RELEASED", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.reservationHoldID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.reservationHoldID ?? Math.random()}>
                    ...
                    <td>{r?.releasedAt ? new Date(r?.releasedAt as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.convertedToLeaseAt ? new Date(r?.convertedToLeaseAt as any).toLocaleDateString() : '-'}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.reservationHoldID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.reservationHoldID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.reservationHoldID ?? '')] || {}),
                                right: 0,
...
export default ReservationHoldGrid;
