...
import { fetchChargebacks } from '../apis/useChargeback';
...
export interface ChargebackGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: Chargeback)=>void;
  onEdit?: (r: Chargeback)=>void;
  onDelete?: (r: Chargeback, setItems: React.Dispatch<React.SetStateAction<Chargeback[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const ChargebackGrid: React.FC<ChargebackGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    ...
    { key: "providerDisputeID", label: "Provider Dispute ID" },
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchChargebacks(params);
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: Chargeback) => {
      const s = (String((r as any)?.tenantID ?? '') + ' ' + String((r as any)?.providerDisputeID ?? '') + ' ' + String((r as any)?.amount ?? '') + ' ' + String((r as any)?.currency ?? '') + ' ' + String((r as any)?.status ?? '') + ' ' + String((r as any)?.openedAt ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.capturedBy ?? '')).toLowerCase();
  ...
  const handleDelete = async (row: Chargeback) => {
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/chargeback/updateChargeback/{ChargebackID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "ChargebackID", type: "string", required: true, source: "record", options: [] }, { name: "status", type: "string", required: true, source: "constant", default: "CLOSED", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.chargebackID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.chargebackID ?? Math.random()}>
                    <td>{lookup_tenantID[String(r?.tenantID ?? '')] || String(r?.tenantID ?? '-')}</td>
                    <td>{String(r?.providerDisputeID ?? '-')}</td>
                    <td>{String(r?.amount ?? '-')}</td>
                    <td>{String(r?.currency ?? '-')}</td>
                    <td>{String(r?.status ?? '-')}</td>
                    <td>{r?.openedAt ? new Date(r?.openedAt as any).toLocaleDateString() : '-'}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.chargebackID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.chargebackID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.chargebackID ?? '')] || {}),
                                right: 0,
