...
import { fetchOrgSubscriptions } from '../apis/useOrgSubscription';
...
export interface OrgSubscriptionGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: OrgSubscription)=>void;
  onEdit?: (r: OrgSubscription)=>void;
  onDelete?: (r: OrgSubscription, setItems: React.Dispatch<React.SetStateAction<OrgSubscription[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const OrgSubscriptionGrid: React.FC<OrgSubscriptionGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const [billing_frequency, set_billing_frequency] = useState<string>('');
  ...
  const load = useCallback(async () => {
    try {
      ...
      const params: any = env.SEARCH_STRATEGY === 'server' ? {
        ...
        billingFrequency: billing_frequency || undefined,
      } : {
      ...
      const result = await fetchOrgSubscriptions(params);
    ...
    ? [pageSize, page, query, sort, status, billing_frequency, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && status === '' && billing_frequency === '') return items;

    let res = items.filter((r: OrgSubscription) => {
      const s = (String((r as any)?.status ?? '') + ' ' + String((r as any)?.billingFrequency ?? '') + ' ' + String((r as any)?.startedAt ?? '') + ' ' + String((r as any)?.trialEndsAt ?? '') + ' ' + String((r as any)?.currentPeriodStart ?? '') + ' ' + String((r as any)?.currentPeriodEnd ?? '') + ' ' + String((r as any)?.cancelledAt ?? '') + ' ' + String((r as any)?.endedAt ?? '') + ' ' + String((r as any)?.providerName ?? '') + ' ' + String((r as any)?.providerCustomerID ?? '') + ' ' + String((r as any)?.providerSubscriptionID ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.updatedDate ?? '')).toLowerCase();
      const matchesSearch = !query || s.includes(query.toLowerCase());
      const match_status = !status || String((r as any)?.status ?? '') === status;
      const match_billing_frequency = !billing_frequency || String((r as any)?.billingFrequency ?? '') === billing_frequency;
      return matchesSearch && match_status && match_billing_frequency;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, status, billing_frequency]);
  ...
  useEffect(() => { if (query || sort || status || billing_frequency) setPage(1); }, [query, sort, status, billing_frequency]);
  ...
  const handleDelete = async (row: OrgSubscription) => {
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/orgsubscription/updateOrgSubscription/{OrgSubscriptionID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "OrgSubscriptionID", type: "string", required: true, source: "record", options: [] }, { name: "status", type: "string", required: true, source: "constant", default: "ACTIVE", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.orgSubscriptionID) === recordId);
      ...
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            <span>Status</span>
            <input className="input" placeholder="All" value={ status } onChange={(e) => set_status(e.target.value)} />
          </label>
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            <span>Billing Frequency</span>
            <input className="input" placeholder="All" value={ billing_frequency } onChange={(e) => set_billing_frequency(e.target.value)} />
          </label>
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.orgSubscriptionID ?? Math.random()}>
                    <td>{String(r?.status ?? '-')}</td>
                    <td>{String(r?.billingFrequency ?? '-')}</td>
                    <td>{r?.startedAt ? new Date(r?.startedAt as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.trialEndsAt ? new Date(r?.trialEndsAt as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.currentPeriodStart ? new Date(r?.currentPeriodStart as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.currentPeriodEnd ? new Date(r?.currentPeriodEnd as any).toLocaleDateString() : '-'}</td>
                    <td>{String(r?.cancelAtPeriodEnd ?? '-')}</td>
                    ...
                    <td>{String(r?.providerCustomerID ?? '-')}</td>
                    <td>{String(r?.providerSubscriptionID ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.orgSubscriptionID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.orgSubscriptionID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.orgSubscriptionID ?? '')] || {}),
                                right: 0,
...
export default OrgSubscriptionGrid;
