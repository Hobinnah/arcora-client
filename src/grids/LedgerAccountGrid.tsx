...
import { fetchLedgerAccounts } from '../apis/useLedgerAccount';
...
export interface LedgerAccountGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: LedgerAccount)=>void;
  onEdit?: (r: LedgerAccount)=>void;
  onDelete?: (r: LedgerAccount, setItems: React.Dispatch<React.SetStateAction<LedgerAccount[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const LedgerAccountGrid: React.FC<LedgerAccountGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    ...
    { key: "name", label: "Name" },
    { key: "isSystemAccount", label: "Is System Account" },
  ...
  const [account_category, set_account_category] = useState<string>('');
  ...
  const load = useCallback(async () => {
    try {
      ...
      const params: any = env.SEARCH_STRATEGY === 'server' ? {
        ...
        accountCategory: account_category || undefined,
      } : {
      ...
      const result = await fetchLedgerAccounts(params);
    ...
    ? [pageSize, page, query, sort, active_status, account_category, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && active_status === '' && account_category === '') return items;

    let res = items.filter((r: LedgerAccount) => {
      const s = (String((r as any)?.accountCode ?? '') + ' ' + String((r as any)?.accountType ?? '') + ' ' + String((r as any)?.accountCategory ?? '') + ' ' + String((r as any)?.currency ?? '') + ' ' + String((r as any)?.name ?? '')).toLowerCase();
      ...
      const match_account_category = !account_category || String((r as any)?.accountCategory ?? '') === account_category;
      return matchesSearch && match_active_status && match_account_category;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, active_status, account_category]);
  ...
  useEffect(() => { if (query || sort || active_status || account_category) setPage(1); }, [query, sort, active_status, account_category]);
  ...
  const handleDelete = async (row: LedgerAccount) => {
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.ledgerAccountID) === recordId);
      ...
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            ...
            <input className="input" placeholder="All" value={ account_category } onChange={(e) => set_account_category(e.target.value)} />
          </label>
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.ledgerAccountID ?? Math.random()}>
                    <td>{String(r?.accountCode ?? '-')}</td>
                    <td>{String(r?.accountType ?? '-')}</td>
                    <td>{String(r?.accountCategory ?? '-')}</td>
                    <td>{String(r?.currency ?? '-')}</td>
                    <td>{String(r?.name ?? '-')}</td>
                    <td>{String(r?.isSystemAccount ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.ledgerAccountID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.ledgerAccountID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.ledgerAccountID ?? '')] || {}),
                                right: 0,
...
export default LedgerAccountGrid;
