...
import { fetchCreditReportings } from '../apis/useCreditReporting';
...
export interface CreditReportingGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: CreditReporting)=>void;
  onEdit?: (r: CreditReporting)=>void;
  onDelete?: (r: CreditReporting, setItems: React.Dispatch<React.SetStateAction<CreditReporting[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const CreditReportingGrid: React.FC<CreditReportingGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    { key: "reportedAmount", label: "Reported Amount" },
    { key: "wasPaidOnTime", label: "Was Paid On Time" },
    { key: "reportingPeriodStart", label: "Reporting Period Start" },
    { key: "reportingPeriodEnd", label: "Reporting Period End" },
    { key: "providerResponse", label: "Provider Response" },
    { key: "reportedAt", label: "Reported At" },
  ] as const;

  const [items, setItems] = useState<CreditReporting[]>([]);
  ...
  const [providerstatus, set_providerstatus] = useState<string>('');
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchCreditReportings(params);
    ...
    ? [pageSize, page, query, sort, providerstatus, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && providerstatus === '') return items;

    let res = items.filter((r: CreditReporting) => {
      const s = (String((r as any)?.reportedAmount ?? '') + ' ' + String((r as any)?.reportingPeriodStart ?? '') + ' ' + String((r as any)?.reportingPeriodEnd ?? '') + ' ' + String((r as any)?.providerResponse ?? '') + ' ' + String((r as any)?.reportedAt ?? '')).toLowerCase();
      const matchesSearch = !query || s.includes(query.toLowerCase());
      const match_providerstatus = !providerstatus || String((r as any)?.providerStatus ?? '') === providerstatus;
      return matchesSearch && match_providerstatus;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, providerstatus]);
  ...
  useEffect(() => { if (query || sort || providerstatus) setPage(1); }, [query, sort, providerstatus]);
  ...
  const handleDelete = async (row: CreditReporting) => {
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/creditreporting/updateCreditReporting/{CreditReportingID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "CreditReportingID", type: "string", required: true, source: "record", options: [] }, { name: "providerStatus", type: "string", required: true, source: "constant", default: "ACTIVE", options: [] } ]
      }
    },
  ], []);
  ...
  function isActionVisible(action: any, row: any) {
    ...
    const v = String(row?.providerStatus ?? '');
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.creditReportingID) === recordId);
      ...
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            ...
            <input className="input" placeholder="All" value={ providerstatus } onChange={(e) => set_providerstatus(e.target.value)} />
          </label>
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.creditReportingID ?? Math.random()}>
                    <td>{String(r?.reportedAmount ?? '-')}</td>
                    <td>{String(r?.wasPaidOnTime ?? '-')}</td>
                    <td>{r?.reportingPeriodStart ? new Date(r?.reportingPeriodStart as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.reportingPeriodEnd ? new Date(r?.reportingPeriodEnd as any).toLocaleDateString() : '-'}</td>
                    <td>{String(r?.providerResponse ?? '-')}</td>
                    <td>{r?.reportedAt ? new Date(r?.reportedAt as any).toLocaleDateString() : '-'}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.creditReportingID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.creditReportingID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.creditReportingID ?? '')] || {}),
                                right: 0,
...
export default CreditReportingGrid;
