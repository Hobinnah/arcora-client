...
import { fetchPaymentAttempts } from '../apis/usePaymentAttempt';
...
export interface PaymentAttemptGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: PaymentAttempt)=>void;
  onEdit?: (r: PaymentAttempt)=>void;
  onDelete?: (r: PaymentAttempt, setItems: React.Dispatch<React.SetStateAction<PaymentAttempt[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const PaymentAttemptGrid: React.FC<PaymentAttemptGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    ...
    { key: "providerAttemptID", label: "Provider Attempt ID" },
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchPaymentAttempts(params);
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: PaymentAttempt) => {
      const s = (String((r as any)?.attemptNumber ?? '') + ' ' + String((r as any)?.amount ?? '') + ' ' + String((r as any)?.status ?? '') + ' ' + String((r as any)?.providerAttemptID ?? '') + ' ' + String((r as any)?.attemptedAt ?? '') + ' ' + String((r as any)?.capturedDate ?? '')).toLowerCase();
  ...
  const handleDelete = async (row: PaymentAttempt) => {
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/paymentattempt/updatePaymentAttempt/{PaymentAttemptID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "PaymentAttemptID", type: "string", required: true, source: "record", options: [] }, { name: "status", type: "string", required: true, source: "constant", default: "ACTIVE", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.paymentAttemptID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.paymentAttemptID ?? Math.random()}>
                    <td>{String(r?.attemptNumber ?? '-')}</td>
                    <td>{String(r?.amount ?? '-')}</td>
                    <td>{String(r?.status ?? '-')}</td>
                    <td>{String(r?.providerAttemptID ?? '-')}</td>
                    <td>{r?.attemptedAt ? new Date(r?.attemptedAt as any).toLocaleDateString() : '-'}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.paymentAttemptID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.paymentAttemptID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.paymentAttemptID ?? '')] || {}),
                                right: 0,
...
export default PaymentAttemptGrid;
