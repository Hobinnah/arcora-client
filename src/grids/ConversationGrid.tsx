...
export interface ConversationGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: Conversation)=>void;
  onEdit?: (r: Conversation)=>void;
  onDelete?: (r: Conversation, setItems: React.Dispatch<React.SetStateAction<Conversation[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const ConversationGrid: React.FC<ConversationGridProps> = ({
  ...
  const [conversation_type, set_conversation_type] = useState<string>('');
  ...
  const load = useCallback(async () => {
    try {
      ...
      const params: any = env.SEARCH_STRATEGY === 'server' ? {
        ...
        conversationType: conversation_type || undefined,
      } : {
    ...
    ? [pageSize, page, query, sort, status, conversation_type, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && status === '' && conversation_type === '') return items;

    let res = items.filter((r: Conversation) => {
      const s = (String((r as any)?.conversationType ?? '') + ' ' + String((r as any)?.subject ?? '') + ' ' + String((r as any)?.status ?? '') + ' ' + String((r as any)?.lastMessageAt ?? '') + ' ' + String((r as any)?.closedAt ?? '')).toLowerCase();
      const matchesSearch = !query || s.includes(query.toLowerCase());
      const match_status = !status || String((r as any)?.status ?? '') === status;
      const match_conversation_type = !conversation_type || String((r as any)?.conversationType ?? '') === conversation_type;
      return matchesSearch && match_status && match_conversation_type;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, status, conversation_type]);
  ...
  useEffect(() => { if (query || sort || status || conversation_type) setPage(1); }, [query, sort, status, conversation_type]);
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/conversation/updateConversation/{ConversationID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "ConversationID", type: "string", required: true, source: "record", options: [] }, { name: "status", type: "string", required: true, source: "constant", default: "CLOSED", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.conversationID) === recordId);
      ...
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            ...
            <select className="select" value={ conversation_type } onChange={(e) => set_conversation_type(e.target.value)}>
              <option value="">All</option>
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.conversationID ?? Math.random()}>
                    <td>{String(r?.conversationType ?? '-')}</td>
                    <td>{String(r?.subject ?? '-')}</td>
                    <td>{String(r?.status ?? '-')}</td>
                    <td>{r?.lastMessageAt ? new Date(r?.lastMessageAt as any).toLocaleDateString() : '-'}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.conversationID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.conversationID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.conversationID ?? '')] || {}),
                                right: 0,
