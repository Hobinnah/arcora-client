...
import { fetchOrgPayoutAccounts } from '../apis/useOrgPayoutAccount';
...
export interface OrgPayoutAccountGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: OrgPayoutAccount)=>void;
  onEdit?: (r: OrgPayoutAccount)=>void;
  onDelete?: (r: OrgPayoutAccount, setItems: React.Dispatch<React.SetStateAction<OrgPayoutAccount[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const OrgPayoutAccountGrid: React.FC<OrgPayoutAccountGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const [items, setItems] = useState<OrgPayoutAccount[]>([]);
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchOrgPayoutAccounts(params);
    ...
    ? [pageSize, page, query, sort, verification_status, active_status, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && verification_status === '' && active_status === '') return items;

    let res = items.filter((r: OrgPayoutAccount) => {
      const s = (String((r as any)?.providerName ?? '') + ' ' + String((r as any)?.providerAccountID ?? '') + ' ' + String((r as any)?.accountType ?? '') + ' ' + String((r as any)?.bankName ?? '') + ' ' + String((r as any)?.accountLast4 ?? '') + ' ' + String((r as any)?.currency ?? '') + ' ' + String((r as any)?.verificationStatus ?? '') + ' ' + String((r as any)?.verifiedAt ?? '')).toLowerCase();
      ...
      return matchesSearch && match_verification_status && match_active_status;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, verification_status, active_status]);
  ...
  useEffect(() => { if (query || sort || verification_status || active_status) setPage(1); }, [query, sort, verification_status, active_status]);
  ...
  const handleDelete = async (row: OrgPayoutAccount) => {
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/orgpayoutaccount/updateOrgPayoutAccount/{OrgPayoutAccountID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "OrgPayoutAccountID", type: "number", required: true, source: "record", options: [] }, { name: "verificationStatus", type: "string", required: true, source: "constant", default: "VERIFIED", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.orgPayoutAccountID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.orgPayoutAccountID ?? Math.random()}>
                    <td>{String(r?.providerName ?? '-')}</td>
                    <td>{String(r?.providerAccountID ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.orgPayoutAccountID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.orgPayoutAccountID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.orgPayoutAccountID ?? '')] || {}),
                                right: 0,
...
export default OrgPayoutAccountGrid;
