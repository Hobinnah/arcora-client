...
import { fetchCalendarEvents } from '../apis/useCalendarEvent';
...
export interface CalendarEventGridProps {
  ...
  onView?: (r: CalendarEvent)=>void;
  onEdit?: (r: CalendarEvent)=>void;
  onDelete?: (r: CalendarEvent, setItems: React.Dispatch<React.SetStateAction<CalendarEvent[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const CalendarEventGrid: React.FC<CalendarEventGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    ...
    { key: "isAllDay", label: "Is All Day" },
    { key: "title", label: "Title" },
    { key: "occupantName", label: "Occupant Name" },
    { key: "occupantCount", label: "Occupant Count" },
    { key: "sourceSystem", label: "Source System" },
    { key: "sourceReferenceID", label: "Source Reference ID" },
    { key: "externalCalendarID", label: "External Calendar ID" },
    { key: "blocksAvailability", label: "Blocks Availability" },
  ...
  const [event_type, set_event_type] = useState<string>('');
  ...
  const load = useCallback(async () => {
    ...
    ? [pageSize, page, query, sort, status, event_type, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && status === '' && event_type === '') return items;

    let res = items.filter((r: CalendarEvent) => {
      const s = (String((r as any)?.eventType ?? '') + ' ' + String((r as any)?.status ?? '') + ' ' + String((r as any)?.startAt ?? '') + ' ' + String((r as any)?.endAt ?? '') + ' ' + String((r as any)?.title ?? '') + ' ' + String((r as any)?.occupantName ?? '') + ' ' + String((r as any)?.occupantCount ?? '') + ' ' + String((r as any)?.sourceSystem ?? '') + ' ' + String((r as any)?.sourceReferenceID ?? '') + ' ' + String((r as any)?.externalCalendarID ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.updatedDate ?? '')).toLowerCase();
      const matchesSearch = !query || s.includes(query.toLowerCase());
      const match_status = !status || String((r as any)?.status ?? '') === status;
      const match_event_type = !event_type || String((r as any)?.eventType ?? '') === event_type;
      return matchesSearch && match_status && match_event_type;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, status, event_type]);
  ...
  useEffect(() => { if (query || sort || status || event_type) setPage(1); }, [query, sort, status, event_type]);
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/calendarevent/updateCalendarEvent/{CalendarEventID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "CalendarEventID", type: "string", required: true, source: "record", options: [] }, { name: "status", type: "string", required: true, source: "constant", default: "ACTIVE", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.calendarEventID) === recordId);
      ...
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            ...
            <input className="input" placeholder="All" value={ event_type } onChange={(e) => set_event_type(e.target.value)} />
          </label>
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.calendarEventID ?? Math.random()}>
                    <td>{String(r?.eventType ?? '-')}</td>
                    <td>{String(r?.status ?? '-')}</td>
                    <td>{r?.startAt ? new Date(r?.startAt as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.endAt ? new Date(r?.endAt as any).toLocaleDateString() : '-'}</td>
                    <td>{String(r?.isAllDay ?? '-')}</td>
                    <td>{String(r?.title ?? '-')}</td>
                    <td>{String(r?.occupantName ?? '-')}</td>
                    <td>{String(r?.occupantCount ?? '-')}</td>
                    <td>{String(r?.sourceSystem ?? '-')}</td>
                    <td>{String(r?.sourceReferenceID ?? '-')}</td>
                    <td>{String(r?.externalCalendarID ?? '-')}</td>
                    <td>{String(r?.blocksAvailability ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.calendarEventID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.calendarEventID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.calendarEventID ?? '')] || {}),
                                right: 0,
