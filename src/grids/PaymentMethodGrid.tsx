...
import { fetchPaymentMethods } from '../apis/usePaymentMethod';
...
export interface PaymentMethodGridProps {
  ...
  onView?: (r: PaymentMethod)=>void;
  onEdit?: (r: PaymentMethod)=>void;
  onDelete?: (r: PaymentMethod, setItems: React.Dispatch<React.SetStateAction<PaymentMethod[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const PaymentMethodGrid: React.FC<PaymentMethodGridProps> = ({
  ...
  const columns = [
    ...
    { key: "paymentMethodType", label: "Payment Method Type" },
    { key: "displayName", label: "Display Name" },
    { key: "accountLast4", label: "Account Last 4" },
  ...
  const [payment_method_type, set_payment_method_type] = useState<string>('');
  ...
  const load = useCallback(async () => {
    try {
      ...
      const params: any = env.SEARCH_STRATEGY === 'server' ? {
        ...
        paymentMethodType: payment_method_type || undefined,
      ...
      const result = await fetchPaymentMethods(params);
    ...
    ? [pageSize, page, query, sort, payment_method_type, verification_status, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && payment_method_type === '' && verification_status === '') return items;

    let res = items.filter((r: PaymentMethod) => {
      const s = (String((r as any)?.tenantID ?? '') + ' ' + String((r as any)?.paymentMethodType ?? '') + ' ' + String((r as any)?.displayName ?? '') + ' ' + String((r as any)?.accountLast4 ?? '') + ' ' + String((r as any)?.cardBrand ?? '') + ' ' + String((r as any)?.bankName ?? '') + ' ' + String((r as any)?.expiryMonth ?? '') + ' ' + String((r as any)?.expiryYear ?? '') + ' ' + String((r as any)?.providerName ?? '') + ' ' + String((r as any)?.verificationStatus ?? '') + ' ' + String((r as any)?.verifiedAt ?? '') + ' ' + String((r as any)?.capturedBy ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.updatedDate ?? '') + ' ' + String((r as any)?.updatedBy ?? '')).toLowerCase();
      const matchesSearch = !query || s.includes(query.toLowerCase());
      const match_payment_method_type = !payment_method_type || String((r as any)?.paymentMethodType ?? '') === payment_method_type;
      const match_verification_status = !verification_status || String((r as any)?.verificationStatus ?? '') === verification_status;
      return matchesSearch && match_payment_method_type && match_verification_status;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, payment_method_type, verification_status]);
  ...
  useEffect(() => { if (query || sort || payment_method_type || verification_status) setPage(1); }, [query, sort, payment_method_type, verification_status]);
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/paymentmethod/updatePaymentMethod/{PaymentMethodID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "PaymentMethodID", type: "string", required: true, source: "record", options: [] }, { name: "IsActive", type: "boolean", required: true, source: "constant", default: "false", options: [] } ]
      }
    },
  ], []);
  ...
  function isActionVisible(action: any, row: any) {
    ...
    const v = String(row?.paymentMethodType ?? '');
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.paymentMethodID) === recordId);
      ...
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            ...
            <input className="input" placeholder="All" value={ payment_method_type } onChange={(e) => set_payment_method_type(e.target.value)} />
          </label>
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.paymentMethodID ?? Math.random()}>
                    <td>{lookup_tenantID[String(r?.tenantID ?? '')] || String(r?.tenantID ?? '-')}</td>
                    <td>{String(r?.paymentMethodType ?? '-')}</td>
                    <td>{String(r?.displayName ?? '-')}</td>
                    <td>{String(r?.accountLast4 ?? '-')}</td>
                    <td>{String(r?.cardBrand ?? '-')}</td>
                    <td>{String(r?.bankName ?? '-')}</td>
                    <td>{String(r?.expiryMonth ?? '-')}</td>
                    <td>{String(r?.expiryYear ?? '-')}</td>
                    <td>{String(r?.providerName ?? '-')}</td>
                    <td>{String(r?.verificationStatus ?? '-')}</td>
                    <td>{r?.verifiedAt ? new Date(r?.verifiedAt as any).toLocaleDateString() : '-'}</td>
                    <td>{String(r?.isDefault ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.paymentMethodID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.paymentMethodID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.paymentMethodID ?? '')] || {}),
                                right: 0,
