...
import { fetchInspections } from '../apis/useInspection';
...
export interface InspectionGridProps {
  ...
  onView?: (r: Inspection)=>void;
  onEdit?: (r: Inspection)=>void;
  onDelete?: (r: Inspection, setItems: React.Dispatch<React.SetStateAction<Inspection[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const InspectionGrid: React.FC<InspectionGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const [inspection_type, set_inspection_type] = useState<string>('');
  ...
  const load = useCallback(async () => {
    try {
      ...
      const params: any = env.SEARCH_STRATEGY === 'server' ? {
        ...
        inspectionType: inspection_type || undefined,
      } : {
      ...
      const result = await fetchInspections(params);
    ...
    ? [pageSize, page, query, sort, status, inspection_type, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && status === '' && inspection_type === '') return items;

    let res = items.filter((r: Inspection) => {
      const s = (String((r as any)?.inspectionType ?? '') + ' ' + String((r as any)?.status ?? '') + ' ' + String((r as any)?.scheduledFor ?? '') + ' ' + String((r as any)?.startedAt ?? '') + ' ' + String((r as any)?.completedAt ?? '')).toLowerCase();
      const matchesSearch = !query || s.includes(query.toLowerCase());
      const match_status = !status || String((r as any)?.status ?? '') === status;
      const match_inspection_type = !inspection_type || String((r as any)?.inspectionType ?? '') === inspection_type;
      return matchesSearch && match_status && match_inspection_type;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, status, inspection_type]);
  ...
  useEffect(() => { if (query || sort || status || inspection_type) setPage(1); }, [query, sort, status, inspection_type]);
  ...
  const actions = useMemo(() => [
    {
      key: "set-in-progress",
      label: "Set IN_PROGRESS",
      variant: "default",
      values: ["SCHEDULED"],
      api: {
        method: "POST",
        pathTemplate: "api/inspection/updateInspection/{InspectionID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "InspectionID", type: "string", required: true, source: "record", options: [] }, { name: "status", type: "string", required: true, source: "constant", default: "IN_PROGRESS", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.inspectionID) === recordId);
      ...
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            ...
            <input className="input" placeholder="All" value={ inspection_type } onChange={(e) => set_inspection_type(e.target.value)} />
          </label>
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.inspectionID ?? Math.random()}>
                    <td>{String(r?.inspectionType ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.inspectionID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.inspectionID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.inspectionID ?? '')] || {}),
                                right: 0,
