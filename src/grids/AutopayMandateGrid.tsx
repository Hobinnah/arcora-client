...
import { fetchAutopayMandates } from '../apis/useAutopayMandate';
...
export interface AutopayMandateGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: AutopayMandate)=>void;
  onEdit?: (r: AutopayMandate)=>void;
  onDelete?: (r: AutopayMandate, setItems: React.Dispatch<React.SetStateAction<AutopayMandate[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const AutopayMandateGrid: React.FC<AutopayMandateGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    ...
    { key: "maximumAmountPerDebit", label: "Maximum Amount Per Debit" },
    { key: "currency", label: "Currency" },
    { key: "frequency", label: "Frequency" },
    { key: "startDate", label: "Start Date" },
    { key: "endDate", label: "End Date" },
    { key: "providerName", label: "Provider Name" },
    { key: "providerMandateID", label: "Provider Mandate ID" },
    { key: "consentVersion", label: "Consent Version" },
    { key: "consentedAt", label: "Consented At" },
  ...
  const [mandate_type, set_mandate_type] = useState<string>('');
  ...
  const load = useCallback(async () => {
    try {
      ...
      const params: any = env.SEARCH_STRATEGY === 'server' ? {
        ...
        mandateType: mandate_type || undefined,
      } : {
      ...
      const result = await fetchAutopayMandates(params);
    ...
    ? [pageSize, page, query, sort, status, mandate_type, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && status === '' && mandate_type === '') return items;

    let res = items.filter((r: AutopayMandate) => {
      const s = (String((r as any)?.status ?? '') + ' ' + String((r as any)?.mandateType ?? '') + ' ' + String((r as any)?.paymentRail ?? '') + ' ' + String((r as any)?.maximumAmountPerDebit ?? '') + ' ' + String((r as any)?.currency ?? '') + ' ' + String((r as any)?.frequency ?? '') + ' ' + String((r as any)?.startDate ?? '') + ' ' + String((r as any)?.endDate ?? '') + ' ' + String((r as any)?.providerName ?? '') + ' ' + String((r as any)?.providerMandateID ?? '') + ' ' + String((r as any)?.consentVersion ?? '') + ' ' + String((r as any)?.consentedAt ?? '') + ' ' + String((r as any)?.activatedAt ?? '') + ' ' + String((r as any)?.cancelledAt ?? '')).toLowerCase();
      const matchesSearch = !query || s.includes(query.toLowerCase());
      const match_status = !status || String((r as any)?.status ?? '') === status;
      const match_mandate_type = !mandate_type || String((r as any)?.mandateType ?? '') === mandate_type;
      return matchesSearch && match_status && match_mandate_type;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, status, mandate_type]);
  ...
  useEffect(() => { if (query || sort || status || mandate_type) setPage(1); }, [query, sort, status, mandate_type]);
  ...
  const handleDelete = async (row: AutopayMandate) => {
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "GET",
        pathTemplate: "api/autopaymandate/activate/{AutopayMandateID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "AutopayMandateID", type: "string", required: true, source: "record", default: "", options: [] } ]
      }
    },
    {
      key: "cancel",
      ...
      api: {
        method: "GET",
        pathTemplate: "api/autopaymandate/cancel/{AutopayMandateID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.autopayMandateID) === recordId);
      ...
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            ...
            <select className="select" value={ mandate_type } onChange={(e) => set_mandate_type(e.target.value)}>
              <option value="">All</option>
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.autopayMandateID ?? Math.random()}>
                    <td>{String(r?.status ?? '-')}</td>
                    <td>{String(r?.mandateType ?? '-')}</td>
                    <td>{String(r?.paymentRail ?? '-')}</td>
                    <td>{String(r?.maximumAmountPerDebit ?? '-')}</td>
                    ...
                    <td>{String(r?.providerMandateID ?? '-')}</td>
                    <td>{String(r?.consentVersion ?? '-')}</td>
                    <td>{r?.consentedAt ? new Date(r?.consentedAt as any).toLocaleDateString() : '-'}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.autopayMandateID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.autopayMandateID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.autopayMandateID ?? '')] || {}),
                                right: 0,
...
export default AutopayMandateGrid;
