...
import { fetchRefunds } from '../apis/useRefund';
...
export interface RefundGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: Refund)=>void;
  onEdit?: (r: Refund)=>void;
  onDelete?: (r: Refund, setItems: React.Dispatch<React.SetStateAction<Refund[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const RefundGrid: React.FC<RefundGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    ...
    { key: "providerRefundID", label: "Provider Refund ID" },
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchRefunds(params);
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: Refund) => {
      const s = (String((r as any)?.amount ?? '') + ' ' + String((r as any)?.currency ?? '') + ' ' + String((r as any)?.status ?? '') + ' ' + String((r as any)?.providerName ?? '') + ' ' + String((r as any)?.providerRefundID ?? '') + ' ' + String((r as any)?.requestedAt ?? '') + ' ' + String((r as any)?.processedAt ?? '') + ' ' + String((r as any)?.failedAt ?? '')).toLowerCase();
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/refund/updateRefund/{RefundID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "RefundID", type: "string", required: true, source: "record", options: [] }, { name: "status", type: "string", required: true, source: "constant", default: "PROCESSED", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.refundID) === recordId);
              ...
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.refundID ?? Math.random()}>
                    ...
                    <td>{r?.failedAt ? new Date(r?.failedAt as any).toLocaleDateString() : '-'}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.refundID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.refundID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.refundID ?? '')] || {}),
                                right: 0,
