...
import { fetchViewingAppointments } from '../apis/useViewingAppointments';
...
export interface ViewingAppointmentsGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: ViewingAppointments)=>void;
  onEdit?: (r: ViewingAppointments)=>void;
  onDelete?: (r: ViewingAppointments, setItems: React.Dispatch<React.SetStateAction<ViewingAppointments[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const ViewingAppointmentsGrid: React.FC<ViewingAppointmentsGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    ...
    { key: "viewingType", label: "Viewing Type" },
  ...
  const [items, setItems] = useState<ViewingAppointments[]>([]);
  ...
  const [viewing_type, set_viewing_type] = useState<string>('');
  ...
  const load = useCallback(async () => {
    try {
      ...
      const params: any = env.SEARCH_STRATEGY === 'server' ? {
        ...
        viewingType: viewing_type || undefined,
      } : {
      ...
      const result = await fetchViewingAppointments(params);
    ...
    ? [pageSize, page, query, sort, status, viewing_type, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && status === '' && viewing_type === '') return items;

    let res = items.filter((r: ViewingAppointments) => {
      const s = (String((r as any)?.scheduledFor ?? '') + ' ' + String((r as any)?.durationMinutes ?? '') + ' ' + String((r as any)?.timeZone ?? '') + ' ' + String((r as any)?.status ?? '') + ' ' + String((r as any)?.viewingType ?? '') + ' ' + String((r as any)?.notes ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.capturedBy ?? '') + ' ' + String((r as any)?.updatedDate ?? '') + ' ' + String((r as any)?.updatedBy ?? '')).toLowerCase();
      const matchesSearch = !query || s.includes(query.toLowerCase());
      const match_status = !status || String((r as any)?.status ?? '') === status;
      const match_viewing_type = !viewing_type || String((r as any)?.viewingType ?? '') === viewing_type;
      return matchesSearch && match_status && match_viewing_type;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, status, viewing_type]);
  ...
  useEffect(() => { if (query || sort || status || viewing_type) setPage(1); }, [query, sort, status, viewing_type]);
  ...
  const handleDelete = async (row: ViewingAppointments) => {
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/viewingappointments/updateViewingAppointmentsStatus/{ViewingAppointmentID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "ViewingAppointmentID", type: "string", required: true, source: "record", options: [] }, { name: "Status", type: "string", required: true, source: "constant", default: "CONFIRMED", options: [] } ]
      }
    },
    {
      ...
      api: {
        ...
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "ViewingAppointmentID", type: "string", required: true, source: "record", options: [] }, { name: "Status", type: "string", required: true, source: "constant", default: "CANCELLED", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.viewingAppointmentID) === recordId);
      ...
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            ...
            <select className="select" value={ viewing_type } onChange={(e) => set_viewing_type(e.target.value)}>
              <option value="">All</option>
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.viewingAppointmentID ?? Math.random()}>
                    <td>{r?.scheduledFor ? new Date(r?.scheduledFor as any).toLocaleDateString() : '-'}</td>
                    <td>{String(r?.durationMinutes ?? '-')}</td>
                    <td>{String(r?.timeZone ?? '-')}</td>
                    <td>{String(r?.status ?? '-')}</td>
                    <td>{String(r?.viewingType ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.viewingAppointmentID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.viewingAppointmentID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.viewingAppointmentID ?? '')] || {}),
                                right: 0,
...
export default ViewingAppointmentsGrid;
