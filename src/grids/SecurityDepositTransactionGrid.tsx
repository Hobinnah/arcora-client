...
import { fetchSecurityDepositTransactions } from '../apis/useSecurityDepositTransaction';
...
export interface SecurityDepositTransactionGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: SecurityDepositTransaction)=>void;
  onEdit?: (r: SecurityDepositTransaction)=>void;
  onDelete?: (r: SecurityDepositTransaction, setItems: React.Dispatch<React.SetStateAction<SecurityDepositTransaction[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const SecurityDepositTransactionGrid: React.FC<SecurityDepositTransactionGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const [items, setItems] = useState<SecurityDepositTransaction[]>([]);
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchSecurityDepositTransactions(params);
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: SecurityDepositTransaction) => {
      const s = (String((r as any)?.transactionType ?? '') + ' ' + String((r as any)?.amount ?? '') + ' ' + String((r as any)?.currency ?? '') + ' ' + String((r as any)?.description ?? '') + ' ' + String((r as any)?.occurredAt ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.capturedBy ?? '')).toLowerCase();
  ...
  const handleDelete = async (row: SecurityDepositTransaction) => {
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.securityDepositTransactionID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.securityDepositTransactionID ?? Math.random()}>
                    <td>{String(r?.transactionType ?? '-')}</td>
                    <td>{String(r?.amount ?? '-')}</td>
                    <td>{String(r?.currency ?? '-')}</td>
                    <td>{String(r?.description ?? '-')}</td>
                    <td>{r?.occurredAt ? new Date(r?.occurredAt as any).toLocaleDateString() : '-'}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.securityDepositTransactionID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.securityDepositTransactionID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.securityDepositTransactionID ?? '')] || {}),
                                right: 0,
...
export default SecurityDepositTransactionGrid;
