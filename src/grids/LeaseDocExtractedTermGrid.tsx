...
import { fetchLeaseDocExtractedTerms } from '../apis/useLeaseDocExtractedTerm';
...
export interface LeaseDocExtractedTermGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: LeaseDocExtractedTerm)=>void;
  onEdit?: (r: LeaseDocExtractedTerm)=>void;
  onDelete?: (r: LeaseDocExtractedTerm, setItems: React.Dispatch<React.SetStateAction<LeaseDocExtractedTerm[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const LeaseDocExtractedTermGrid: React.FC<LeaseDocExtractedTermGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    { key: "extractedStartDate", label: "Extracted Start Date" },
    { key: "extractedEndDate", label: "Extracted End Date" },
    { key: "extractedRentAmount", label: "Extracted Rent Amount" },
    { key: "extractedDepositAmount", label: "Extracted Deposit Amount" },
    { key: "extractedRenewalDate", label: "Extracted Renewal Date" },
  ...
  const [items, setItems] = useState<LeaseDocExtractedTerm[]>([]);
  ...
  const [extraction_status, set_extraction_status] = useState<string>('');
  ...
  const load = useCallback(async () => {
    try {
      ...
      const params: any = env.SEARCH_STRATEGY === 'server' ? {
        ...
        extractionStatus: extraction_status || undefined,
      } : {
      ...
      const result = await fetchLeaseDocExtractedTerms(params);
    ...
    ? [pageSize, page, query, sort, extraction_status, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && extraction_status === '') return items;

    let res = items.filter((r: LeaseDocExtractedTerm) => {
      const s = (String((r as any)?.extractedStartDate ?? '') + ' ' + String((r as any)?.extractedEndDate ?? '') + ' ' + String((r as any)?.extractedRentAmount ?? '') + ' ' + String((r as any)?.extractedDepositAmount ?? '') + ' ' + String((r as any)?.extractedRenewalDate ?? '') + ' ' + String((r as any)?.extractionConfidence ?? '') + ' ' + String((r as any)?.extractionStatus ?? '') + ' ' + String((r as any)?.extractedDate ?? '') + ' ' + String((r as any)?.reviewedAt ?? '')).toLowerCase();
      const matchesSearch = !query || s.includes(query.toLowerCase());
      const match_extraction_status = !extraction_status || String((r as any)?.extractionStatus ?? '') === extraction_status;
      return matchesSearch && match_extraction_status;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, extraction_status]);
  ...
  useEffect(() => { if (query || sort || extraction_status) setPage(1); }, [query, sort, extraction_status]);
  ...
  const handleDelete = async (row: LeaseDocExtractedTerm) => {
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/leasedocextractedterm/updateLeaseDocExtractedTerm/{LeaseDocExtractedTermID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "LeaseDocExtractedTermID", type: "string", required: true, source: "record", options: [] }, { name: "extractionStatus", type: "string", required: true, source: "constant", default: "ACTIVE", options: [] } ]
      }
    },
  ], []);
  ...
  function isActionVisible(action: any, row: any) {
    ...
    const v = String(row?.extractionStatus ?? '');
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.leaseDocExtractedTermID) === recordId);
      ...
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            ...
            <input className="input" placeholder="All" value={ extraction_status } onChange={(e) => set_extraction_status(e.target.value)} />
          </label>
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.leaseDocExtractedTermID ?? Math.random()}>
                    <td>{r?.extractedStartDate ? new Date(r?.extractedStartDate as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.extractedEndDate ? new Date(r?.extractedEndDate as any).toLocaleDateString() : '-'}</td>
                    <td>{String(r?.extractedRentAmount ?? '-')}</td>
                    <td>{String(r?.extractedDepositAmount ?? '-')}</td>
                    <td>{r?.extractedRenewalDate ? new Date(r?.extractedRenewalDate as any).toLocaleDateString() : '-'}</td>
                    <td>{String(r?.extractionConfidence ?? '-')}</td>
                    <td>{String(r?.extractionStatus ?? '-')}</td>
                    <td>{r?.extractedDate ? new Date(r?.extractedDate as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.reviewedAt ? new Date(r?.reviewedAt as any).toLocaleDateString() : '-'}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.leaseDocExtractedTermID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.leaseDocExtractedTermID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.leaseDocExtractedTermID ?? '')] || {}),
                                right: 0,
...
export default LeaseDocExtractedTermGrid;
