...
import { fetchLedgerTransactions } from '../apis/useLedgerTransaction';
...
export interface LedgerTransactionGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: LedgerTransaction)=>void;
  onEdit?: (r: LedgerTransaction)=>void;
  onDelete?: (r: LedgerTransaction, setItems: React.Dispatch<React.SetStateAction<LedgerTransaction[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const LedgerTransactionGrid: React.FC<LedgerTransactionGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const [transaction_date, set_transaction_date] = useState<string>('');
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchLedgerTransactions(params);
    ...
    ? [pageSize, page, query, sort, status, transaction_date, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && status === '' && transaction_date === '') return items;

    let res = items.filter((r: LedgerTransaction) => {
      const s = (String((r as any)?.transactionType ?? '') + ' ' + String((r as any)?.transactionDate ?? '') + ' ' + String((r as any)?.referenceNumber ?? '') + ' ' + String((r as any)?.status ?? '')).toLowerCase();
      const matchesSearch = !query || s.includes(query.toLowerCase());
      const match_status = !status || String((r as any)?.status ?? '') === status;
      const match_transaction_date = !transaction_date || String((r as any)?.transactionDate ?? '') === transaction_date;
      return matchesSearch && match_status && match_transaction_date;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, status, transaction_date]);
  ...
  useEffect(() => { if (query || sort || status || transaction_date) setPage(1); }, [query, sort, status, transaction_date]);
  ...
  const handleDelete = async (row: LedgerTransaction) => {
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/ledgertransaction/updateLedgerTransaction/{LedgerTransactionID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "LedgerTransactionID", type: "string", required: true, source: "record", options: [] }, { name: "status", type: "string", required: true, source: "constant", default: "REVERSED", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.ledgerTransactionID) === recordId);
      ...
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            ...
            <input className="input" placeholder="All" value={ transaction_date } onChange={(e) => set_transaction_date(e.target.value)} />
          </label>
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.ledgerTransactionID ?? Math.random()}>
                    <td>{String(r?.transactionType ?? '-')}</td>
                    <td>{r?.transactionDate ? new Date(r?.transactionDate as any).toLocaleDateString() : '-'}</td>
                    <td>{String(r?.referenceNumber ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.ledgerTransactionID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.ledgerTransactionID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.ledgerTransactionID ?? '')] || {}),
                                right: 0,
...
export default LedgerTransactionGrid;
