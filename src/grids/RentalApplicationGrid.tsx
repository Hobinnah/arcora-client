...
export interface RentalApplicationGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: RentalApplication)=>void;
  onEdit?: (r: RentalApplication)=>void;
  onDelete?: (r: RentalApplication, setItems: React.Dispatch<React.SetStateAction<RentalApplication[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const RentalApplicationGrid: React.FC<RentalApplicationGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    ...
    { key: "desiredMoveOutDate", label: "Desired Move-Out Date" },
    { key: "status", label: "Status" },
    { key: "screeningStatus", label: "Screening Status" },
  ...
  const [screening_status, set_screening_status] = useState<string>('');
  ...
  const load = useCallback(async () => {
    try {
      ...
      const params: any = env.SEARCH_STRATEGY === 'server' ? {
        ...
        screeningStatus: screening_status || undefined,
      } : {
      ...
      const result = await fetchRentalApplications(params);
    ...
    ? [pageSize, page, query, sort, status, screening_status, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && status === '' && screening_status === '') return items;

    let res = items.filter((r: RentalApplication) => {
      const s = (String((r as any)?.applicationCode ?? '') + ' ' + String((r as any)?.desiredMoveInDate ?? '') + ' ' + String((r as any)?.desiredMoveOutDate ?? '') + ' ' + String((r as any)?.status ?? '') + ' ' + String((r as any)?.screeningStatus ?? '') + ' ' + String((r as any)?.submittedAt ?? '') + ' ' + String((r as any)?.reviewedAt ?? '') + ' ' + String((r as any)?.approvedAt ?? '') + ' ' + String((r as any)?.declinedAt ?? '') + ' ' + String((r as any)?.expiresAt ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.capturedBy ?? '') + ' ' + String((r as any)?.updatedDate ?? '') + ' ' + String((r as any)?.updatedBy ?? '')).toLowerCase();
      const matchesSearch = !query || s.includes(query.toLowerCase());
      const match_status = !status || String((r as any)?.status ?? '') === status;
      const match_screening_status = !screening_status || String((r as any)?.screeningStatus ?? '') === screening_status;
      return matchesSearch && match_status && match_screening_status;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, status, screening_status]);
  ...
  useEffect(() => { if (query || sort || status || screening_status) setPage(1); }, [query, sort, status, screening_status]);
  ...
  const handleDelete = async (row: RentalApplication) => {
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/rentalapplication/updateRentalApplication/{RentalApplicationID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "RentalApplicationID", type: "string", required: true, source: "record", options: [] }, { name: "status", type: "string", required: true, source: "constant", default: "SUBMITTED", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.rentalApplicationID) === recordId);
      ...
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            ...
            <select className="select" value={ screening_status } onChange={(e) => set_screening_status(e.target.value)}>
              <option value="">All</option>
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.rentalApplicationID ?? Math.random()}>
                    <td>{String(r?.applicationCode ?? '-')}</td>
                    <td>{r?.desiredMoveInDate ? new Date(r?.desiredMoveInDate as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.desiredMoveOutDate ? new Date(r?.desiredMoveOutDate as any).toLocaleDateString() : '-'}</td>
                    <td>{String(r?.status ?? '-')}</td>
                    <td>{String(r?.screeningStatus ?? '-')}</td>
                    <td>{r?.submittedAt ? new Date(r?.submittedAt as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.reviewedAt ? new Date(r?.reviewedAt as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.approvedAt ? new Date(r?.approvedAt as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.declinedAt ? new Date(r?.declinedAt as any).toLocaleDateString() : '-'}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.rentalApplicationID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.rentalApplicationID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.rentalApplicationID ?? '')] || {}),
                                right: 0,
...
export default RentalApplicationGrid;
