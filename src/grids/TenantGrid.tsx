...
import { fetchTenants } from '../apis/useTenant';
...
export interface TenantGridProps {
  ...
  onView?: (r: Tenant)=>void;
  onEdit?: (r: Tenant)=>void;
  onDelete?: (r: Tenant, setItems: React.Dispatch<React.SetStateAction<Tenant[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const TenantGrid: React.FC<TenantGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const [profile_status, set_profile_status] = useState<string>('');
  ...
  const load = useCallback(async () => {
    try {
      ...
      const params: any = env.SEARCH_STRATEGY === 'server' ? {
        ...
        profileStatus: profile_status || undefined,
      } : {
    ...
    ? [pageSize, page, query, sort, profile_status, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && profile_status === '') return items;

    let res = items.filter((r: Tenant) => {
      const s = (String((r as any)?.code ?? '') + ' ' + String((r as any)?.userID ?? '') + ' ' + String((r as any)?.description ?? '') + ' ' + String((r as any)?.phoneNumber ?? '') + ' ' + String((r as any)?.photoUrl ?? '') + ' ' + String((r as any)?.dateOfBirth ?? '') + ' ' + String((r as any)?.profileStatus ?? '') + ' ' + String((r as any)?.capturedBy ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.updatedBy ?? '') + ' ' + String((r as any)?.updatedDate ?? '')).toLowerCase();
      const matchesSearch = !query || s.includes(query.toLowerCase());
      const match_profile_status = !profile_status || String((r as any)?.profileStatus ?? '') === profile_status;
      return matchesSearch && match_profile_status;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, profile_status]);
  ...
  useEffect(() => { if (query || sort || profile_status) setPage(1); }, [query, sort, profile_status]);
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/tenant/updateTenant/{tenantID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "tenantID", type: "string", required: true, source: "record", options: [] }, { name: "profileStatus", type: "string", required: true, source: "constant", default: "COMPLETE", options: [] } ]
      }
    },
    {
      ...
      api: {
        ...
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "tenantID", type: "string", required: true, source: "record", options: [] }, { name: "profileStatus", type: "string", required: true, source: "constant", default: "VERIFIED", options: [] } ]
      }
    },
  ], []);
  ...
  function isActionVisible(action: any, row: any) {
    ...
    const v = String(row?.profileStatus ?? '');
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.tenantID) === recordId);
      ...
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            ...
            <select className="select" value={ profile_status } onChange={(e) => set_profile_status(e.target.value)}>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.tenantID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.tenantID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.tenantID ?? '')] || {}),
                                right: 0,
