...
export interface OrganizationMemberGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: OrganizationMember)=>void;
  onEdit?: (r: OrganizationMember)=>void;
  onDelete?: (r: OrganizationMember, setItems: React.Dispatch<React.SetStateAction<OrganizationMember[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const OrganizationMemberGrid: React.FC<OrganizationMemberGridProps> = ({
  ...
  const columns = [
    ...
    { key: "isPrimaryOwner", label: "Is Primary Owner" },
  ...
  const [lookup_userID, setLookup_userID] = useState<Record<string, string>>({});
  useEffect(() => {
    (async () => {
      try {
        const res = await fetchData<any>('api/account/getUsers?pageSize=1000&pageNumber=1', 'lookup_userID');
        const payload = (res as any)?.data;
        const arr = Array.isArray(payload?.data) ? payload.data : (Array.isArray(payload) ? payload : []);
        const map: Record<string, string> = {};
        arr.forEach((it: any) => { map[String(it?.id ?? it?.id ?? '')] = String(it?.displayName ?? it?.name ?? ''); });
        setLookup_userID(map);
      } catch { /* soft-fail */ }
    })();
  }, []);
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchOrganizationMembers(params);
    ...
    ? [pageSize, page, query, sort, status, role, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && status === '' && role === '') return items;

    let res = items.filter((r: OrganizationMember) => {
      const s = (String((r as any)?.organizationID ?? '') + ' ' + String((r as any)?.userID ?? '') + ' ' + String((r as any)?.roleName ?? '') + ' ' + String((r as any)?.status ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.capturedBy ?? '') + ' ' + String((r as any)?.updatedDate ?? '') + ' ' + String((r as any)?.updatedBy ?? '')).toLowerCase();
      const matchesSearch = !query || s.includes(query.toLowerCase());
      const match_status = !status || String((r as any)?.status ?? '') === status;
      const match_role = !role || String((r as any)?.roleName ?? '') === role;
      return matchesSearch && match_status && match_role;
    });
    if (sort) {
      const { key, dir } = sort;
  ...
  useEffect(() => { if (query || sort || status || role) setPage(1); }, [query, sort, status, role]);
  ...
  const handleDelete = async (row: OrganizationMember) => {
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/organizationmember/updateOrganizationMemberStatus/{OrganizationMemberID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "OrganizationMemberID", type: "string", required: true, source: "record", options: [] }, { name: "Status", type: "string", required: true, source: "constant", default: "ACTIVE", options: [] } ]
      }
    },
    {
      key: "deactivate",
      ...
      api: {
        ...
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "OrganizationMemberID", type: "string", required: true, source: "record", options: [] }, { name: "Status", type: "string", required: true, source: "constant", default: "DEACTIVATED", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.organizationMemberID) === recordId);
    ...
    <section className="card container">
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            <span>Role</span>
            <select className="select" value={ role } onChange={(e) => set_role(e.target.value)}>
              <option value="">All</option>
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.organizationMemberID ?? Math.random()}>
                    <td>{lookup_organizationID[String(r?.organizationID ?? '')] || String(r?.organizationID ?? '-')}</td>
                    <td>{lookup_userID[String(r?.userID ?? '')] || String(r?.userID ?? '-')}</td>
                    <td>{String(r?.roleName ?? '-')}</td>
                    <td>{String(r?.status ?? '-')}</td>
                    <td>{String(r?.isPrimaryOwner ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.organizationMemberID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.organizationMemberID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.organizationMemberID ?? '')] || {}),
                                right: 0,
