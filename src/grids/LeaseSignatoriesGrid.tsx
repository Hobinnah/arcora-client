...
import { fetchLeaseSignatories } from '../apis/useLeaseSignatories';
...
export interface LeaseSignatoriesGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: LeaseSignatories)=>void;
  onEdit?: (r: LeaseSignatories)=>void;
  onDelete?: (r: LeaseSignatories, setItems: React.Dispatch<React.SetStateAction<LeaseSignatories[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const LeaseSignatoriesGrid: React.FC<LeaseSignatoriesGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const [items, setItems] = useState<LeaseSignatories[]>([]);
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchLeaseSignatories(params);
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: LeaseSignatories) => {
      const s = (String((r as any)?.signatoryRole ?? '') + ' ' + String((r as any)?.name ?? '') + ' ' + String((r as any)?.email ?? '') + ' ' + String((r as any)?.status ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.capturedBy ?? '')).toLowerCase();
  ...
  const handleDelete = async (row: LeaseSignatories) => {
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/leasesignatories/updateLeaseSignatoriesStatus/{LeaseSignatoryID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "LeaseSignatoryID", type: "string", required: true, source: "record", options: [] }, { name: "Status", type: "string", required: true, source: "constant", default: "SIGNED", options: [] } ]
      }
    },
    {
      ...
      api: {
        ...
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "LeaseSignatoryID", type: "string", required: true, source: "record", options: [] }, { name: "Status", type: "string", required: true, source: "constant", default: "DECLINED", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.leaseSignatoryID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.leaseSignatoryID ?? Math.random()}>
                    <td>{String(r?.signatoryRole ?? '-')}</td>
                    <td>{String(r?.name ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.leaseSignatoryID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.leaseSignatoryID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.leaseSignatoryID ?? '')] || {}),
                                right: 0,
...
export default LeaseSignatoriesGrid;
