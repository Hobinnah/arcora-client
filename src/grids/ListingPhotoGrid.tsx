...
export interface ListingPhotoGridProps {
  ...
  onView?: (r: ListingPhoto)=>void;
  onEdit?: (r: ListingPhoto)=>void;
  onDelete?: (r: ListingPhoto, setItems: React.Dispatch<React.SetStateAction<ListingPhoto[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const ListingPhotoGrid: React.FC<ListingPhotoGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    ...
    { key: "isCoverPhoto", label: "Is Cover Photo" },
    { key: "capturedDate", label: "Captured Date" },
    { key: "capturedBy", label: "Captured By" },
  ] as const;

  const [items, setItems] = useState<ListingPhoto[]>([]);
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchListingPhotos(params);
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: ListingPhoto) => {
      const s = (String((r as any)?.url ?? '') + ' ' + String((r as any)?.location ?? '') + ' ' + String((r as any)?.caption ?? '') + ' ' + String((r as any)?.displayOrder ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.capturedBy ?? '')).toLowerCase();
  ...
  const handleDelete = async (row: ListingPhoto) => {
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.listingPhotoID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.listingPhotoID ?? Math.random()}>
                    <td>{String(r?.url ?? '-')}</td>
                    <td>{String(r?.location ?? '-')}</td>
                    <td>{String(r?.caption ?? '-')}</td>
                    <td>{String(r?.displayOrder ?? '-')}</td>
                    <td>{String(r?.isCoverPhoto ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.listingPhotoID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.listingPhotoID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.listingPhotoID ?? '')] || {}),
                                right: 0,
