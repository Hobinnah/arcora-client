...
import { fetchContractors } from '../apis/useContractor';
...
export interface ContractorGridProps {
  ...
  onView?: (r: Contractor)=>void;
  onEdit?: (r: Contractor)=>void;
  onDelete?: (r: Contractor, setItems: React.Dispatch<React.SetStateAction<Contractor[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const ContractorGrid: React.FC<ContractorGridProps> = ({
  ...
  const [filterOpts_category, setFilterOpts_category] = useState<{label:string;value:string}[]>([]);
  useEffect(() => {
    (async () => {
      try {
        const res = await fetchData<any>('api/contractorcategory/get?pageSize=1000&pageNumber=1', 'category');
        const payload = (res as any)?.data;
        const arr = Array.isArray(payload?.data) ? payload.data : (Array.isArray(payload) ? payload : []);
        setFilterOpts_category(arr.map((it:any) => ({ label: String(it?.categoryName ?? it?.name ?? ''), value: String(it?.categoryID ?? it?.id ?? '') })));
      } catch { /* soft-fail */ }
    })();
  }, []);
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchContractors(params);
    ...
    ? [pageSize, page, query, sort, status, category, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && status === '' && category === '') return items;

    let res = items.filter((r: Contractor) => {
      const s = (String((r as any)?.companyName ?? '') + ' ' + String((r as any)?.contactName ?? '') + ' ' + String((r as any)?.email ?? '') + ' ' + String((r as any)?.phoneNumber ?? '') + ' ' + String((r as any)?.status ?? '')).toLowerCase();
      const matchesSearch = !query || s.includes(query.toLowerCase());
      const match_status = !status || String((r as any)?.status ?? '') === status;
      const match_category = !category || String((r as any)?.categoryID ?? '') === category;
      return matchesSearch && match_status && match_category;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, status, category]);
  ...
  useEffect(() => { if (query || sort || status || category) setPage(1); }, [query, sort, status, category]);
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/contractor/updateContractor/{ContractorID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "ContractorID", type: "string", required: true, source: "record", options: [] }, { name: "status", type: "string", required: true, source: "constant", default: "INACTIVE", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.contractorID) === recordId);
    ...
    <section className="card container">
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            <span>Category</span>
            <select className="select" value={ category } onChange={(e) => set_category(e.target.value)}>
              <option value="">All</option>
              {filterOpts_category.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
            </select>
          </label>
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.contractorID ?? Math.random()}>
                    <td>{String(r?.companyName ?? '-')}</td>
                    <td>{String(r?.contactName ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.contractorID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.contractorID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.contractorID ?? '')] || {}),
                                right: 0,
