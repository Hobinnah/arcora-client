...
import { fetchSubscriptionPlans } from '../apis/useSubscriptionPlan';
...
export interface SubscriptionPlanGridProps {
  ...
  onView?: (r: SubscriptionPlan)=>void;
  onEdit?: (r: SubscriptionPlan)=>void;
  onDelete?: (r: SubscriptionPlan, setItems: React.Dispatch<React.SetStateAction<SubscriptionPlan[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const SubscriptionPlanGrid: React.FC<SubscriptionPlanGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    ...
    { key: "maxRentalUnits", label: "Max Rental Units" },
    { key: "maxActiveListings", label: "Max Active Listings" },
    { key: "maxOrganizationMembers", label: "Max Organization Members" },
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchSubscriptionPlans(params);
    ...
    ? [pageSize, page, query, sort, active_status, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && active_status === '') return items;

    let res = items.filter((r: SubscriptionPlan) => {
      const s = (String((r as any)?.code ?? '') + ' ' + String((r as any)?.name ?? '') + ' ' + String((r as any)?.monthlyPrice ?? '') + ' ' + String((r as any)?.annualPrice ?? '') + ' ' + String((r as any)?.currency ?? '') + ' ' + String((r as any)?.maxProperties ?? '') + ' ' + String((r as any)?.maxRentalUnits ?? '') + ' ' + String((r as any)?.maxActiveListings ?? '') + ' ' + String((r as any)?.maxOrganizationMembers ?? '')).toLowerCase();
      ...
      return matchesSearch && match_active_status;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, active_status]);
  ...
  useEffect(() => { if (query || sort || active_status) setPage(1); }, [query, sort, active_status]);
  ...
  const handleDelete = async (row: SubscriptionPlan) => {
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.subscriptionPlanID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.subscriptionPlanID ?? Math.random()}>
                    <td>{String(r?.code ?? '-')}</td>
                    <td>{String(r?.name ?? '-')}</td>
                    <td>{String(r?.monthlyPrice ?? '-')}</td>
                    <td>{String(r?.annualPrice ?? '-')}</td>
                    <td>{String(r?.currency ?? '-')}</td>
                    <td>{String(r?.maxProperties ?? '-')}</td>
                    <td>{String(r?.maxRentalUnits ?? '-')}</td>
                    <td>{String(r?.maxActiveListings ?? '-')}</td>
                    <td>{String(r?.maxOrganizationMembers ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.subscriptionPlanID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.subscriptionPlanID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.subscriptionPlanID ?? '')] || {}),
                                right: 0,
...
export default SubscriptionPlanGrid;
