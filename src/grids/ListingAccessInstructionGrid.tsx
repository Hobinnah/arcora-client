...
import { fetchListingAccessInstructions } from '../apis/useListingAccessInstruction';
...
export interface ListingAccessInstructionGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: ListingAccessInstruction)=>void;
  onEdit?: (r: ListingAccessInstruction)=>void;
  onDelete?: (r: ListingAccessInstruction, setItems: React.Dispatch<React.SetStateAction<ListingAccessInstruction[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const ListingAccessInstructionGrid: React.FC<ListingAccessInstructionGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    { key: "instructionType", label: "Instruction Type" },
  ...
  const [items, setItems] = useState<ListingAccessInstruction[]>([]);
  ...
  const [instruction_type, set_instruction_type] = useState<string>('');
  ...
  const load = useCallback(async () => {
    try {
      ...
      const params: any = env.SEARCH_STRATEGY === 'server' ? {
        ...
        instructionType: instruction_type || undefined,
      } : {
      ...
      const result = await fetchListingAccessInstructions(params);
    ...
    ? [pageSize, page, query, sort, active, instruction_type, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && active === '' && instruction_type === '') return items;

    let res = items.filter((r: ListingAccessInstruction) => {
      const s = (String((r as any)?.instructionType ?? '') + ' ' + String((r as any)?.availableFrom ?? '') + ' ' + String((r as any)?.availableUntil ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.capturedBy ?? '') + ' ' + String((r as any)?.updatedDate ?? '') + ' ' + String((r as any)?.updatedBy ?? '')).toLowerCase();
      const matchesSearch = !query || s.includes(query.toLowerCase());
      const match_active = !active || String((r as any)?.isActive ?? '') === active;
      const match_instruction_type = !instruction_type || String((r as any)?.instructionType ?? '') === instruction_type;
      return matchesSearch && match_active && match_instruction_type;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, active, instruction_type]);
  ...
  useEffect(() => { if (query || sort || active || instruction_type) setPage(1); }, [query, sort, active, instruction_type]);
  ...
  const handleDelete = async (row: ListingAccessInstruction) => {
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.listingAccessInstructionID) === recordId);
      ...
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            <span>Active</span>
            <select className="select" value={ active } onChange={(e) => set_active(e.target.value)}>
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            ...
            <input className="input" placeholder="All" value={ instruction_type } onChange={(e) => set_instruction_type(e.target.value)} />
          </label>
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.listingAccessInstructionID ?? Math.random()}>
                    <td>{String(r?.instructionType ?? '-')}</td>
                    <td>{r?.availableFrom ? new Date(r?.availableFrom as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.availableUntil ? new Date(r?.availableUntil as any).toLocaleDateString() : '-'}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.listingAccessInstructionID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.listingAccessInstructionID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.listingAccessInstructionID ?? '')] || {}),
                                right: 0,
...
export default ListingAccessInstructionGrid;
