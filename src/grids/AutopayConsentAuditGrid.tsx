...
import { fetchAutopayConsentAudits } from '../apis/useAutopayConsentAudit';
...
export interface AutopayConsentAuditGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: AutopayConsentAudit)=>void;
  onEdit?: (r: AutopayConsentAudit)=>void;
  onDelete?: (r: AutopayConsentAudit, setItems: React.Dispatch<React.SetStateAction<AutopayConsentAudit[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const AutopayConsentAuditGrid: React.FC<AutopayConsentAuditGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    ...
    { key: "actionAt", label: "Action At" },
  ] as const;

  const [items, setItems] = useState<AutopayConsentAudit[]>([]);
  ...
  const [action_date, set_action_date] = useState<string>('');
  ...
  const load = useCallback(async () => {
    try {
      ...
      const params: any = env.SEARCH_STRATEGY === 'server' ? {
        ...
        actionAt: action_date || undefined,
      } : {
      ...
      const result = await fetchAutopayConsentAudits(params);
    ...
    ? [pageSize, page, query, sort, action, action_date, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && action === '' && action_date === '') return items;

    let res = items.filter((r: AutopayConsentAudit) => {
      const s = (String((r as any)?.action ?? '') + ' ' + String((r as any)?.providerReferenceID ?? '') + ' ' + String((r as any)?.actionAt ?? '')).toLowerCase();
      const matchesSearch = !query || s.includes(query.toLowerCase());
      const match_action = !action || String((r as any)?.action ?? '') === action;
      const match_action_date = !action_date || String((r as any)?.actionAt ?? '') === action_date;
      return matchesSearch && match_action && match_action_date;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, action, action_date]);
  ...
  useEffect(() => { if (query || sort || action || action_date) setPage(1); }, [query, sort, action, action_date]);
  ...
  const handleDelete = async (row: AutopayConsentAudit) => {
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.autopayConsentAuditID) === recordId);
      ...
      <div className="card-header">
        <div className="datatable-controls">
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            <span>Action</span>
            <select className="select" value={ action } onChange={(e) => set_action(e.target.value)}>
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginRight: 8 }}>
            ...
            <input className="input" placeholder="All" value={ action_date } onChange={(e) => set_action_date(e.target.value)} />
          </label>
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.autopayConsentAuditID ?? Math.random()}>
                    <td>{String(r?.action ?? '-')}</td>
                    <td>{String(r?.providerReferenceID ?? '-')}</td>
                    <td>{r?.actionAt ? new Date(r?.actionAt as any).toLocaleDateString() : '-'}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.autopayConsentAuditID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.autopayConsentAuditID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.autopayConsentAuditID ?? '')] || {}),
                                right: 0,
...
export default AutopayConsentAuditGrid;
