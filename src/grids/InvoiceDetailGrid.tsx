...
import { fetchInvoiceDetails } from '../apis/useInvoiceDetail';
...
export interface InvoiceDetailGridProps {
  ...
  onView?: (r: InvoiceDetail)=>void;
  onEdit?: (r: InvoiceDetail)=>void;
  onDelete?: (r: InvoiceDetail, setItems: React.Dispatch<React.SetStateAction<InvoiceDetail[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const InvoiceDetailGrid: React.FC<InvoiceDetailGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    ...
    { key: "servicePeriodStart", label: "Service Period Start" },
    { key: "servicePeriodEnd", label: "Service Period End" },
    { key: "quantity", label: "Quantity" },
    { key: "unitAmount", label: "Unit Amount" },
    { key: "lineAmount", label: "Line Amount" },
    { key: "taxRate", label: "Tax Rate" },
    { key: "taxAmount", label: "Tax Amount" },
    { key: "totalLineAmount", label: "Total Line Amount" },
  ] as const;
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchInvoiceDetails(params);
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: InvoiceDetail) => {
      const s = (String((r as any)?.lineType ?? '') + ' ' + String((r as any)?.description ?? '') + ' ' + String((r as any)?.servicePeriodStart ?? '') + ' ' + String((r as any)?.servicePeriodEnd ?? '') + ' ' + String((r as any)?.quantity ?? '') + ' ' + String((r as any)?.unitAmount ?? '') + ' ' + String((r as any)?.lineAmount ?? '') + ' ' + String((r as any)?.taxRate ?? '') + ' ' + String((r as any)?.taxAmount ?? '') + ' ' + String((r as any)?.totalLineAmount ?? '')).toLowerCase();
  ...
  const handleDelete = async (row: InvoiceDetail) => {
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.invoiceDetailID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.invoiceDetailID ?? Math.random()}>
                    <td>{String(r?.lineType ?? '-')}</td>
                    <td>{String(r?.description ?? '-')}</td>
                    <td>{r?.servicePeriodStart ? new Date(r?.servicePeriodStart as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.servicePeriodEnd ? new Date(r?.servicePeriodEnd as any).toLocaleDateString() : '-'}</td>
                    <td>{String(r?.quantity ?? '-')}</td>
                    <td>{String(r?.unitAmount ?? '-')}</td>
                    <td>{String(r?.lineAmount ?? '-')}</td>
                    <td>{String(r?.taxRate ?? '-')}</td>
                    <td>{String(r?.taxAmount ?? '-')}</td>
                    <td>{String(r?.totalLineAmount ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.invoiceDetailID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.invoiceDetailID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.invoiceDetailID ?? '')] || {}),
                                right: 0,
