...
import { fetchPayoutItems } from '../apis/usePayoutItem';
...
export interface PayoutItemGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: PayoutItem)=>void;
  onEdit?: (r: PayoutItem)=>void;
  onDelete?: (r: PayoutItem, setItems: React.Dispatch<React.SetStateAction<PayoutItem[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const PayoutItemGrid: React.FC<PayoutItemGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    ...
    { key: "netPayoutAmount", label: "Net Payout Amount" },
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchPayoutItems(params);
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: PayoutItem) => {
      const s = (String((r as any)?.grossAmount ?? '') + ' ' + String((r as any)?.deductionAmount ?? '') + ' ' + String((r as any)?.netPayoutAmount ?? '') + ' ' + String((r as any)?.description ?? '')).toLowerCase();
  ...
  const handleDelete = async (row: PayoutItem) => {
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.payoutItemID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.payoutItemID ?? Math.random()}>
                    <td>{String(r?.grossAmount ?? '-')}</td>
                    <td>{String(r?.deductionAmount ?? '-')}</td>
                    <td>{String(r?.netPayoutAmount ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.payoutItemID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.payoutItemID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.payoutItemID ?? '')] || {}),
                                right: 0,
