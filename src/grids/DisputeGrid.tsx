...
import { fetchDisputes } from '../apis/useDispute';
...
export interface DisputeGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: Dispute)=>void;
  onEdit?: (r: Dispute)=>void;
  onDelete?: (r: Dispute, setItems: React.Dispatch<React.SetStateAction<Dispute[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const DisputeGrid: React.FC<DisputeGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: Dispute) => {
      const s = (String((r as any)?.status ?? '') + ' ' + String((r as any)?.title ?? '') + ' ' + String((r as any)?.description ?? '') + ' ' + String((r as any)?.resolutionNotes ?? '') + ' ' + String((r as any)?.openedAt ?? '') + ' ' + String((r as any)?.resolvedAt ?? '')).toLowerCase();
  ...
  const handleDelete = async (row: Dispute) => {
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/dispute/updateDispute/{DisputeID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "DisputeID", type: "string", required: true, source: "record", options: [] }, { name: "status", type: "string", required: true, source: "constant", default: "ACTIVE", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.disputeID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.disputeID ?? Math.random()}>
                    <td>{String(r?.status ?? '-')}</td>
                    <td>{String(r?.title ?? '-')}</td>
                    <td>{String(r?.description ?? '-')}</td>
                    <td>{String(r?.resolutionNotes ?? '-')}</td>
                    <td>{r?.openedAt ? new Date(r?.openedAt as any).toLocaleDateString() : '-'}</td>
                    <td>{r?.resolvedAt ? new Date(r?.resolvedAt as any).toLocaleDateString() : '-'}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.disputeID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.disputeID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.disputeID ?? '')] || {}),
                                right: 0,
