...
import { fetchListingRules } from '../apis/useListingRule';
...
export interface ListingRuleGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: ListingRule)=>void;
  onEdit?: (r: ListingRule)=>void;
  onDelete?: (r: ListingRule, setItems: React.Dispatch<React.SetStateAction<ListingRule[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const ListingRuleGrid: React.FC<ListingRuleGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const [items, setItems] = useState<ListingRule[]>([]);
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchListingRules(params);
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: ListingRule) => {
      const s = (String((r as any)?.ruleType ?? '') + ' ' + String((r as any)?.ruleTitle ?? '') + ' ' + String((r as any)?.ruleDescription ?? '') + ' ' + String((r as any)?.effectiveFrom ?? '') + ' ' + String((r as any)?.effectiveTo ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.updatedDate ?? '')).toLowerCase();
  ...
  const handleDelete = async (row: ListingRule) => {
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.listingRuleID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.listingRuleID ?? Math.random()}>
                    <td>{String(r?.ruleType ?? '-')}</td>
                    <td>{String(r?.ruleTitle ?? '-')}</td>
                    <td>{String(r?.ruleDescription ?? '-')}</td>
                    <td>{String(r?.isAllowed ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.listingRuleID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.listingRuleID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.listingRuleID ?? '')] || {}),
                                right: 0,
...
export default ListingRuleGrid;
