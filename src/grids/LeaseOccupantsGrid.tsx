...
import { fetchLeaseOccupants } from '../apis/useLeaseOccupants';
...
export interface LeaseOccupantsGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: LeaseOccupants)=>void;
  onEdit?: (r: LeaseOccupants)=>void;
  onDelete?: (r: LeaseOccupants, setItems: React.Dispatch<React.SetStateAction<LeaseOccupants[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const LeaseOccupantsGrid: React.FC<LeaseOccupantsGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    ...
    { key: "isPrimaryTenant", label: "Is Primary Tenant" },
    { key: "isFinanciallyResponsible", label: "Is Financially Responsible" },
  ...
  const [items, setItems] = useState<LeaseOccupants[]>([]);
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchLeaseOccupants(params);
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: LeaseOccupants) => {
      const s = (String((r as any)?.firstName ?? '') + ' ' + String((r as any)?.lastName ?? '') + ' ' + String((r as any)?.email ?? '') + ' ' + String((r as any)?.phoneNumber ?? '') + ' ' + String((r as any)?.occupantType ?? '') + ' ' + String((r as any)?.status ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.capturedBy ?? '') + ' ' + String((r as any)?.updatedDate ?? '') + ' ' + String((r as any)?.updatedBy ?? '')).toLowerCase();
  ...
  const handleDelete = async (row: LeaseOccupants) => {
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/leaseoccupants/updateLeaseOccupantsStatus/{LeaseOccupantID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "LeaseOccupantID", type: "string", required: true, source: "record", options: [] }, { name: "Status", type: "string", required: true, source: "constant", default: "INACTIVE", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.leaseOccupantID) === recordId);
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.leaseOccupantID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.leaseOccupantID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.leaseOccupantID ?? '')] || {}),
                                right: 0,
...
export default LeaseOccupantsGrid;
