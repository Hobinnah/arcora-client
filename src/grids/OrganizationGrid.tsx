...
import { fetchOrganizations } from '../apis/useOrganization';
...
export interface OrganizationGridProps {
  ...
  onView?: (r: Organization)=>void;
  onEdit?: (r: Organization)=>void;
  onDelete?: (r: Organization, setItems: React.Dispatch<React.SetStateAction<Organization[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const OrganizationGrid: React.FC<OrganizationGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const columns = [
    ...
    { key: "isPersonal", label: "Is Personal" },
    { key: "status", label: "Status" },
    { key: "defaultCurrency", label: "Default Currency" },
    { key: "lateFeeEnabled", label: "Late Fee Enabled" },
    { key: "autoInvoiceGeneration", label: "Auto Invoice Generation" },
    { key: "autoPaymentRetry", label: "Auto Payment Retry" },
    { key: "requireBackgroundCheck", label: "Require Background Check" },
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchOrganizations(params);
    ...
    ? [pageSize, page, query, sort, status, country, onAlert]
    : [pageSize, onAlert]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if ( !query && !sort && status === '' && country === '') return items;

    let res = items.filter((r: Organization) => {
      const s = (String((r as any)?.legalName ?? '') + ' ' + String((r as any)?.displayName ?? '') + ' ' + String((r as any)?.countryCode ?? '') + ' ' + String((r as any)?.provinceCode ?? '') + ' ' + String((r as any)?.status ?? '') + ' ' + String((r as any)?.defaultCurrency ?? '') + ' ' + String((r as any)?.capturedDate ?? '') + ' ' + String((r as any)?.capturedBy ?? '') + ' ' + String((r as any)?.updatedDate ?? '') + ' ' + String((r as any)?.updatedBy ?? '')).toLowerCase();
      ...
      return matchesSearch && match_status && match_country;
    });
    if (sort) {
      const { key, dir } = sort;
    ...
    return res;
  }, [items, query, sort, status, country]);
  ...
  useEffect(() => { if (query || sort || status || country) setPage(1); }, [query, sort, status, country]);
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/organization/updateOrganizationStatus/{OrganizationID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "OrganizationID", type: "string", required: true, source: "record", options: [] }, { name: "Status", type: "string", required: true, source: "constant", default: "CLOSED", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.organizationID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.organizationID ?? Math.random()}>
                    <td>{String(r?.legalName ?? '-')}</td>
                    <td>{String(r?.displayName ?? '-')}</td>
                    <td>{String(r?.countryCode ?? '-')}</td>
                    <td>{String(r?.provinceCode ?? '-')}</td>
                    <td>{String(r?.isPersonal ?? '-')}</td>
                    <td>{String(r?.status ?? '-')}</td>
                    <td>{String(r?.defaultCurrency ?? '-')}</td>
                    <td>{String(r?.lateFeeEnabled ?? '-')}</td>
                    <td>{String(r?.autoInvoiceGeneration ?? '-')}</td>
                    <td>{String(r?.autoPaymentRetry ?? '-')}</td>
                    <td>{String(r?.requireBackgroundCheck ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.organizationID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.organizationID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.organizationID ?? '')] || {}),
                                right: 0,
