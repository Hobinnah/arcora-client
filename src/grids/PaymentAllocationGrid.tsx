...
import { fetchPaymentAllocations } from '../apis/usePaymentAllocation';
...
export interface PaymentAllocationGridProps {
  ...
  onView?: (r: PaymentAllocation)=>void;
  onEdit?: (r: PaymentAllocation)=>void;
  onDelete?: (r: PaymentAllocation, setItems: React.Dispatch<React.SetStateAction<PaymentAllocation[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const PaymentAllocationGrid: React.FC<PaymentAllocationGridProps> = ({
  ...
  const columns = [
    ...
    { key: "allocatedAt", label: "Allocated At" },
  ] as const;

  const [items, setItems] = useState<PaymentAllocation[]>([]);
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchPaymentAllocations(params);
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: PaymentAllocation) => {
      const s = (String((r as any)?.allocatedAmount ?? '') + ' ' + String((r as any)?.allocationType ?? '') + ' ' + String((r as any)?.allocatedAt ?? '')).toLowerCase();
  ...
  const handleDelete = async (row: PaymentAllocation) => {
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.paymentAllocationID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.paymentAllocationID ?? Math.random()}>
                    <td>{String(r?.allocatedAmount ?? '-')}</td>
                    <td>{String(r?.allocationType ?? '-')}</td>
                    <td>{r?.allocatedAt ? new Date(r?.allocatedAt as any).toLocaleDateString() : '-'}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.paymentAllocationID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.paymentAllocationID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.paymentAllocationID ?? '')] || {}),
                                right: 0,
