...
import { fetchCreditReportingConsentAudits } from '../apis/useCreditReportingConsentAudit';
...
export interface CreditReportingConsentAuditGridProps {
  initialPageSize?: number; // default: 5
  onView?: (r: CreditReportingConsentAudit)=>void;
  onEdit?: (r: CreditReportingConsentAudit)=>void;
  onDelete?: (r: CreditReportingConsentAudit, setItems: React.Dispatch<React.SetStateAction<CreditReportingConsentAudit[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const CreditReportingConsentAuditGrid: React.FC<CreditReportingConsentAuditGridProps> = ({
  initialPageSize = 5,
  onView,
  onEdit,
  onDelete,
  ...
  const [items, setItems] = useState<CreditReportingConsentAudit[]>([]);
  ...
  const load = useCallback(async () => {
    try {
      ...
      const result = await fetchCreditReportingConsentAudits(params);
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: CreditReportingConsentAudit) => {
      const s = (String((r as any)?.action ?? '') + ' ' + String((r as any)?.actionAt ?? '')).toLowerCase();
  ...
  const handleDelete = async (row: CreditReportingConsentAudit) => {
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.consentAuditID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.consentAuditID ?? Math.random()}>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.consentAuditID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.consentAuditID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.consentAuditID ?? '')] || {}),
                                right: 0,
...
export default CreditReportingConsentAuditGrid;
