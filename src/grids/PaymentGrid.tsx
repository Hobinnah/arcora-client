...
import { fetchPayments } from '../apis/usePayment';
...
export interface PaymentGridProps {
  ...
  onView?: (r: Payment)=>void;
  onEdit?: (r: Payment)=>void;
  onDelete?: (r: Payment, setItems: React.Dispatch<React.SetStateAction<Payment[]>>, setError: React.Dispatch<React.SetStateAction<string | null>>) => Promise<void>;
...
const PaymentGrid: React.FC<PaymentGridProps> = ({
  ...
  const columns = [
    ...
    { key: "providerChargeID", label: "Provider Charge ID" },
    { key: "status", label: "Status" },
    { key: "grossAmount", label: "Gross Amount" },
    { key: "platformFeeAmount", label: "Platform Fee Amount" },
    { key: "processorFeeAmount", label: "Processor Fee Amount" },
  ...
  const filtered = useMemo(() => {
    ...
    let res = items.filter((r: Payment) => {
      const s = (String((r as any)?.tenantID ?? '') + ' ' + String((r as any)?.providerChargeID ?? '') + ' ' + String((r as any)?.status ?? '') + ' ' + String((r as any)?.grossAmount ?? '') + ' ' + String((r as any)?.platformFeeAmount ?? '') + ' ' + String((r as any)?.processorFeeAmount ?? '') + ' ' + String((r as any)?.refundedAmount ?? '') + ' ' + String((r as any)?.netAmount ?? '') + ' ' + String((r as any)?.currency ?? '') + ' ' + String((r as any)?.paidAt ?? '') + ' ' + String((r as any)?.capturedBy ?? '') + ' ' + String((r as any)?.capturedDate ?? '')).toLowerCase();
  ...
  const actions = useMemo(() => [
    {
      ...
      api: {
        method: "POST",
        pathTemplate: "api/payment/updatePayment/{PaymentID}",
        queryTemplate: {  },
        headers: {  },
        bodyTemplate: null,
        useFormData: false,
        requiresConfirm: true,
        params: [ { name: "PaymentID", type: "string", required: true, source: "record", options: [] }, { name: "status", type: "string", required: true, source: "constant", default: "PROCESSING", options: [] } ]
      }
    },
  ], []);
  ...
  const toggleDropdown = (recordId: string, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const record = (items || []).find(r => String((r as any)?.paymentID) === recordId);
          ...
          <table className="table">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                current.map(r => r && (
                  <tr key={r?.paymentID ?? Math.random()}>
                    <td>{lookup_tenantID[String(r?.tenantID ?? '')] || String(r?.tenantID ?? '-')}</td>
                    <td>{String(r?.providerChargeID ?? '-')}</td>
                    <td>{String(r?.status ?? '-')}</td>
                    <td>{String(r?.grossAmount ?? '-')}</td>
                    <td>{String(r?.platformFeeAmount ?? '-')}</td>
                    <td>{String(r?.processorFeeAmount ?? '-')}</td>
                    <td>{String(r?.refundedAmount ?? '-')}</td>
                        ...
                        {showActionDropdown && getQualifyingActions(r).length > 0 && (
                          <>
                            <button
                              className="icon-link"
                              title="More actions"
                              onClick={(e) => toggleDropdown(String(r?.paymentID ?? ''), e)}
                              style={{ fontSize: '14px', padding: '4px 8px', color: 'currentColor' }}
                            >⋮</button>
                            {activeDropdown === String(r?.paymentID ?? '') && (
                              <div style={{
                                position: 'absolute',
                                ...(dropdownPosition[String(r?.paymentID ?? '')] || {}),
                                right: 0,
